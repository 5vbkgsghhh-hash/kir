...
THE PINNED DECISIONS (Sonnet waves tile water-supply/heating-ventilation/electrical chains on these; see
...
Revit to elbow a straight line. See KIR_CONNECT_SPEC.md §2 ("a diameter
transition at a straight joint -> NewTransitionFitting") and the wiki
(data/wiki/pages/mep/duct-routing.md: "An elbow fitting goes only at a turn
(two non-collinear segments)"; fittings-and-connectors.md's verified "ConnectTo vs
...
from kir.diag import (Diagnostic, TYPE_BAD_TYPE, TYPE_BOUNDS,
                      TYPE_GEOM_RELATION,
                      # An ad-hoc literal `"KIR-L003"` lived here without a name and
                      # was exactly the precedent that `route_mep` referenced,
                      # taking someone else's `L004`. The name belongs to the allocator.
                      PLAN_REF_ORDER)
...
# THE SAME bound — and now it READS instead of being repeated. The note «same
# static sanity bound as authoring» has stood here from the start: the author
# KNEW about the copy and left it. A copy that is known about is still a copy.
...
def graph_validate(op: dict, oid, diags: list, default_dia: Optional[float],
    ...
    for si, seg in enumerate(segs_raw):
        ...
        for end, lbl in ((a, "from"), (b, "to")):
            ...
            if end not in nodes:
                diags.append(Diagnostic(
                    code=PLAN_REF_ORDER, op_id=oid, field_name=f"segments[{si}].{lbl}",
