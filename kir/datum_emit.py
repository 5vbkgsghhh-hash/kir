...
from kir.emit_core import (  # noqa: F401
    _cs, _eid, _gid, _level_expr,
    _pt3, _safe, _stamp_block, _stamp_readback,
...
def emit_extrusion_roof(op: dict, ver: str, stamp: str,
    ...
    # "roof"). Ни `GetElement`, ни сторож дрейфа не нужны и были бы ложью:
    # элемент создан выше, в этой же транзакции. Та же ветка и тот же довод,
    # что у `create_wall.type` с 23.08.
    ...
    post = [
        ...
        WitnessCheck(
            ...
            reader_cs=(f"    var __blp_{s} = __rr_{s} == null ? null : "
                       f"__rr_{s}.get_Parameter(BuiltInParameter.ROOF_CONSTRAINT_LEVEL_PARAM);\n"),
            verdict_cs=(
...
def emit_multistory_stairs(op: dict, ver: str, stamp: str,
    ...
    # `kir/tests/test_datums_multistory_net48.py` покраснеет и потребует
    # решения.
    if ver < "2025":
...
EMITTERS = {
    "create_extrusion_roof": emit_extrusion_roof,
    "create_multi_segment_grid": emit_multi_segment_grid,
    "create_multistory_stairs": emit_multistory_stairs,
}
