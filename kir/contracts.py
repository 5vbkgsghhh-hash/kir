...
class ElementIdentityProof:
    ...
    detects an address resolving to a different element. ``VersionGuid``
    identifies a saved-element version, not every in-session edit: Revit API
    documentation explicitly says it cannot detect changes between saves or
    synchronization actions. Comparing these fields is an identity/version
    guard, NOT a freshness proof for observed values. Writes based on a prior
    observation also need its document identity and DocumentChanged revision
    checked at the execution boundary (or an appropriate value precondition).
    """
...
class CoverageProof:
    ...
    def from_dict(
        ...
        *,
        ...
        # 🔴 ABSENCE IS NOT ZERO (29.08.2026, audit finding F-119). Previously
        # a missing `element_count`/`link_count` silently became zero, and
        # two DIFFERENT facts — "we read the document, and it has zero
        # walls" and "the source did not say how many walls" — collapsed
        # into one record, `element_count: 0`. Zero is a NUMBER, and from
        # then on it travels as a fact ABOUT THE BUILDING; the absence of a
        # measurement is not a fact about the building.
        #
        # A refusal here, not a `None` default: `None` would have to be
        # threaded through every reader of the counter, and the first one to
        # forget would get a `TypeError` right in front of the user. A
        # refusal at parse time hits whoever ASSEMBLED the incomplete proof
        # — and that is the actual defect.
        #
        # MEASURED BEFORE LAYING THIS DOWN (29.08.2026), because a refusal on
        # a legitimate input is forbidden: the only writer is this same
        # class's `to_dict`, and it always lays down both counters; in the
        # live A5 corpus journals, 16 of 16 manifests carry counters, and
        # those WITHOUT them are ZERO of them; there is one schema
        # (`snapshot-manifest/1`), it has no prior versions. So an input
        # without counters is ALREADY invalid, and the refusal cannot fire
        # on a legitimate one.
        if "element_count" not in row or "link_count" not in row:
            raise ContractSchemaError(
                "coverage proof без element_count/link_count: отсутствие "
                "счётчика не есть ноль. Источник, который их не знает, обязан "
                "сказать это отдельно, а не промолчать")
        proof = cls(
...
class CommitReceipt:
    ...
    #: Per-op refusals INSIDE a committed transaction (/3, 29.07).
    #: Each row: {"op_id", "op_name", "intent", "reason"}. `reason` is the
    #: bridge's verbatim text, unabridged.
    #:
    #: THE CAUSE IS MEASURED (rebuild #11, v18): 113 curtain-wall cut lines
    #: failed per-op INSIDE successful transactions. The run reported
    #: "Committed", the lines ended up in missing, and not one word about
    #: the reason was retained — the receipt carried only element_ids. Yet
    #: the reason had been surfacing from the start: under
    #: isolation="per_op" emission puts it in the very same `result` that
    #: ids are taken from (`__rf["refused"]`).
    op_refusals: tuple[Mapping[str, Any], ...] = ()
    #: Total ops in the chunk's program, and ops that closed without an
    #: element being born, BY SEMANTICS (`created:false` — a panel type
    #: swapped in place). Zero in ``ops_total`` means the census law is
    #: switched off: this is how journals written before /3 are read.
    ops_total: int = 0
    ...
    def __post_init__(self) -> None:
        ...
        # 🔴 THE THIRD COMBINATION, AND IT IS WORSE THAN THE FIRST TWO
        # (29.08.2026, audit finding F-118). What was checked was
        # "confirmation with a bridge error" and "confirmation with a
        # non-Committed status". NOT checked — a `Committed` status WITHOUT
        # confirmation, with a bridge error: such a receipt used to pass and
        # land in `refused_without_commit`, whose docstring promises the
        # opposite — "Revit answered, nothing was created, the program is
        # closed". The one substantive field in it says "committed".
        #
        # The cost of an error here is asymmetric: `decided == True` closes
        # the program, and that decision lets it be RETRIED — that is, lets
        # something Revit may have already recorded be recorded a second
        # time. The outcome here is UNKNOWN, not negative.
        #
        # A refusal, not a silent reclassification: somebody ASSEMBLED such
        # a receipt, and that somebody is the defect; the refusal names it.
        # Who turns it into an unknown is the caller's decision — it has
        # `timeout_unconfirmed` (named in `refused_without_commit`'s
        # docstring).
        if (self.bridge_error and not self.commit_confirmed
                and self.commit_status == "Committed"):
            raise ContractSchemaError(
                "commit_receipt: статус Committed при ошибке моста и без "
                "подтверждения — исход НЕИЗВЕСТЕН, а не отрицателен. Такую "
                "квитанцию нельзя ни подтвердить, ни объявить отказом; "
                "приносящий обязан отдать её как timeout_unconfirmed")
        refusals = tuple(
        ...
        # THE CHUNK CENSUS LAW: every op must have an OUTCOME.
        #
        #     created + refused + closed-without-an-element == ops
        #
        # A mismatch is not a reporting nicety, it is the "silently not
        # created" class: an op nobody knows anything about. v18
        # measurement: chunk 6 = 198 + 37 + 15 = 250 reconciles, and exactly
        # those 37 were invisible without the law.
        #
        # THE ASSUMPTION BAKED IN HERE: ONE OP CREATES NO MORE THAN ONE
        # ELEMENT, so the number of created ids can be equated to the number
        # of successful ops. Today, in the rebuild, this holds (the v18
        # retro-balance reconciled on all committed chunks), but this is A
        # PROPERTY OF THE CURRENT SET OF OPS, not a law of nature: a
        # multi-element op (move_elements with a set of targets is already
        # close) will break the equality, and break it HONESTLY, in red. To
        # the author of such an op: the law needs to be counted by OPS, not
        # by ids (carry ops_created in the receipt), not have the comparison
        # weakened. See test_law_assumes_one_element_per_op.
        if self.ops_total:
    ...
    def refused_without_commit(self) -> bool:
        """A known NEGATIVE outcome: Revit answered, nothing was created.

        A refusal is evidence too. Telling it apart from UNKNOWNNESS is
        mandatory: with `timeout_unconfirmed` there is no answer at all,
        Revit may have committed, and such an effect remains a hole in plan
        coverage. Here, by contrast, an answer was received, the
        transaction was rolled back, there are no element witnesses — the
        program is closed.
        """
        ...
        return (
            ...
            and self.commit_status == "RolledBack"
    ...
    def decided(self) -> bool:
        """The outcome is KNOWN: either the commit is confirmed, or the refusal is witnessed."""
    ...
    def from_dict(cls, value: Any) -> "CommitReceipt":
        ...
        return cls(
            ...
            # THE /3 FIELDS ARE OPTIONAL: journals from runs #9-#11 do not
            # have them, and ``ops_total=0`` switches off the census law —
            # old evidence stays readable, as the contracts' general rule
            # requires.
            op_refusals=tuple(
...
class IdempotenceMetrics:
    ...
    canon_version: str
    #: 🔴 WAS THERE ANYTHING TO COMPARE (30.08.2026, audit finding E-43).
    #:
    #: `_compare` on an empty stand honestly gives `multiset_match=True`: an
    #: empty multiset equals an empty one, the hashes match. But 100% match
    #: over NOTHING proves no idempotence — it proves there was nothing to
    #: measure, and the reader had no way to tell one from the other: both
    #: records looked like "the comparison happened and matched".
    #:
    #: WHY A SEPARATE FIELD, NOT `comparison_performed=False`. The fix's
    #: first edition did exactly that — and broke a LEGITIMATE input: a run
    #: where all the work lies in atoms (`atom_escrow`) has an empty op
    #: multiset BY CONSTRUCTION, and `a5_recovery` rejects a COMPARED proof
    #: with no comparison ("Compared proof has no comparison"). That is, an
    #: honest atomic run would have failed outright. Caught by the suite
    #: (`test_exact_post_commit_surface_raises_coverage`, whose own comment
    #: reads "semantic multiset is empty"), not by reasoning.
    #:
    #: The `False` default preserves the old reading of old records: no
    #: field means the volume was not stated, and this is NOT the claim
    #: "the volume was non-empty".
    comparison_volume_zero: bool = False
    ...
    def __post_init__(self) -> None:
        ...
        # 🔴 A MULTISET MATCH IS ARITHMETIC, NOT AN OPINION (29.08.2026,
        # audit finding F-120). Previously `multiset_match=True` was
        # accepted with ANY counters: "the sets matched" with 100 expected
        # and 7 found. The idempotence stand exists for the sake of ONE
        # number — did it reconcile or not — and that number cannot be
        # declared against its own counters.
        #
        # A reconciliation, NOT a computation: `multiset_match` is set by
        # the comparing side, which knows more about the multiset than four
        # numbers. Computing it would take away its authorship; a
        # reconciliation only forbids a contradiction, and these are
        # different things.
        #
        # A refusal cannot fire on a legitimate input, and this is
        # MEASURED, not just declared: 8 live `idempotence.json` corpus
        # reports, in ALL of which `multiset_match` agrees with the
        # counters; an empty stand (all zeros) is NOT touched by this check
        # — 0 == 0 with extra == 0.
        #
        # 🔴 ONLY THE KNOWN IS ASKED FOR, and this is NOT a softening but a
        # correction PAID FOR IN RED. The first edition (as in the package)
        # required `expected == actual == matched`, and the suite rejected
        # it: for the TRANSITIONAL "recall only" shape
        # (`test_legacy_recall_only_metrics_remain_
        # readable_without_fake_precision`) `total_actual` and `total_extra`
        # are LEGITIMATELY None — the run knew the expected and matched
        # counts, but not what was found. Requiring equality against an
        # UNKNOWN means rejecting a legitimate input, that is, exactly the
        # separator this check exists for. Not knowing a counter is not a
        # contradiction; so each known number is checked separately.
        if self.multiset_match and (
                matched != expected
                or (actual is not None and actual != expected)
                or (extra is not None and extra != 0)):
            raise ContractSchemaError(
                "idempotence: multiset_match=true противоречит собственным "
                f"счётчикам (ожидалось {expected}, получено {actual}, "
                f"совпало {matched}, лишних {extra}). Совпадение множеств — "
                "арифметика, а не мнение")
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "comparison_volume_zero": self.comparison_volume_zero,
        }
    ...
    def from_dict(cls, value: Any) -> "IdempotenceMetrics":
        ...
        return cls(
            ...
            # An old record does not carry the field: `False` there means
            # "the volume was not stated", not "the volume was non-empty"
            # (E-43).
            comparison_volume_zero=_strict_bool(
                row.get("comparison_volume_zero", False),
                "metrics.comparison_volume_zero"),
        )
