...
that yields 0 BLOCKING. Each bad_* deep-copies make_good() and breaks ONE THING IN THE
BUILDING — one authored fact, not one rule. The rule set that fires is a CONSEQUENCE and
is often larger than one: isolating a floor vertically necessarily also isolates it for
connectivity (HAB001) and for its apartment rules (HAB002/003/004), and under v2 the
overlapping-rooms fixture fires nine. The expected set per fixture, per lever, is pinned
in `checker/tests/test_a_fixture_breaks_what_it_says.py` — THAT FILE, not this docstring,
is the contract; it also names the two fixtures whose own target rule stays silent (a debt,
pinned so that closing it cannot happen quietly).

🔴 «EXACTLY one thing» stood here until 2026-08-29 and was false by measurement: half the
fixtures fire several rules. It was not the fixtures that were wrong. Fitting a fixture to
the promise would be fitting the subject to the instrument — the promise was fixed instead.

The fifteen bad_* mutators are authored in the dedicated builders task (after graph, before
rules) — signatures fixed in the plan foundation; this file is their single home. The
sixteenth, bad_floors_hidden_by_balcony_doors, is a named reproduction of the 2026-08-03
egress vacuity and has no single target rule.
...
def bad_room_no_door(d: dict | None = None) -> dict:
    ...
    Isolate the bathroom 'wc': remove EVERY door whose from_room_id or to_room_id is 'wc'
    (in GOOD that is exactly d_kit_wc), so the bathroom is unreachable from the building
...
def bad_apartment_into_apartment(d: dict | None = None) -> dict:
    ...
    Add a SECOND apartment A2 (its own entryway hall2 + habitable room bed2) whose ONLY entrance door
    connects into apartment A1's entryway instead of into public circulation — you must pass
    ...
    # A2's ONLY entrance is into A1's entryway (apartment-into-apartment), not public circ.
    d["doors"].append(
...
def bad_tiny_bedroom(d: dict | None = None) -> dict:
    ...
    Shrink the bedroom (habitable room) to 4 m² (< 8 m² minimum). Boundary is shrunk consistently so
...
def bad_narrow_corridor(d: dict | None = None) -> dict:
    ...
    Collapse the corridor to ~700 mm wide (< 900 mm minimum) by moving its WEST edge to
...
def bad_low_ceiling(d: dict | None = None) -> dict:
    ...
    Lower the bedroom's height to 2200 mm (< 2500 mm minimum). Area/width/window/connectivity
...
def bad_bedroom_no_window(d: dict | None = None) -> dict:
    ...
    The bedroom (habitable room) loses its exterior window: clear has_window/window_area_m2 on the
...
def bad_low_daylight(d: dict | None = None) -> dict:
    ...
    The bedroom KEEPS a window (so HAB030 'has a window' stays silent), but its glazing area is
...
def bad_overlapping_rooms(d: dict | None = None) -> dict:
    ...
    Move the bedroom's footprint so it overlaps the kitchen-living-room by well over 0.05 m².
    ...
    kitchen-living-room boundary: x[0,4000] y[6500,11000]. We shove the bedroom to
...
def stacked_floors(n_levels: int = 3, *, stairs: bool = False,
                   exterior_door_on_every_level: bool = True,
                   d: dict | None = None) -> dict:
    """The GOOD plan repeated on `n_levels`, with the two knobs the egress defect turns.

    NOT a one-thing `bad_*` mutator — it is the DISARMING stand (2026-08-03). The defect
    it was built for is not "one broken thing" but a COMBINATION: a building with no
    vertical connection at all, carrying the one extra input (an exterior door per floor)
    that used to make the egress rules unable to notice.

      stairs=False, exterior_door_on_every_level=True
          -> the vacuity case: 3 occupied levels, ZERO stair runs, a street door on each.
             Before the grade-anchored OUTSIDE node this read PASS.
      stairs=False, exterior_door_on_every_level=False
          -> the same broken building without the disarming doors. The two inputs MUST
             now produce the same verdict; that equality is the anti-vacuity property.
      stairs=True
          -> the legitimate control: real runs bridge every consecutive pair of levels,
             so the building must PASS with or without the upper doors.

    Every level is a verbatim copy of the GOOD level's geometry (ids suffixed `_L<i>`), so
    the plan stays v2-real: door locations still lie on the boundaries they claim, windows
    still sit on envelope-exterior walls, declared areas still equal their polygons.
    """
    base = make_good() if d is None else copy.deepcopy(d)
    out = copy.deepcopy(base)
    out["building_id"] = f"stacked_{n_levels}"
    out["stairs"] = []
    for i in range(1, n_levels):
        lid = f"L{i}"
        out["levels"].append({"id": lid, "name": f"Этаж {i + 1}",
                              "elevation_mm": 3000.0 * i, "index": i})
        for room in base["rooms"]:
            r = copy.deepcopy(room)
            r["id"], r["level_id"] = f"{room['id']}_{lid}", lid
            if r["apartment_id"]:
                r["apartment_id"] = f"{r['apartment_id']}_{lid}"
            out["rooms"].append(r)
        for door in base["doors"]:
            if door["is_exterior"] and not exterior_door_on_every_level:
                continue
            nd = copy.deepcopy(door)
            nd["id"], nd["level_id"] = f"{door['id']}_{lid}", lid
            for side in ("from_room_id", "to_room_id"):
                if nd[side]:
                    nd[side] = f"{nd[side]}_{lid}"
            out["doors"].append(nd)
        for window in base["windows"]:
            nw = copy.deepcopy(window)
            nw["id"], nw["level_id"] = f"{window['id']}_{lid}", lid
            nw["room_id"] = f"{window['room_id']}_{lid}"
            nw["host_wall_id"] = f"{window['host_wall_id']}_{lid}"
            out["windows"].append(nw)
        for wall in base["walls"]:
            nwl = copy.deepcopy(wall)
            nwl["id"], nwl["level_id"] = f"{wall['id']}_{lid}", lid
            out["walls"].append(nwl)
    if stairs:
        # One real run per consecutive pair, all stacked on the GOOD stair room footprint
        # (so HAB011 geometry stays sane and HAB012 sees a continuous core).
        footprint = [[4500, 0], [7000, 0], [7000, 4000], [4500, 4000]]
        for i in range(n_levels - 1):
            out["stairs"].append({
                "id": f"s_L{i}_L{i + 1}",
                "base_level_id": f"L{i}", "top_level_id": f"L{i + 1}",
                "base_z": 3000.0 * i, "top_z": 3000.0 * (i + 1),
                "run_width_mm": 1200.0, "riser_count": 17, "tread_depth_mm": 280.0,
                "footprint": footprint})
    return out


def bad_floors_hidden_by_balcony_doors(d: dict | None = None) -> dict:
    """The reproduction of the 2026-08-03 egress vacuity, as a named fixture.

    Three occupied levels, ZERO stair runs — no vertical connection of any kind — and an
    exterior door on every level. Measured on the pristine code: verdict PASS, HAB010
    EVALUATED(n=3) with zero violations. Deleting just the two upper doors flipped the
    same building to FAIL. The rule's answer depended on the doors, not on the stairs it
    claimed to be about."""
    return stacked_floors(3, stairs=False, exterior_door_on_every_level=True, d=d)
