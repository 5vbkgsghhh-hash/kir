...
from kir.effects import (  # noqa: E402
    Dependency,
    EffectCycleError,
    EffectError,
    WriteWriteConflict,
...
def _grounded(program_name: str):
    # Version bounds (`__min_ver__`/`__max_ver__`) are corpus metadata, not
    # fields of the program envelope: the version-effects planner does not know about them at all.
...
class FailClosed(unittest.TestCase):
    ...
    def test_write_write_is_dominated_by_the_id_check(self) -> None:
        ...
        """
        from kir.effects import DuplicateOpId
        from kir import effects as _eff
