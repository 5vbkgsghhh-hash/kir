...
class TheExtractorReadsTheLocationLine(unittest.TestCase):
    ...
    def test_it_is_pulled_as_an_integer_not_a_length(self):
        """The parameter is the ORDINAL NUMBER of an enum. Reading it as a
        length means silently converting units and producing a plausible
        wrong number.

        🔴 REWRITTEN 2026-08-21 FROM A NAME TO A PROPERTY. This used to read
        `assertIn("__PutIntParam", line)`, while the helper has long been
        called `__PutSectionIntParam` (renamed by the wedge wave, commits
        3941f1e7 / fd23d537). `"__PutIntParam"` is not a substring of
        `"__PutSectionIntParam"` — the test had been red since that day and
        was speaking about the RENAME, not about the subject it guards.

        The check is now two-sided and survives the next rename: the helper
        must be an INTEGER one (the name ends in `IntParam`) and must NOT BE
        a length. The second half is not decoration: it is precisely the
        substitution for `__PutLengthParam` that is the defect this test was
        written for, and a check for "Int" alone would have missed it if the
        string called both.
        """
        line = next(l for l in extract._ELEMENT_HELPERS_CS.splitlines()
                    if "WALL_KEY_REF_PARAM" in l and "BuiltInParameter" in l)
        helpers = re.findall(r"__Put[A-Za-z]+", line)
        self.assertTrue(helpers, f"строка не зовёт ни одного помощника: {line}")
        self.assertTrue(
            all(h.endswith("IntParam") for h in helpers),
            f"порядковый номер читается НЕцелочисленным помощником: {helpers}")
        self.assertNotIn("__PutLengthParam", line)
...
class TheOrdinalTable(unittest.TestCase):
    def test_the_reverse_table_loses_no_row(self):
        """The same assertion, called by its own name (rewritten 2026-08-12).

        This used to be `test_the_two_directions_are_exact_inverses`:
        checking ORDINALS against the inversion of NAMES, where NAMES IS
        BUILT by inverting ORDINALS (`ops_authoring.py:22`). Its strength is
        EXACTLY THE SAME as the line below's, and this is measured, not
        derived:

            a clean table            old green      new green
            SWAPPING a pair          old green      new green
            a DUPLICATE ordinal      old RED        new RED

        So what changed was NOT the strength but the name's honesty. The old
        name promised "two tables — exact inverses of each other," that is,
        it sounded like a check of PAIRS; what it actually checked was the
        one thing inversion is capable of failing to preserve — that the
        ordinals are pairwise distinct. Two names sharing one ordinal
        collapse the inverse table, and the lift silently reads the wrong
        word; that is exactly what is stated here, plainly.

        A caveat, so as not to slide to the opposite extreme: the test is
        NOT vacuous — on a non-injective table it goes red. It is vacuous
        precisely on the class that matters to us today — on SWAPS.

        This file does NOT and CANNOT pin the name-ordinal PAIR — see the
        block above the table itself: both ends of the compiler take the
        number from the same place, so the witness has no standing to
        object to the table.
        """
        from kir.ops_authoring import (
...
class TheCertificateKnowsTheRule(unittest.TestCase):
    ...
    def test_it_is_semantic_not_geometry(self):
        from kir.translation_cert import KIND_SEMANTIC
