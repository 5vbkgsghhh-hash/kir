"""The compiler's eyes: REFUTING tests.

The order of sections mirrors the order of the laws in :mod:`kir.preview`.
The most important is §4: if an element is not drawn, it must be COUNTED
and NAMED. A preview that silently drops part of the building is worse
than no preview at all, so what is checked here is not "does the wall get
drawn" but "can it be lost silently".
"""
...
from kir.decompile.schema import (  # noqa: E402
...
def _render(plan):
    # INSTRUMENT SHEET: these gates pin the census, blind spots, signature,
    # and coverage — that is, exactly what moved on 08.09 from the HUMAN
    # sheet to the machine channel. The subject of the pins did not change,
    # only the sheet's addressee did.
    return P.render_svg(plan, audience="instrument")
...
# ---------------------------------------------------------------------------
# LAW #4 — the main one: nothing may be lost silently
# ---------------------------------------------------------------------------

class CensusLawTests(unittest.TestCase):
    def test_census_refuses_to_exist_when_an_element_vanishes(self):
        """Identity sits in the constructor, not in the test: a test can be
        forgotten for a new path, but the constructor cannot be bypassed."""
    ...
    def test_every_omitted_element_is_named_by_reason_model(self):
        """No discarded element has the right to disappear from the census."""
        elements = [
            _wall("1", (0, 0), (5000, 0)),
            # without geometry
            ...
            # the category is not in the rules table
            _bbox_only("4", "OST_Zzz_Unknown", (0, 0), (100, 100)),
            # floor slab without a side sketch
        ...
        # the sixth element was not lost — it is in the BUILDING's census
    ...
    def test_levels_not_rendered_are_counted_not_silent(self):
        """A run over one of two levels has NO right to look complete."""
    ...
    def test_program_census_accounts_for_every_op(self):
        ...
        # 🔴 THE CAUSE WAS FIXED ON 21.08, AND THE TEST WAS PINNING A LIE.
        #
        # `set_param` and `delete` address an ELEMENT; they have NO level
        # field AT ALL. The census explained their omission as «селектор
        # уровня не сведён к плану» — an assertion about a field the
        # operation does not have. Exactly the case for which
        # `NOT_AN_OP` was split off from `SELECTOR_UNRESOLVED` on 04.08;
        # here it came back from the other side.
        self.assertEqual(
            {group.reason for group in building.census.omitted},
            {P.OmitReason.NOT_VISIBLE_IN_PLAN, P.OmitReason.NO_LEVEL_SLOT})

    def test_an_op_that_HAS_a_level_slot_still_says_selector_unresolved(self):
        """THE SECOND HALF OF THE FIX — ITS OWN CONTROL.

        Canon: a patch made of two halves needs two halves and two
        controls, otherwise one colors the tests while the other is
        checked by nothing. The first half is "no level field" (test
        above). The second is "the field exists but is not resolved to a
        plan": it must still say `selector_unresolved`, otherwise the fix
        did not split the cases but substituted one for the other.
        """
        program = _program(
            {"op": "create_wall", "id": "W9", "p0_mm": [0, 0],
             "p1_mm": [5000, 0],
             # the level IS NAMED, but not in a way that resolves to a plan
             "level": {"by": "default"}},
        )
        building = P.build_program_preview(program)
        reasons = {group.reason for group in building.census.omitted}
        self.assertIn(
            P.OmitReason.SELECTOR_UNRESOLVED, reasons,
            "у `create_wall` поле `level` ЕСТЬ — значит это недосмотр "
            "автора, а не свойство языка, и причина обязана остаться прежней")
        self.assertNotIn(P.OmitReason.NO_LEVEL_SLOT, reasons)

    def test_drawn_element_cannot_be_empty(self):
        """"Drawn" with not a single shape is a loss, not a rendering."""
        with self.assertRaises(P.PreviewError):
            P.DrawnElement("1", "OST_Walls", P.Layer.WALL, ())


# ---------------------------------------------------------------------------
# Refuting cases named in the task
# ---------------------------------------------------------------------------
...
class RefutingCaseTests(unittest.TestCase):
    ...
    def test_door_past_the_end_of_its_wall_is_drawn_and_flagged(self):
        """A door past the wall's edge is a FINDING, not a reason to hide it.

        It is drawn where it was computed (garbage must be visible), and at
        the same time lands among the anomalies.  Hiding it would mean
        bringing back exactly the blindness this screen was written to
        cure.
        """
    ...
    def test_rotated_grid_building_keeps_its_geometry(self):
        """A building with rotated axes: nothing is straightened out and nothing is lost."""
        ...
        # a rotated rectangle is WIDER and TALLER than its own sides
        self.assertGreater(max_x - min_x, 8000.0)
        self.assertGreater(max_y - min_y, 6000.0)
        # not a single vertex is "straightened" onto an axis
    ...
    def test_spline_wall_is_refused_not_straightened_into_a_chord(self):
        """A chord in place of a spline used to pass VERIFY as exact — here it is refused."""
        wall = _wall("1", (0, 0), (5000, 0),
...
class DeterminismTests(unittest.TestCase):
    ...
    def test_no_wall_clock_or_random_identity_in_the_artifact(self):
        ...
        # rebuilding THROUGH NEW objects must produce the same bytes
...
# ---------------------------------------------------------------------------
# LAW #6 — the strength of an assertion must be VISIBLE
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# "Drawn" ≠ "drawn precisely", and the suspect is named
# ---------------------------------------------------------------------------
...
class ApproximationAndAnomalyTests(unittest.TestCase):
    ...
    def test_coincident_walls_are_flagged_but_an_arc_is_not_its_chord(self):
        ...
        # the same chord, but DIFFERENT arcs — these are two different walls, not duplicates
        arc_index = {
            "1": {"curve_kind": "arc",
    ...
    def test_far_outliers_are_named_and_do_not_hide_the_building(self):
        """Measurement 03.08 (k2/L16): 44 stray telephone sets dragged the
        scale from 32 to 146 mm/px and hid 1610 legitimate elements."""
        ...
        # the outlier is STILL drawn — not discarded
        self.assertEqual(plan.census.drawn, len(walls) + len(strays))
        # but the frame is set by the core
    ...
    def test_outlier_rule_stays_silent_on_two_legitimate_clouds(self):
        """More than a quarter "outliers" = two clouds, not garbage.  Better
        to say nothing than to declare half the building a stray."""
    ...
    def test_level_recovered_from_a_parameter_is_marked_not_asserted(self):
        """Stairs hold STAIRS_BASE_LEVEL_PARAM, and their level_id is empty —
        falling back to the parameter is a SECOND READ, not direct
        knowledge."""
        stair = L0Element(
...
# ---------------------------------------------------------------------------
# One renderer: both sides speak the same language of shapes
# ---------------------------------------------------------------------------
...
class SingleDrawerTests(unittest.TestCase):
    ...
    def test_blind_spots_are_printed_on_the_sheet(self):
        """A blindness list is more useful than a capabilities list: a
...
# ---------------------------------------------------------------------------
# Live corpus: the preview must withstand a REAL parse
# ---------------------------------------------------------------------------
...
class CoincidentWallsAreFoundOnTheProgramPathToo(unittest.TestCase):
    """`COINCIDENT_WALLS` was set EXACTLY in one place — for the parsed
    building. A program that declared two walls with the same endpoints was
    marked with nothing.

    The same input skew as with closedness: the capability exists on one
    source and is absent on the other. And a duplicate wall is an AUTHOR'S
    defect: it is fixed by whoever writes the program, and they must learn
    about it BEFORE it becomes two walls in the model.
    """
    ...
    def test_the_same_ends_reversed_are_still_the_same_wall(self):
        """A wall's identity does not depend on the order of its endpoints —
        otherwise an author could dodge the finding by swapping two points."""
    ...
    def test_two_genuinely_different_walls_are_left_alone(self):
        """CONTROL. Without it, an instrument that marks EVERYTHING would be
        green on the first two checks and would not be measuring anything."""
...
class TheLevelIsPartOfAWallIdentity(unittest.TestCase):
    """🔴 A TYPICAL LEVEL IS NOT A DUPLICATE OF ITSELF (measurement
    15.08.2026).

    On the PROGRAM path a point is two-dimensional: `p0_mm`/`p1_mm` carry
    the plan, a separate `level` field carries the elevation. As long as
    the identity key did not include the level, an ordinary tower was
    marked as duplicates ENTIRELY — a measurement on 5 levels of 8 walls
    each gave **40 out of 40**. This is the one observation about assembly
    that reaches the model, and it was 100 % false on any multi-story
    building.

    The MODEL path could not suffer this way: there, endpoints are
    three-dimensional. One predicate was answering two different
    questions, because its inputs have different dimensionality.
    """
    ...
    def test_a_repeated_storey_is_not_a_pile_of_duplicates(self):
        """THE MAIN NUMBER. Five identical levels of eight walls each — zero findings."""
    ...
    def test_a_real_duplicate_on_one_level_is_still_caught(self):
        """FAIL CONTROL. The level in the key has no right to WEAKEN a
        finding: a segmentation that cancels the predicate itself is green
        by construction."""
    ...
    def test_a_wall_without_a_level_is_not_drawn_at_all(self):
        """A BOUNDARY THAT WAS MEASURED, NOT ASSUMED (15.08.2026).

        The first version of this test required two levelless walls to be
        marked as each other's duplicates. The measurement refuted that: a
        wall with no level DOES NOT LAND ON ANY PLAN — the preview is built
        by level, and it does not draw such walls at all (0 plans, 0
        elements). So it cannot carry an anomaly under any key.

        The `None` segment in the identity key nonetheless stays, and
        stays correct: it keeps a levelless wall from merging with "Level
        1" if it ever does reach the predicate. But TODAY this property is
        not observable through the public entry point, and the test states
        exactly what can be checked — otherwise it would be guarding a
        fiction.
        """
