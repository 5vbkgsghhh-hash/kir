...
class NegativeCorpus(unittest.TestCase):
    CASES = [
        ...
        # THE BOUND IS TAKEN FROM THE AUTHORITY. A literal 21 used to stand
        # here — "one more than the budget," correct while the budget was
        # 20. The owner raised it to 100 (15.08), and the negative case
        # stopped being negative: the program compiles, while the corpus
        # that is supposed to catch the refusal was catching silence.
    ...
    def test_catalog_selector_ref_is_typed_refusal_not_emitter_panic(self):
        """THE LAW IS THE SAME, THE CODE IS DIFFERENT — AND THIS IS A GAIN IN
        PRECISION, NOT A REGRESSION.

        The check's meaning is unchanged: a reference of the wrong kind in a
        catalog slot must become a TYPED REFUSAL, not an emitter panic
        (`KIR-P000`).

        The code changed from `KIR-T001` to `KIR-L004` on 23.08.2026, when
        `create_wall.type` got `ref_kinds=(WALL_TYPE,)`: the check on the
        reference's KIND runs BEFORE the check for "the catalog slot does
        not accept a ref at all," and it names the cause more precisely —
        "a level in a wall-type slot" instead of "the slot does not accept a
        reference." The expectation here stood firm for a day and stayed red
        (verified on a clean HEAD on 24.08: the same `KIR-L004`), meaning
        the test stopped holding its own law on the exact day the language
        became stricter.

        BOTH codes are checked: the new one must exist, the old one must not
        get lost silently — if `KIR-L004` ever stops firing, the refusal
        must remain typed, not become a panic.
        """
        out = compile_program(_p([
        ...
        codes = [d.code for d in out.diagnostics]
        self.assertFalse(out.ok)
        self.assertTrue({"KIR-L004", "KIR-T001"} & set(codes), codes)
