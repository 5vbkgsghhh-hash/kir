...
from kir.acceptance_probe import (
...
_REFERENCE_VALUES = [
    ("Материал несущих конструкций", {"material": "Бетон М300"},
     "ref", "materials", "Бетон М300"),
    ("Comments", {"phase": "Новая конструкция"},
     "ref", "phases", "Новая конструкция"),
    ("Comments", {"workset": "Общие уровни и сетки"},
     "int_ref", "worksets", "Общие уровни и сетки"),
]


@pytest.mark.parametrize("param,value,kind,pool,name", _REFERENCE_VALUES)
def test_a_reference_value_is_named_blind_instead_of_crashing(
        param, value, kind, pool, name):
    """F-096: A LEGAL LANGUAGE INPUT WAS CRASHING ACCEPTANCE PREPARATION.

    `set_param` with a reference (material, phase, workset) compiles, the
    emission carries `Set(__rf_*.Id)`, grounding checks the name against
    the pool. But `derive_mutation_expectation` normalized such a value to
    `value_kind='ref'` with a STRING in `expected_number`, and
    `MutationClaim` required a finite NUMBER: `expected_number must be
    finite`. This is a `ValueError`, not an `AcceptanceRuntimeError`,
    meaning the refusal escaped ABOVE `prepare_acceptance` — before the
    single independent re-read.

    THE INPUT IS BUILT BY PROD CODE (`plan_program`), and the value's kind
    is taken from the PLAN, not declared by the test: a hand-written
    `{'type': 'ref'}` would be guarding a fixture, while the normalization
    of kinds lives in `authoring_validation`.
    """
    plan = plan_program({
        "ir_version": "1.0",
        "ops": [{"op": "set_param", "id": "S1",
                 "target": {"by": "element_id", "value": 9001},
                 "param": param, "value": value}],
    })
    planned_value = plan.to_ops()[0]["value"]
    assert planned_value["type"] == kind, planned_value
    assert planned_value["pool"] == pool, planned_value

    expectation = derive_mutation_expectation(plan)

    assert expectation.claims == ()
    (blind,) = expectation.blind_ops
    assert blind.op_name == "set_param"
    assert blind.op_id == "S1"
    assert pool in blind.reason and name in blind.reason, blind.reason


def test_a_measurable_value_still_becomes_a_claim_beside_a_blind_reference():
    """🔴 THE COUNTERPART TO THE PREVIOUS ONE, AND WITHOUT IT THAT CONTROL GUARDS NOTHING.

    The test "no longer fails on material" turns GREEN from the fix "let
    ANY `set_param` slip past acceptance" as well — that is, from removing
    the check for all four remaining value kinds. So the cardinality here
    MUST be ≥ 2 kinds WITHIN ONE program: a reference MUST become
    BLINDNESS, a string and a number MUST become CLAIMS, and both
    assertions stand side by side.
    """
    plan = plan_program({
        "ir_version": "1.0",
        "ops": [
            ...
            {"op": "set_param", "id": "STR",
             "target": {"by": "element_id", "value": 9002},
             "param": "Comments", "value": "видно"},
            {"op": "set_param", "id": "NUM",
             "target": {"by": "element_id", "value": 9003},
             "param": "Comments", "value": {"value": 2500, "unit": "mm"}},
        ],
    })
    expectation = derive_mutation_expectation(plan)

    assert [item.op_id for item in expectation.blind_ops] == ["REF"]
    assert sorted(op_id
                  for claim in expectation.claims
                  for op_id in claim.op_ids) == ["NUM", "STR"]
    assert {claim.value_kind for claim in expectation.claims} == {"str", "mm"}


def test_a_reference_only_program_can_never_be_accepted():
    """THE PRICE PAID FOR BLINDNESS: the evidence MUST remain INCONCLUSIVE.

    Stage 1 removes the crash and does NOT provide a check in return. So it
    MUST leave the state `INCONCLUSIVE / partial_blind_scope`: "passed"
    here would be exactly the false green this module was written to forbid.
    """
    plan = plan_program({
        "ir_version": "1.0",
        "ops": [{"op": "set_param", "id": "REF",
                 "target": {"by": "element_id", "value": 9001},
                 "param": "Материал несущих конструкций",
                 "value": {"material": "Бетон М300"}}],
    })
    ...
    assert registration.blind
    evidence = assess_acceptance(registration, None, None)
    assert evidence.state is AcceptanceState.INCONCLUSIVE
    assert evidence.reason is AcceptanceReason.PARTIAL_BLIND_SCOPE
    assert [item.op_id for item in registration.blind_ops] == ["REF"]
...
def test_generated_probe_is_document_phase_and_version_bound():
    ...
    from kir.gate_runner import mutation_acceptance_gate_body
...
def test_long_element_id_uses_only_the_supported_revit_dialect():
    ...
    plan = plan_program({
        ...
        "ops": [{
            ...
            # 🔴 WAS [1, 0, 0] AND BECAME A REFUSAL ON 22.08.2026. This
            # test's subject is the LONG-id DIALECT, the delta here is
            # incidental; but a shift landing exactly on the witness's
            # tolerance (`location_mm` = 1.0 mm) is henceforth rejected as
            # UNPROVABLE: if Revit shifts nothing at all, the residual is
            # exactly 1.0 and the obligation is still satisfied.
            "delta_mm": [1000, 0, 0],
        }],
    })
    expectation = derive_mutation_expectation(plan)
