...
class HostedTopology(unittest.TestCase):
    ...
    def test_host_by_unsupported_selector_still_refused(self):
        """28.07: host in v1 narrowed to ref|element_id (it used to be —
        only ref). Any THIRD kind of selector remains a refusal — the rule
        was narrowed, not lifted."""
        out = compile_program(_prog([
            {"op": "create_door", "id": "D1",
...
class RoomOrderingRule(unittest.TestCase):
    def test_regenerate_before_room_after_walls(self):
        ...
        # THE FOURTH COPY OF ONE LINE IN THE SUITE, AND THE FOURTH ONE
        # LAGGING BEHIND. `test_regen_before_spatial` held
        # `"doc.Regenerate();  // finalize"`, `test_space` — `"finalize wall
        # enclosures"`, here the same thing, while the emitter writes a
        # fifth. `index` on top of that throws `ValueError: substring not
        # found` — a message naming neither the line nor the reason.
        # We ask the emitter (`authoring.SPATIAL_REGEN_CS`).
    ...
    def test_no_spurious_regen_without_walls(self):
        ...
        # THE FIFTH COPY, AND THE ONLY DANGEROUS ONE: it is in a NEGATIVE
        # assertion. The emitter has not written this line since the text
        # changed, so `assertNotIn` was true BY CONSTRUCTION and could never
        # fail — a check that can never trigger is worse than a missing
        # one: it occupies the place of a real one and is still counted in
        # the suite. The neighboring positive test (`i_regen =
        # cs.index(...)`) was failing LOUDLY, while this one stayed green
        # and silent on the very same stale line.
    ...
    def test_room_witness_rejects_unplaced_or_unenclosed_room(self):
        ...
        # THE NAME IS CHECKED AGAINST THE PARAMETER, NOT `Room.Name` — and
        # this assertion is two-sided on purpose. Measurement of 04.08
        # («Проект1», Revit 2026): the `Room.Name` setter stores only the
        # name, while the getter concatenates it with the room's NUMBER, so
        # the postcondition on the getter NEVER held, and the live matrix
        # was rolling back GOOD rooms (`f570aee2`). The emitter was fixed
        # the same day, but this assertion stayed in its old form and kept
        # the suite red for five days. The second half is a ratchet: if
        # `Room.Name` ever returns to the witness, the test must fail here,
        # not on the user's live model.
