...
class ViewSpaceTypeLaw(unittest.TestCase):
    ...
    def test_annotation_far_from_the_view_origin_is_accepted(self):
        """An annotation 50 m from the view's origin is legitimate. Measured
        27.07: for the plan, `View.Origin` = (0,0,0) with world axes, while
        the real building's walls lie in Y between 82,693 and 110,160 mm —
        the previous ±10 m boundary made it impossible to annotate anything
        in it at all. A leaked model coordinate cannot be told apart from a
        distant legitimate annotation by magnitude; what discriminates is
        arity, and it is checked by the test above (KIR-T001 on a
        three-component point)."""
    ...
    def test_coordinate_beyond_the_workable_extent_still_refused(self):
        """The sanity boundary remains — the same one as for model points."""
...
class NegativeAnnotation(unittest.TestCase):
    ...
    CASES_NO_WALL = [
        ...
        # The length is taken FROM A CONSTANT, not rewritten as a number: a
        # previous case hard-coded 2001, the ceiling had been raised from
        # a measured building (maximum 4763 characters against a limit of
        # 2000), and the test silently stopped checking the boundary. A
        # refusal on length is KIR-T002 (boundaries), not KIR-T001 (type):
        # previously the two refusals shared one message, and the same
        # "non-empty string" was arriving for text that was too LONG.
        (lambda: [_text(content="x" * (_TEXT_CONTENT_MAX_CHARS + 1))],
         "KIR-T002"),
        # 50 m from the view's origin is LEGITIMATE (see
        # test_annotation_far_from_the_view_origin_is_accepted); the
        # negative corpus keeps only what lies outside the model's working
        # limit.
        (lambda: [_text(at=[99_000_000, 0])], "KIR-T002"),  # outside ~16 km
    ]
...
#: The Revit version on which EVERY golden is frozen. Pulled out of the
#: body of `test_golden` NOT for beauty's sake: the determinism subprocess
#: MUST rebuild the very same program on THE SAME version, and a second
#: copy of this table would be exactly the defect ("two carriers of one
#: fact") that this very class is fixing today in its own docstring.
GOLDEN_VERSIONS = {"annotation_full_set": "2026",
...
class Golden(unittest.TestCase):
    ...
    FROZEN ON 13.08.2026, AND BEFORE THAT DAY THE FILES DID NOT EXIST AT
    ALL. The test stood red, and its own message stated the cause verbatim
    — "missing — run once with KIR_UPDATE_GOLDEN=1 and review". This was
    NOT emission drift and NOT an owner collision (two other red goldens
    that same night were exactly that): there was nothing to reconcile
    against.

    DETERMINISM BEFORE THE FREEZE, not after: freezing by several
    processes with DIFFERENT `PYTHONHASHSEED` values MUST give
    byte-for-byte the same result.

    🔴 AND THIS IS NO LONGER A RECORD, BUT A RUN — BECAUSE THE RECORD WENT
    STALE, AND THIS IS A MEASUREMENT, NOT A WORRY. Four literals used to
    stand here: «`annotation_full_set` 33 425 bytes `sha a7fa70aafe1d`,
    `annotation_explicit_types` 35 697 bytes `sha 1d55a658630d`». They
    described files frozen by commit `00def477` on 13.08 — and only forty
    minutes into that same day, `61895cc1` ("mark: tag leader guard with no
    dead branch + three goldens re-frozen AFTER the read") re-froze both
    under spatial marks: **36 163 bytes `sha 3a467645d606`** and **39 120
    bytes `sha 90abfa4fb267`**. Nobody corrected the attestation, and it
    lay there as a lie for nine days — exactly the same defect this file
    exists to hold goldens against in the first place: one fact grew TWO
    CARRIERS, and only one of them mutates. The emitter mutates the
    goldens; nothing mutates the docstring.

    That is why the number here is not rewritten to a new one (it would go
    stale a third time), but REPLACED BY A CALL:
    `test_the_freeze_is_deterministic_under_another_hash_seed` below
    rebuilds both goldens in a SEPARATE process with a different
    `PYTHONHASHSEED` and demands a byte-for-byte match with the file. The
    evidence is captured fresh on every run and cannot go stale by
    construction.

    WHAT THESE TWO GOLDENS PIN DOWN (read from the emitted C#, not
    assumed):

    * `AngularDimension.Create(doc, view, arc, refs, dimType)` — the
      FIVE-argument overload, the only one in the whole dimension family
      that compiles on all six (`LinearDimension.Create`/`RadialDimension`/
      `ArcLengthDimension` — 2025-2026 only; `NewAngularDimension`/
      `NewDiameterDimension` live on `FamilyItemFactory`, that is, only in
      the family editor). Swapping in any of them will turn this red;
    * a linear dimension through `NewDimension` + **`ReferenceArray`** —
      that is, GEOMETRIC references, not `Reference(element)`, which
      `NewDimension` does not accept (measured, canon);
    * a view point through the VIEW'S BASIS:
      `__vw.Origin + RightDirection.Multiply(U(x)) + UpDirection.Multiply(U(y))`
      for `TextNote.Create` — that is, `[u,v]` as a model-space offset from
      `View.Origin`, not as sheet coordinates. A raw XY will turn this red;
    * TWO DIFFERENT forms of `IndependentTag.Create`: without a type
      (`false`, `TM_ADDBY_CATEGORY`) in `full_set`, and with a type plus a
      leader (`__ttel_T1.Id`, `true`, `TagOrientation`) in
      `explicit_types`. A cardinality of 2 here is not decoration: with
      only one form, a substitution of the other would be invisible.

    WHAT THEY DO NOT PIN DOWN, and this matters more than the list above:

    * THE VERSION AXIS. Every golden is captured on ONE version (2026 and
      2022), so they reconcile nothing at all about emission differences
      across the six versions — that is the gate's job, and there
      annotations run as separate programs;
    * that Revit WILL BUILD what is described. A golden reconciles C#
      bytes, not behavior; there is no live evidence for
      `create_angular_dimension` (see `UNPROVEN`);
    * the angular dimension's ARC as such: it is DERIVED by the emitter
      from two references, and the golden pins the text that builds it,
      but does not check that the center landed at the vertex where the
      planes intersect — offline this is indistinguishable;
    * `leader_to` on the text and `leader` on the tag, each only in ONE
      configuration: the reverse (text with no leader under an explicit
      type, a tag with a leader and no type) is covered by neither.
    """
    ...
    def test_the_freeze_is_deterministic_under_another_hash_seed(self):
        """ATTESTATION OF THE FREEZE — BY A RUN, NOT A RECORD (see the class's docstring).

        A golden makes sense exactly to the extent the emission is
        REPRODUCIBLE: for a non-deterministic emitter, a byte-for-byte
        comparison turns red at random and teaches people to regenerate
        without reading. This used to be proved ONCE, and then lived on as
        four literals in a docstring — and the literals went stale on the
        very next commit of that same day, unnoticed by anyone.

        A separate process is needed as a matter of substance:
        `PYTHONHASHSEED` is read by the interpreter at startup, and cannot
        be changed within a single process. What is compared are HASHES,
        not texts: a discrepancy MUST print as three lines, not thirty
        thousand characters — a diff-length message gets read as "a drift
        of 40,000 characters", when the actual drift here is 917 bytes
        (paid for on 22.08).
        """
        import hashlib
        import subprocess
        ...
        script = (
            "import json, hashlib\n"
            "from kir.tests.test_annotation import "
            "ANNOTATION_PROGRAMS, GOLDEN_VERSIONS\n"
            "from kir.compiler import compile_program\n"
            "from kir.tests.fixtures import GROUND_SNAPSHOT as S\n"
            "out = {}\n"
            "for n, p in ANNOTATION_PROGRAMS.items():\n"
            "    r = compile_program(p, revit_version=GOLDEN_VERSIONS[n], snapshot=S)\n"
            "    assert r.ok, n\n"
            # BYTES, not characters: the file is read 'rb', while
            # `len(str)` counts characters — with Cyrillic in the C#
            # comments these are different numbers (35,349 characters
            # against 37,156 bytes), and the control would turn red over
            # the unit of measurement instead of the subject. Paid for on
            # 22.08 by its own fix.
            "    b = r.csharp.encode('utf-8')\n"
            "    out[n] = [len(b), hashlib.sha256(b).hexdigest()]\n"
            "print(json.dumps(out))\n")
        env = dict(os.environ)
        # A different seed than THIS process's, whatever it may be: a
        # hard-coded constant would coincide with the run's environment
        # and render the control meaningless.
        env["PYTHONHASHSEED"] = "0" if os.environ.get("PYTHONHASHSEED") == "12345" else "12345"
        env["PYTHONPATH"] = backend + os.pathsep + env.get("PYTHONPATH", "")
        env.setdefault("KIR_REJECTIONS_PATH",
                       os.path.join(tempfile.gettempdir(), "kir_test_queue.jsonl"))
        proc = subprocess.run([sys.executable, "-c", script], cwd=backend,
                              env=env, capture_output=True, text=True, timeout=300)
        self.assertEqual(proc.returncode, 0,
                         f"подпроцесс замера не отработал:\n{proc.stderr[-2000:]}")
        import json as _json
        got = _json.loads(proc.stdout.strip().splitlines()[-1])

        self.assertEqual(sorted(got), sorted(ANNOTATION_PROGRAMS),
                         "подпроцесс собрал не тот набор программ")
        for name in ANNOTATION_PROGRAMS:
            with self.subTest(name=name):
                path = os.path.join(GOLDEN_DIR, f"{name}.golden.cs")
                # 🔴 READ IT THE SAME WAY THE RATCHET ITSELF READS IT, AND
                # THIS IS NOT PEDANTRY. `.gitattributes` declares `*.cs
                # text eol=crlf`, while the emitter prints LF. Measured on
                # 22.08: `git cat-file --filters` gives, on a fresh
                # checkout, **36,799 bytes and 636 CRs** where the index
                # has 36,163 bytes and zero CRs. `open(...)` in text mode
                # translates `\r\n` → `\n` (universal newlines), so
                # `test_golden` never notices; a 'rb' read would have
                # noticed — and would have turned red ON LINE ENDINGS for
                # anyone who cloned fresh, with the message "emit drifted".
                # The first edition of this control read 'rb' and was
                # carrying exactly this landmine.
                with open(path, encoding="utf-8") as fh:
                    frozen = fh.read().encode("utf-8")
                self.assertEqual(
                    [len(frozen), hashlib.sha256(frozen).hexdigest()], got[name],
                    f"{name}: пересборка с другим PYTHONHASHSEED разошлась с "
                    f"замороженным файлом. Замороженный: {len(frozen)} байт "
                    f"{hashlib.sha256(frozen).hexdigest()[:12]}; пересборка: "
                    f"{got[name][0]} байт {got[name][1][:12]}")

    def test_update_mode_cannot_pass_for_a_green_corpus(self):
        """🔴 A RATCHET THAT CAN BE SWITCHED OFF SILENTLY IS NOT A RATCHET.

        With `KIR_UPDATE_GOLDEN=1`, `test_golden` OVERWRITES the files and
        does a `continue` — that is, it compares nothing and prints `ok`.
        The flag lives in the environment: one forgotten variable in the
        shell, in `tox.ini`, or in CI — and the whole corpus is green by
        construction, saying nothing about it. This is the same defect
        form as "a green control on a degenerate input", only the lever
        sits outside the process.

        The cure is not to forbid the flag (it is needed), but to make the
        run, while in update mode, MUST turn red: the goldens have been
        rewritten, so this run proved nothing and must not be read as "the
        corpus matched".
        """
        self.assertNotEqual(
            os.environ.get("KIR_UPDATE_GOLDEN"), "1",
            "KIR_UPDATE_GOLDEN=1 — это ПЕРЕЗАПИСЬ эталонов, а не проверка. "
            "Голдены переписаны; прочитай диф глазами и прогони ещё раз БЕЗ "
            "этой переменной, иначе зелёный отчёт не означает ничего.")
