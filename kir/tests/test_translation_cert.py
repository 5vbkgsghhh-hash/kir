...
def _grounded_programs():
    ...
    for pname, prog in PROGRAMS.items():
        ...
        # The upper bound on the version is the same symmetry as in the corpus itself (see
        # its header): for the load wave, the version axis points the other way,
        # and on 2024-2026 the honest answer is a typed KIR-E003, not C#.
...
class MutationCatches(unittest.TestCase):
    ...
    def _mutate_and_certify(
        ...
        import kir.translation_cert as tc
...
class FailClosedAndFlag(unittest.TestCase):
    ...
    def test_unknown_op_fails_closed(self) -> None:
        from kir.translation_cert import CertificateSchemaError
