...
from kir import authoring_validation, contour, geom
from kir.decompile.l1_schema import AtomReason
from kir.decompile.tests.test_lift_floor_contour import _lift as _lift_floor
from kir.decompile.tests.test_lift_railing_path import (
    _document as _railing_document,
    _node as _lift_railing,
    _row as _railing_row,
    _sketch_index as _railing_sketch,
)
from kir.diag import TYPE_BAD_TYPE, TYPE_BOUNDS
from kir.instruments import bounds_audit
...
def test_profile_and_path_limits_have_public_independent_owners():
    ...
    # 🔴 64 -> 256 and 32 -> 128 via commit `37705f6` (02.09.2026), the
    # ratchet was updated ON THE MERITS. The limits were holding down not
    # the cost but the INVISIBILITY of the quadratic self-intersection
    # check; the all-pairs traversal was replaced with a sweep along x
    # (256 points: 44.5 ms -> 0.36 ms), and the harm was measured: outer
    # rings rejected 67 of 652, holes 82 of 183 — after the raise, 0 and 0.
    # The whole argument rests on the constants themselves.
    #
    # This line was NOT updated by that commit and therefore stayed RED
    # until the evening of 02.09: the limits were raised in one carrier,
    # while eight other places kept saying "64", including the model's
    # permanent doc.
    assert geom.MAX_RING_POINTS == 256
    assert geom.MAX_HOLES == 8
    assert geom.MAX_HOLE_RING_POINTS == 128
    # 🔴 10 000 -> 100 by measurement 21.08: the old threshold was derived
    # from BUILDING rings (the smallest of 709 — 62 500 mm²) and applied to
    # FAMILY profiles, which are two orders of magnitude smaller. It
    # rejected a 25×25 mm baluster (625 mm²), a GOST door leaf (759), a
    # pipe strip (200) — twenty shapes of the real building. The ratchet
    # was updated ON THE MERITS, not by fitting: the whole argument rests
    # on the constant itself.
    assert geom.MIN_RING_AREA_MM2 == 100.0
...
def _triangle_of_area(area_mm2: float) -> list:
    """A right triangle of exactly the specified area.

    🔴 DERIVED FROM THE THRESHOLD, NOT WRITTEN AS A LITERAL. The previous
    version used `[[0,0],[199.99,0],[0,100]]` — "just under 10 000 mm²". On
    21.08 the threshold was lowered to 100 mm² by measurement on family
    profiles, and both boundary tests turned red, guarding NOT their own
    assertion ("the boundary is inclusive") but a stale number. The height
    is held at 100 mm: it sets the scale, while the base is computed.
    """
    height = 100.0
    return [[0.0, 0.0], [2.0 * area_mm2 / height, 0.0], [0.0, height]]


def test_forward_ring_area_boundary_is_inclusive():
    at_limit = _triangle_of_area(geom.MIN_RING_AREA_MM2)
    below_limit = _triangle_of_area(geom.MIN_RING_AREA_MM2 - 0.5)
...
def test_contour_reads_the_same_inclusive_area_owner():
    at_limit = {"shape": "poly",
                "points_mm": _triangle_of_area(geom.MIN_RING_AREA_MM2)}
    below_limit = {"shape": "poly",
                   "points_mm": _triangle_of_area(geom.MIN_RING_AREA_MM2 - 0.5)}
