...
def test_a_missing_a5_runs_dir_names_its_two_meanings(tmp_path):
    """🔴 «There are no unfinished runs» and «the journal directory was wiped» — different.

    `find_resumable` returns `None` when the `a5_runs` directory is absent,
    and that answer must not be dropped: finding no journal, the turn must
    start a new run. But `None` has two meanings, and the second is more
    costly: A5 exists FOR THE SAKE OF EVIDENCE after a failure, and a silent
    zero here is worth more than usual.

    From inside the directory the two cannot be told apart; from outside it
    is visible: the parse itself is not empty, meaning work WAS HAPPENING
    here. This is not proof of loss — A5 is not enabled on every turn — and
    so the suspicion must be put IN WORDS, with the reader deciding.
    """
    empty = tmp_path / "пустой"
    empty.mkdir()
    assert A5Journal.resumable_absence_reason(empty) is None, (
        "в пустом разборе «прогонов нет» значит ровно «их не было»")

    worked = tmp_path / "работали"
    worked.mkdir()
    (worked / "L0.jsonl").write_text("{}", encoding="utf-8")
    why = A5Journal.resumable_absence_reason(worked)
    assert why is not None, "разбор не пуст — молчать об утрате улик нельзя"
    assert "a5_runs" in why and "МАШИНЕ" in why, (
        "причина обязана назвать и КАТАЛОГ, и то, что это факт о машине")

    (worked / "a5_runs").mkdir()
    assert A5Journal.resumable_absence_reason(worked) is None, (
        "каталог на месте — подозревать нечего; прибор, подозревающий "
        "всегда, не сторожит")


def test_the_a5_probe_does_not_depend_on_compression(tmp_path):
    """The evidence is found by the NAMES of directory entries, not by reading files.

    Lesson from this same shift: a bare `is_file("passport.json")` misses 82
    parses out of 93, because the janitor compresses side files in place.
    There is no reading here at all — but this must be checked EXPLICITLY, or
    tomorrow's patch will add reading and silently repeat the same defect.
    """
    import gzip

    run = tmp_path / "сжатый"
    run.mkdir()
    with gzip.open(str(run / "L0.jsonl.gz"), "wt", encoding="utf-8") as handle:
        handle.write("{}")
    why = A5Journal.resumable_absence_reason(run)
    assert why is not None, (
        "разбор со СЖАТЫМИ файлами так же не пуст: улика — имя записи, и "
        "сжатие её не прячет")
