...
Wall __el_WC = null;
ElementId __requestedType_WC = null;
ElementId __assignedType_WC = null;
...
using (Transaction __t = new Transaction(doc, "KIR: витраж: стеклопакет в единственную ячейку"))
{
    try
    {
        ...
        int __operationPostStart_WC = __post.Count;
        // operation WC
        {
            try { __requestedType_WC = __wt_WC.Id;
                __assignedType_WC = __el_WC.GetTypeId(); } catch { }
            if (__requestedType_WC == null || __requestedType_WC == ElementId.InvalidElementId
                || __assignedType_WC == null || __assignedType_WC == ElementId.InvalidElementId
                || !__assignedType_WC.Equals(__requestedType_WC))
                __post.Add("WC: type assignment mismatch or unavailable (operation)");
        }
        if (__post.Count > __operationPostStart_WC) { __t.RollBack(); return __Refuse("WC", String.Join(" ; ", __post.Skip(__operationPostStart_WC))); }
...
// witness WC
{
    ...
    __rb["type_assignment"] = new Dictionary<string, object> {
        ...
        {"requested_type_id", __requestedType_WC == null ? null : __requestedType_WC.ToString()},
        {"observed_type_id", __assignedType_WC == null ? null : __assignedType_WC.ToString()} };
    __rb["type_id"] = null; __rb["type_id_status"] = "unavailable";
    __rb["type_id_reason"] = "type_id_unreadable";
    try { var __finalType = __el_WC.GetTypeId();
