...
using (Transaction __t = new Transaction(doc, "KIR: двускатная кровля"))
{
    try
    {
        ...
        double[] __sl_R1 = new double[] { 0.577350269, -1.0, 0.577350269, -1.0 };
        int __sk_R1 = 0;
        foreach (ModelCurve __mc_R1 in __ma_R1)
        {
            if (__sk_R1 < __sl_R1.Length && __sl_R1[__sk_R1] >= 0.0)
            {
                __el_R1.set_DefinesSlope(__mc_R1, true);
                __el_R1.set_SlopeAngle(__mc_R1, __sl_R1[__sk_R1]);
            }
            __sk_R1++;
        }
        doc.Regenerate();
        try { Parameter __cm = __el_R1.get_Parameter(BuiltInParameter.ALL_MODEL_INSTANCE_COMMENTS); if (__cm != null && !__cm.IsReadOnly) __cm.Set("kir:1476ecf2:R1"); } catch { }
        ...
        // post R1
        {
            ...
            var __rb2 = __el_R1.get_BoundingBox(null);
            if (__rb2 == null)
                __post.Add("R1: нет bbox для проверки уклона (geometry)");
            else if (MM(__rb2.Max.Z - __rb2.Min.Z) < 1645.4)
                __post.Add("R1: уклон крыши не тот (geometry): подъём "
                    + MM(__rb2.Max.Z - __rb2.Min.Z).ToString("F1")
                    + " мм против ожидаемых не менее 1645.4 мм");
        }
        if (__post.Count > 0)
        {
