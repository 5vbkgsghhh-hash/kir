...
using (Transaction __t = new Transaction(doc, "KIR: подвесной потолок с закруглённым краем и проёмом под шахту"))
{
    try
    {
        ...
        __ol_C1.Append(Line.CreateBound(P(0.0, 0.0, 0), P(6000.0, 0.0, 0)));
        __ol_C1.Append(Arc.Create(P(6000.0, 0.0, 0), P(6000.0, 4000.0, 0), P(6800.0, 2000.0, 0)));
        __ol_C1.Append(Line.CreateBound(P(6000.0, 4000.0, 0), P(0.0, 4000.0, 0)));
        __ol_C1.Append(Line.CreateBound(P(0.0, 4000.0, 0), P(0.0, 0.0, 0)));
        __loops_C1.Add(__ol_C1);
        CurveLoop __hl_C1_0 = new CurveLoop();
        __hl_C1_0.Append(Line.CreateBound(P(2000.0, 1500.0, 0), P(3000.0, 1500.0, 0)));
        __hl_C1_0.Append(Line.CreateBound(P(3000.0, 1500.0, 0), P(3000.0, 2500.0, 0)));
        __hl_C1_0.Append(Line.CreateBound(P(3000.0, 2500.0, 0), P(2000.0, 2500.0, 0)));
        __hl_C1_0.Append(Line.CreateBound(P(2000.0, 2500.0, 0), P(2000.0, 1500.0, 0)));
        ...
        __cho_C1.Set(U(2700.0));try { Parameter __cm = __el_C1.get_Parameter(BuiltInParameter.ALL_MODEL_INSTANCE_COMMENTS); if (__cm != null && !__cm.IsReadOnly) __cm.Set("kir:abbbccd9:C1"); } catch { }
        ...
        // post C1
        {
            ...
            if (__bb == null) __post.Add("C1: нет BoundingBox");
            else if (Math.Abs(MM(__bb.Min.X) - 0.0) > 50.0 || Math.Abs(MM(__bb.Max.X) - 6800.0) > 50.0 ||
                     Math.Abs(MM(__bb.Min.Y) - 0.0) > 50.0 || Math.Abs(MM(__bb.Max.Y) - 4000.0) > 50.0)
                __post.Add("C1: bbox extents mismatch (geometry): "
                    ...
                    + " против авторских [0.0 6800.0 | 0.0 4000.0] при допуске 50.0 мм");
            string __slf_C1 = null;
            try
            {
                List<List<Curve>> __slL_C1 = null;
                var __slk_C1 = new List<Sketch>();
                foreach (ElementId __sld_C1 in __el_C1.GetDependentElements(null))
                {
                    var __sls_C1 = doc.GetElement(__sld_C1) as Sketch;
                    if (__sls_C1 != null) __slk_C1.Add(__sls_C1);
                }
                if (__slk_C1.Count == 1)
                {
                    __slL_C1 = new List<List<Curve>>();
                    foreach (CurveArray __slc0_C1 in __slk_C1[0].Profile)
                    {
                        var __slo_C1 = new List<Curve>();
                        foreach (Curve __slx_C1 in __slc0_C1) __slo_C1.Add(__slx_C1);
                        __slL_C1.Add(__slo_C1);
                    }
                }
                if (__slL_C1 != null)
                {
                    var __slr_C1 = new List<string>();
                    foreach (var __slc_C1 in __slL_C1)
                    {
                        var __slv_C1 = new List<long[]>();
                        foreach (Curve __slq_C1 in __slc_C1)
                        {
                            XYZ __slp_C1 = __slq_C1.GetEndPoint(0);
                            __slv_C1.Add(new long[] { __KirCanonUnit(MM(__slp_C1.X), 1.0), __KirCanonUnit(MM(__slp_C1.Y), 1.0) });
                        }
                        __slv_C1.Sort(__KirCanonCmp);
                        var __slb_C1 = new StringBuilder();
                        for (int __sli_C1 = 0; __sli_C1 < __slv_C1.Count; __sli_C1++)
                        {
                            if (__sli_C1 > 0) __slb_C1.Append('|');
                            __slb_C1.Append(__slv_C1[__sli_C1][0].ToString(System.Globalization.CultureInfo.InvariantCulture));
                            __slb_C1.Append(',');
                            __slb_C1.Append(__slv_C1[__sli_C1][1].ToString(System.Globalization.CultureInfo.InvariantCulture));
                        }
                        __slr_C1.Add(__slb_C1.ToString());
                    }
                    __slr_C1.Sort(StringComparer.Ordinal);
                    __slf_C1 = string.Join(";", __slr_C1);
                }
            }
            catch (Exception) { __slf_C1 = null; }
            if (__slf_C1 == null)
                __post.Add("C1: эскиз построенного элемента не прочитан(ы) — форма НЕ ПРОВЕРЕНА (geometry)");
            else if (__slf_C1 != "0,0|0,4000|6000,0|6000,4000;2000,1500|2000,2500|3000,1500|3000,2500")
                __post.Add("C1: sketch loops mismatch, expected 2 loop(s) (geometry)");
