    ...
    try
    {
        ...
        try {
            ...
            if (__se_T1 != null)
            {
                ...
                __t.RollBack(); return __Refuse("T1", "leader:true у пространственной марки в этой волне отказан: точка означала бы КОНЕЦ ВЫНОСКИ, а сверяется ГОЛОВА (дословно RevitAPI.xml про седьмой аргумент) — молчаливый сдвиг вместо марки");
                var __uv_T1 = new UV(U(3000.0), U(800.0));
                ...
                else if (__ar_T1 != null)
                {
                    ...
                    { __t.RollBack(); return __Refuse("T1", "цель — SpatialElement рода " + __ClassName(__se_T1) + ", а марки строятся только для помещения, площади и пространства"); }
                if (__el_T1 != null)
                    { try { __el_T1.ChangeTypeId(__ttel_T1.Id); }
                      catch (Exception __tex_T1) { __t.RollBack(); return __Refuse("T1", "тип марки не принят пространственной маркой: " + __tex_T1.Message); } }
            }
            else
            {
                __el_T1 = IndependentTag.Create(doc, __ttel_T1.Id, __vw_T1.Id, new Reference(__tg_T1), true, TagOrientation.Horizontal, (__vw_T1.Origin + __vw_T1.RightDirection.Multiply(U(3000.0)) + __vw_T1.UpDirection.Multiply(U(800.0))));
                if (__el_T1 != null) ((IndependentTag)__el_T1).TagHeadPosition = (__vw_T1.Origin + __vw_T1.RightDirection.Multiply(U(3000.0)) + __vw_T1.UpDirection.Multiply(U(800.0)));
            }
        }
...
// witness W1
{
    ...
    try
    {
        ...
        if (__kirIdentityEl != null)
        {
            ...
            {
                long __kirIdentityNumber = (long)__kirIdentityId.IntegerValue;
