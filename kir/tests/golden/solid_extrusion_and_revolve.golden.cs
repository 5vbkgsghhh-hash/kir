...
using (Transaction __t = new Transaction(doc, "KIR: параметрическое тело: выдавливание с проёмом и вращение"))
{
    try
    {
        ...
        // post SE1
        {
            ...
            else if (MM(__bb_SE1.Min.X) < 0.0 - __dt_SE1 || MM(__bb_SE1.Min.X) > 0.0 + 5.800000000000001 + __dt_SE1 ||
                     MM(__bb_SE1.Max.X) > 6800.0 + __dt_SE1 || MM(__bb_SE1.Max.X) < 6800.0 - 5.800000000000001 - __dt_SE1 ||
                     MM(__bb_SE1.Min.Y) < 0.0 - __dt_SE1 || MM(__bb_SE1.Min.Y) > 0.0 + 5.800000000000001 + __dt_SE1 ||
                     MM(__bb_SE1.Max.Y) > 4000.0 + __dt_SE1 || MM(__bb_SE1.Max.Y) < 4000.0 - 5.800000000000001 - __dt_SE1 ||
                     MM(__bb_SE1.Min.Z) < 3300.0 - __dt_SE1 || MM(__bb_SE1.Min.Z) > 3300.0 + 0.0 + __dt_SE1 ||
                     MM(__bb_SE1.Max.Z) > 5100.0 + __dt_SE1 || MM(__bb_SE1.Max.Z) < 5100.0 - 0.0 - __dt_SE1)
                __post.Add("SE1: bbox extents mismatch (geometry) — ожидалось X 0.0..6800.0 · Y 0.0..4000.0 · Z 3300.0..5100.0"
                  + "; получено X " + Math.Round(MM(__bb_SE1.Min.X), 1) + ".." + Math.Round(MM(__bb_SE1.Max.X), 1)
                  + " · Y " + Math.Round(MM(__bb_SE1.Min.Y), 1) + ".." + Math.Round(MM(__bb_SE1.Max.Y), 1)
                  + " · Z " + Math.Round(MM(__bb_SE1.Min.Z), 1) + ".." + Math.Round(MM(__bb_SE1.Max.Z), 1)
                  + " · допуск наружу " + Math.Round(__dt_SE1, 2) + " мм, внутрь " + Math.Round(5.800000000000001 + __dt_SE1, 2) + " мм");
        }
        // post SR1
        {
            ...
            else if (MM(__bb_SR1.Min.X) < 10200.0 - __dt_SR1 || MM(__bb_SR1.Min.X) > 10200.0 + 3.6 + __dt_SR1 ||
                     MM(__bb_SR1.Max.X) > 13800.0 + __dt_SR1 || MM(__bb_SR1.Max.X) < 13800.0 - 3.6 - __dt_SR1 ||
                     MM(__bb_SR1.Min.Y) < 2200.0 - __dt_SR1 || MM(__bb_SR1.Min.Y) > 2200.0 + 3.6 + __dt_SR1 ||
                     MM(__bb_SR1.Max.Y) > 5800.0 + __dt_SR1 || MM(__bb_SR1.Max.Y) < 5800.0 - 3.6 - __dt_SR1 ||
                     MM(__bb_SR1.Min.Z) < 0.0 - __dt_SR1 || MM(__bb_SR1.Min.Z) > 0.0 + 3.6 + __dt_SR1 ||
                     MM(__bb_SR1.Max.Z) > 2400.0 + __dt_SR1 || MM(__bb_SR1.Max.Z) < 2400.0 - 3.6 - __dt_SR1)
                __post.Add("SR1: bbox extents mismatch (geometry) — ожидалось X 10200.0..13800.0 · Y 2200.0..5800.0 · Z 0.0..2400.0"
                  + "; получено X " + Math.Round(MM(__bb_SR1.Min.X), 1) + ".." + Math.Round(MM(__bb_SR1.Max.X), 1)
                  + " · Y " + Math.Round(MM(__bb_SR1.Min.Y), 1) + ".." + Math.Round(MM(__bb_SR1.Max.Y), 1)
                  + " · Z " + Math.Round(MM(__bb_SR1.Min.Z), 1) + ".." + Math.Round(MM(__bb_SR1.Max.Z), 1)
                  + " · допуск наружу " + Math.Round(__dt_SR1, 2) + " мм, внутрь " + Math.Round(3.6 + __dt_SR1, 2) + " мм");
        }
        if (__post.Count > 0)
        {
