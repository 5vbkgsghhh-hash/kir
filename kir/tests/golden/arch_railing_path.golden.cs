    ...
    try
    {
        ...
        // post R1
        {
            ...
            try
            {
                ...
                var __slP_R1 = __el_R1.GetPath();
                if (__slP_R1 != null && __slP_R1.Count > 0)
                {
                    __slL_R1 = new List<List<Curve>>();
                    foreach (Curve __slx_R1 in __slP_R1)
                    {
                        var __slo_R1 = new List<Curve>();
                        __slo_R1.Add(__slx_R1);
                        __slo_R1.Add(__slx_R1.CreateReversed());
            ...
            if (__slf_R1 == null)
                __post.Add("R1: путь построенного элемента не прочитан(ы) — форма НЕ ПРОВЕРЕНА (geometry)");
            else if (__slf_R1 != "0,0|4000,0;4000,0|4000,2500")
                __post.Add("R1: sketch loops mismatch, expected 2 loop(s) (geometry)");
        }
        if (__post.Count > 0)
        {
...
// witness R1
{
    ...
    __rb["railing_ids"] = new string[] { __el_R1.Id.ToString() };
