...
from kir.acceptance import derive_expectation, symbol_rows_from_snapshot
from kir.acceptance_evidence import (
    REGULAR_WRITE_EXECUTION_LANE,
    AcceptanceReason,
)
from kir.acceptance_journal import AcceptanceJournal
from kir.acceptance_live import observation_from_census
from kir.acceptance_probe import ACCEPTANCE_OBSERVATION_SCHEMA_VERSION
...
from kir.midend import GroundedProgram, GroundingContext
...
from kir.tests.acceptance_fakes import PassingAcceptanceBridge
...
def _family_plan():
    """A single place_family is the registry's most heavily loaded writing op."""
    return plan_program({
        "ir_version": "1.0",
        "ops": [{
            "op": "place_family",
...
def _payload(plan, census, *, phase, snapshot=GROUND_SNAPSHOT):
    # The reference tables MUST match prepare_acceptance: the expectation
    # participates in the signature, and an extra or missing pool
    # immediately breaks strict parsing.
...
async def test_pre_read_and_fsync_precede_write_authority(tmp_path: Path):
    ...
    with mock.patch(
            "kir.acceptance_runtime.new_acceptance_run_id",
            return_value=RUN_ID):
...
async def test_missing_durable_sink_refuses_before_write():
    ...
    with mock.patch(
            ...
            "kir.acceptance_runtime.configured_evidence_root",
            return_value=None):
        with pytest.raises(AcceptanceRuntimeError) as caught:
            await prepare_acceptance(
                plan, GROUND_SNAPSHOT, _document(), reader,
...
async def test_place_family_reaches_independent_acceptance(tmp_path: Path):
    """THE LIVE LOOP FOR THE MOST HEAVILY LOADED WRITING OP.

    Before 09.08 this program could not obtain an independent "match" for
    any construction whatsoever: `place_family` was unconditionally blind,
    and a blind op yields INCONCLUSIVE by construction. The category is
    taken from the `family_symbols` pool of the SAME snapshot the program
    is grounded on — that is, before the effect.
    """
    plan = _family_plan()
    calls = []
    ...
    # The predicate is fixed to disk BEFORE the write, not justified after it.
    assert calls == ["acceptance_before"]
    assert session.journal.path.exists()
    assert not session.journal.state.finalized
...
async def test_place_family_that_did_not_happen_is_rejected(tmp_path: Path):
    """The same loop, but the instance did not appear — there can be no green."""
    plan = _family_plan()
    ...
    assert [m.code.value for m in evidence.verdict.mismatches] == [
        "category_shortfall"]
    # An internal witness has no right to override the independent measurement.
