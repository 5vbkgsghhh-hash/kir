...
from kir import macros  # noqa: E402
from kir.macros import MAX_EXPANDED_OPS, MAX_STACK_LEVELS
...
class StackMacro(unittest.TestCase):
    ...
    def test_expansion_budget_checked_before_copying_floor(self):
        """🔴 THE TEST WAS TRIPPING OVER SOMEONE ELSE'S CEILING, MEASURED
        25.08.2026.

        The 18.08 fix made the input depend on `MAX_EXPANDED_OPS` (it was
        40x7=280, which silently stopped exceeding the budget) — and
        stopped there. The input became `20000 // 7 + 2 = 2859` floors,
        while `MAX_STACK_LEVELS = 40`, meaning the refusal came from a
        DIFFERENT limit:

            2859 x 7  ->  "stack.levels — an integer 1..40"
            40 x 500  ->  "stack will expand into 20040 ops — limit 20000"

        Both carry `KIR-M001`, and so `assertIn("KIR-M001", ...)` was green
        by construction: strip the budget check entirely and the test
        would not notice. Verified by a run, see the FAIL control below.

        The input is now built from BOTH ceilings, and the TEXT is
        compared, not the code: the code is the same for the two different
        refusals, and it cannot tell them apart.
        """
        per_floor = MAX_EXPANDED_OPS // MAX_STACK_LEVELS
        st = dict(STACK)
        st["levels"] = MAX_STACK_LEVELS
        st["floor"] = [dict(STACK["floor"][0], id=f"W{i}")
                       for i in range(per_floor)]
        out = compile_program(_prog([st]))
        self.assertFalse(out.ok)
        self.assertIn("KIR-M001", [d.code for d in out.diagnostics])
        тексты = " ".join(d.message_ru or "" for d in out.diagnostics)
        self.assertIn(
            "развернётся в", тексты,
            f"отказ пришёл от ДРУГОГО предела, а не от бюджета развёртки: "
            f"{тексты[:140]}")
        self.assertIn(str(MAX_EXPANDED_OPS), тексты)

    def test_КОНТРОЛЬ_вход_упирается_именно_в_бюджет_а_не_в_число_этажей(self):
        """The instrument must prove that it measures ITS OWN limit.

        The same input with the number of floors one less than the
        ceiling must pass the `levels` check and still hit the budget —
        meaning the red comes from the budget, not from its neighbor."""
        per_floor = MAX_EXPANDED_OPS // MAX_STACK_LEVELS
        st = dict(STACK)
        st["levels"] = MAX_STACK_LEVELS
        st["floor"] = [dict(STACK["floor"][0], id=f"W{i}")
        ...
        out = compile_program(_prog([st]))
        тексты = " ".join(d.message_ru or "" for d in out.diagnostics)
        self.assertNotIn("целое 1..", тексты,
                         "отказ всё ещё про число этажей, а не про бюджет")
...
#: A taper with a KINK: hw drops 8000->4000 over the first half and
#: 4000->1000 over the second, that is, at different rates. A uniform
#: taper would give 6250 at step 1, not 6000 — this is exactly how a
#: piecewise track differs from stack.transform.
...
class SeriesMacro(unittest.TestCase):
    ...
    def test_next_chains_segments_end_to_start(self):
        """N repeats stitch N segments across N+1 stations: the end of k == the start of k+1."""
        flat = macros.expand([SERIES])
    ...
    def test_full_compile_walls_no_snapshot_needed(self):
        """Variable edge height/length compiles as ordinary flat IR.

        The level is declared as a SEPARATE op and taken by ref: series,
        unlike stack, creates no levels, and this is visible directly in
        the program's shape."""
    ...
    def test_malformed_reference_is_a_typo_not_a_literal(self):
        """'$hw@nxt' must not end up as text in a numeric field."""
    ...
    def test_track_must_cover_every_index_it_is_read_at(self):
        # hw is read over 0..4 (there is @next), but covers only 0..3
        out = compile_program(_prog([_series(
            track={"hw": [[0, 8000], [3, 1000]], "z": [[0, 0], [4, 12000]]})]))
        self.assertFalse(out.ok)
        d = [x for x in out.diagnostics if x.code == "KIR-M001"][0]
        self.assertIn("hw", d.message_ru)
        # without @next, 0..3 would suffice — checking that the limit depends on @next
    ...
    def test_expansion_cap_counts_items_times_count(self):
        # 40 x 6 = 240 > MAX_SERIES_OPS(200), even though neither count nor items is over the limit
    ...
    def test_cap_is_below_the_program_wide_budget(self):
        """A single series cannot claim the entire program budget."""
    ...
    def test_reference_cannot_forge_op_or_id(self):
        """op/id do not take part in substitution — otherwise the op's name would be assembled from a number."""
    ...
    def test_existing_macros_untouched_by_the_new_one(self):
        """Regression: stack and grid_array expand byte-for-byte as before."""
