...
from kir.authoring import validate as _validate_op  # noqa: E402
from kir.compiler import _parse_and_check, compile_program  # noqa: E402
from kir.decompile.fold import canon_op  # noqa: E402
from kir.decompile.lift import lift_document  # noqa: E402
from kir.decompile.schema import L0Document  # noqa: E402
...
class F5EmitFlips(unittest.TestCase):
    ...
    def test_flip_branches_and_witnesses_are_emitted(self) -> None:
        # F5 v3.1 HYBRID (exam LOT31, 2026-07-21): semantic door flip —
        # flipHand/flipFacing when CanFlip=true (STABLE, doesn't orphan), and
        # mirror-COPY as fallback when CanFlip=false (cracks flip-locked
        # DL/Block, 1-2 per floor).  Live: −1 (cluster parking) 0%→100%,
        # doors 30/30. Witnesses for all three states remain.
        out = compile_program(
            copy.deepcopy(self._PROG), snapshot=GROUND_SNAPSHOT)
        self.assertTrue(out.ok)
        cs = out.csharp
        for marker in (
                "CanFlipHand", "flipHand()",           # the only path
        ...
        self.assertNotIn("MirrorElements", cs)  # F5 v4 — see the live measurement below

    def test_flip_locked_door_never_mirrors(self) -> None:
        """LIVE MEASUREMENT 27.07 (SOB6.2, Revit 2023): mirror-copy wrecks
        OTHER PEOPLE'S doors.

        Three runs on the same set of 178 neighborhood ops, each on its own
        strip (artifacts `mirror_cause_20260727.json`,
        `mirror_cause_BC_20260727.json`):

          A as-is                 — 3 refusals «зеркальная копия недоступна»,
                                    3 violated postconditions, and three doors
                                    WITH NO GEOMETRY AT ALL at point [0, 0];
          B flips removed
            everywhere             — 0 refusals, 0 violations, 0 breakage;
          C flips removed on
            the three failing ops  — the breakage MOVED to a fourth op.

        The key point in A: ops e7300294/e7382303/e9164901 fail, while
        geometry is lost by ENTIRELY DIFFERENT doors — e7813016/e7813017/
        e7813240 on another host. So `MirrorElements` with mirrorCopies=true
        on a hosted door corrupts the document beyond its own op, and no
        per-op SubTransaction contains it.

        The same conclusion stood in the emitter's comment since 21.07 ("no
        mirrors on hosted"), but it was never carried into the code — a rule
        established by reasoning and never closed with the code. This test
        closes it.

        AN UNREACHABLE FLIP IS A TYPED REFUSAL (updated 09.08). It used to
        reach the postcondition, and the witness's verdict is program-scoped,
        so the whole program paid for one leaf that wouldn't turn (16 red
        `create_door` lines on 21.07; all FOUR with recorded violations had
        geometry_ok and topology_ok GREEN). Now the refusal sits on the flip,
        names the family and the next move; under `per_op` it costs its own
        op, not the neighbors. The rule "no mirrors on hosted" is not
        weakened by this: a refusal is the absence of an action, not a new
        one. Details and refuting tests — `test_hosted_flip_refusal.py`.
        """
        ...
        self.assertTrue(out.ok)
        cs = out.csharp
        # the stable path lives on, but unreachability is now a REFUSAL, not
        # silence
        self.assertIn("if (!__el_D1.CanFlipHand)", cs)
        self.assertIn("flipHand()", cs)
        self.assertNotIn("MirrorElements", cs)            # the branch no longer exists
        self.assertNotIn("__kirLockedMirror", cs)         # and neither does its budget
        # the cause is named both for the refusal and for the surviving
        # postcondition
    ...
    def test_a5_carries_no_mirror_budget_because_there_is_no_mirror(self) -> None:
        """The limiter outlived its own mechanism — now it doesn't.

        The `__kirLockedMirror_*` budget existed for exactly one path —
        mirror-copy when CanFlip*=false — and held it at two copies per host,
        because a Revit refusal deferred until Commit could not be contained
        inside a SubTransaction. The path was removed per the live
        measurement of 27.07 (see
        F5EmitFlips.test_flip_locked_door_never_mirrors), so the counter, its
        threshold, and its refusal text must vanish along with it: code that
        guards something nonexistent reads as guarding something that
        exists.
        """
...
class F6LiftWallVertical(unittest.TestCase):
    ...
    def test_column_vertical_dof_is_lifted(self) -> None:
        # P1 DOF-completeness (fidelity audit 2026-07-21): on "demo" 100% of
        # columns are top-attached — without this vertical, every column
        # would be rebuilt as-placed at the symbol's height.
        ...
        self.assertEqual(
            ...
            {"by": "name", "value": "Этаж 2", "_id": "101"})
        self.assertEqual(node["params"]["top_offset_mm"], -300.0)
        # Without the vertical parameters — the historical byte-identical
        # form.
        ...
        # An unresolvable top level = an honest atom (as with a wall).
...
class F6WallHeightVsTopConstraint(unittest.TestCase):
    ...
    def test_control_the_old_unconditional_check_would_have_fired(self) -> None:
        ...
        from kir.authoring import WitnessCheck, _cs, _safe
        from kir.emit_model import tolerance

        s = _safe("s01")
        h = 3000.0            # the registry default -- never asked for
        # The tolerance is presented BY AN OBJECT from the registry: since
        # 03.08 a witness cannot be built with a declared key against a
        # number typed by hand alongside it (law 2, emit_model.py) —
        # reconstructing the old form must obey the same law, otherwise it
...
class LiftEmitRoundTrip(unittest.TestCase):
    def test_f5_lifted_flips_reach_the_emitted_csharp(self) -> None:
        ...
        out = compile_program(prog, snapshot=GROUND_SNAPSHOT)
        self.assertTrue(out.ok)
        # F5 v4: the flip is set only via flipHand/flipFacing where Revit
        # allows it; an unreached flip is held by the witness.
