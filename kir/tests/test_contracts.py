...
class ReceiptContracts(unittest.TestCase):
    ...
    def test_refused_without_commit_requires_explicit_rollback_status(
        self,
    ) -> None:
        common = {
            "run_id": RunId(RUN_ID),
        ...
        self.assertFalse(CommitReceipt(
            **common, commit_status=None).refused_without_commit)
        self.assertFalse(CommitReceipt(
            **common, commit_status="NotStarted").refused_without_commit)
        rolled_back = CommitReceipt(
            **common, commit_status="RolledBack")
        self.assertTrue(rolled_back.refused_without_commit)
        self.assertTrue(rolled_back.decided)

    def test_rebuild_receipt_proves_exact_resume_boundary(self) -> None:
        """THE WRITER-VERSION GUARD.

        /2 -> /3 (29.07, task #25): the chunk receipt gained
        ``op_refusals``/``ops_total``/``ops_no_element`` and the census
        law "created + refused + no-element == ops". The occasion was
        rebuild #11: 113 curtain-wall mullion lines were refused INSIDE
        already committed chunks, and the reason was not preserved
        anywhere. Readability of /1 and /2 is checked by the tests below;
        ``ops_total=0`` turns the law off for old journals.
        """
        receipt = CommitReceipt(
            run_id=RunId(RUN_ID),
            operation="rebuild",
    ...
    def test_v2_receipt_remains_readable(self) -> None:
        """The journals from runs #9-#11 were written as /2 and must
        replay."""
