...
class StructuralFilterLiveCase(unittest.TestCase):
    """2026-07-16 prod case: "highlight the load-bearing walls" burned
    ~5 min of repair (invented STATIC_WALL_BASE_IMAGE / Wall.Structural).
    One filter now."""
    ...
    def test_snapshot_family_pool_is_universal_and_canonically_identified(self):
        ...
        # The same guard as in `test_open_model`, and ONE for both:
        # holding a rule as two copies of a substring is this
        # project's own named defect, and it has already produced a
        # false red here on `WorksetId.IntegerValue`.
...
class DeviceGate(unittest.TestCase):
    ...
    def test_flag_off_and_shadow_hidden(self):
        for v in (None, "off", "shadow"):
            if v is None:
                ...
                подменить_env(self, "KUKAI_KIR_TOOL", v)
    ...
    def test_stage2_admin_device_is_NOT_enough_without_the_mode(self):
        """THE ASSERTION WAS FLIPPED ON 13.08, and this is a change of
        MEANING, not of the fixture.

        Before `61e276bb` the test was called `..._visible` and
        required: the stage2 flag plus an admin device ⇒ the tool is
        visible. **The operator decided otherwise:** KIR is a
        separate mode (a button, a separate window), and restoring
        the flag on its own no longer opens it. So the previous
        assertion became FALSE, and giving this test the mode's
        fixture would have turned it green for nothing, making the
        operator's ban unguardable.

        Here and below are both sides of the third condition, because
        one side alone distinguishes nothing: "closed" is also true
        for a gate closed for a different reason.
        """
        подменить_env(self, "KUKAI_KIR_TOOL", "stage2")
    ...
    def test_stage2_admin_device_and_mode_opens_it(self):
        """The other side: all three conditions together OPEN it,
        otherwise the gate is deaf."""
...
class HandlerOutcomes(unittest.TestCase):
    def setUp(self):
        ...
        # THE GATE'S THIRD CONDITION (13.08): KIR mode is set
        # EXPLICITLY.
        enter_kir_mode(self)
    ...
    def test_structured_write_refusal_without_typed_status_is_unknown(self):
        """A reason code does not substitute for evidence of the
    ...
    def test_handoff_is_normal_result_not_exception(self):
        """"handoff does not break the turn": out-of-coverage -> typed
        dict, no raise."""
        res = self._handle({"ir_version": "1.0", "ops": [
    ...
    def test_write_program_fetches_snapshot(self):
        ...
        # THE THIRD PASS APPEARED ON 17.08.2026, AND IT APPEARED ON
        # PURPOSE: the built-result judge rereads what was created
        # and checks it against what was declared (`built_verdict`).
        # Measurement before the insertion, a live `13A-RD-AR-K2`
        # with 310,558 elements: **1.04–1.17 s, slope over N is
        # ZERO** — the cost is a document walk, not the id count.
        # That is +6.3% of the turn against the 16.08 measurement of
        # 17–18 s, of which 84.6% is spent waiting on the model.
        #
        # The line was updated, not relaxed: this test guards the
        # NUMBER OF PASSES, and its purpose is to make sure a pass
        # can never be added silently.
        from kir import built_verdict as _bv
        self.assertEqual(calls, ["ground_snapshot", "write", _bv.PHASE])
    ...
    def test_live_exact_snapshot_guards_document_and_pinned_identity(self):
        # The open-model preflight is opt-in (it can refuse a write
        # on a truncated snapshot of a large model, and has never
        # been checked live), so the test turns the flag on itself:
        # it checks the GATED behavior.
        подменить_env(self, "KUKAI_IR_OPEN_MODEL_PREFLIGHT", "1")
    ...
    def test_disambiguate_by_parameter_is_fetched_and_resolved_same_round_trip(self):
        ...
        self.assertTrue(res["ok"], res)
        # THIS TEST GUARDS "ONE PASS PER GROUNDING," not their total
        # count: the third pass is the built-result judge
        # (`built_verdict`, 17.08.2026), and it runs AFTER the write.
        # That is exactly the difference, and it shows in the phase's
        # name.
        from kir import built_verdict as _bv
        self.assertEqual([op for op, _ in calls],
                         ["ground_snapshot", "write", _bv.PHASE])
