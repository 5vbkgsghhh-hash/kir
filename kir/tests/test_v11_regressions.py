...
class Nahodka1OkOverError(unittest.TestCase):
    ...
    def test_stale_and_duplicate_and_unclassified(self):
        for payload, want in (
                # X009, not X003: there is no "after grounding" signature
                # here, so drift is proven by nothing. One code for drift and
                # for a runtime refusal was split apart on 09.08 — see
                # `test_refusal_identity.OneCodeIsOneWorld`.
    ...
    def test_success_carries_witness_triple(self):
        ...
        # THE TRIPLE IS CHECKED BY NAME, NOT BY DICT EQUALITY. On 10.08 the
        # witness gained `unwitnessed_axes` (which axes nobody promised to
        # measure), and exact equality would have broken on ANY added field —
        # even though the test is called "carries witness triple" and is
        # about the triple.
...
class StaticGeometryLaw(unittest.TestCase):
    ...
    def test_nonadjacent_touch_and_collinear_overlap_are_intersections(self):
        from kir.geom import _seg_intersect
