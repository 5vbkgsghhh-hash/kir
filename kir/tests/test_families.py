...
class CreateTypeIdempotency(unittest.TestCase):
    ...
    same Family. Only an exact owned, unchanged result may be reused read-only;
    a name alone never authorizes mutation of a shared type."""
    def test_emits_twin_search_before_duplicate(self):
        ...
        self.assertIn("if (__twin_T1 != null) {", cs)
        self.assertLess(cs.index("String.Equals(__owner_T1"), cs.index("__el_T1 = __twin_T1;"))
        self.assertIn("StringComparison.Ordinal", cs)
        self.assertIn("if (__dupd_T1) {", cs)
...
class CreateTypeNegative(unittest.TestCase):
    ...
    def test_source_type_name_not_found_offers_candidates(self):
        """THE CANDIDATE MUST CARRY AN ADDRESS, NOT JUST A NAME.

        🔴 HERE STOOD `assertIn("К 300x300", d.candidates)`, and this was
        the THIRD instance of the same miss. Before `4f6a9316` (24.08.2026)
        candidates were bare strings; that commit put an `id` into the
        string — the only thing by which the author reassembles the
        selector deterministically
        (`{"by": "element_id", "value": <id из candidates>}`), — and a sign
        of unfitness. Two checks in `test_struct.py` turned red at the time
        and were taken apart by the helper `_candidate_row_with_address`;
        THIS ONE did not turn red along with them and stood red until 01.09,
        because it lives in a different file, and the search was by red,
        not by pattern.

        The neighbor's helper states verbatim what must NOT be done: a
        mechanical replacement with a comparison on the `name` field would
        come back green AND WOULD LOSE EXACTLY WHAT THE COMMIT ADDED — the
        next day the `id` would be gone, and no instrument would beep. That
        is why the PAIR is checked, and the disappearance of either half
        turns red. The pair `500`/«К 300x300» is the same one recorded
        further down the file as the `(source_type, category)` correlation.

        By its own body, not by calling the neighbor: the helper lives in
        `test_struct.py`, and importing a test from a test would tie two
        files together for the sake of six lines. The shared home
        (`fixtures.py`) RIDES IN THE WHEEL — putting the helper there for
        one call would mean growing the delivery.
        """
        out = compile_program(_prog([_ctype(
        ...
        self.assertIn("К 300x300", by_name,
                      f"кандидат «К 300x300» не предъявлен: {rows!r}")
        self.assertEqual(
            by_name["К 300x300"].get("id"), 500,
            f"кандидат приехал БЕЗ АДРЕСА либо с чужим: {by_name!r}. "
    ...
    def test_runtime_lookup_failure_is_typed_rollback_not_silent(self):
        ...
        self.assertIn('"материал «Бетон» отсутствует или неоднозначен в документе"', cs)
        self.assertIn('__materials_T1.Count != 1', cs)
...
class LoadFamilyHappyPath(unittest.TestCase):
    def test_whole_family_load(self):
        ...
        # NOT ``GetFamilySymbolIds()``: it returns ``ISet<ElementId>`` from
        # ``System.dll``, which the deployed plugin does not have (CS0012).
...
# THE GOLDEN CORPUS OF THIS WAVE WAS TAKEN DOWN ON 13.08.2026, AND THE REASON
# IS NOT "THE EMITTER DRIFTED".
#
# Here stood a `Golden` class with two programs — `families_create_type_full`
# and `families_load_family_whole` — writing into a SHARED `golden/`
# directory. Its docstring claimed, verbatim: «Own files in the SHARED
# golden/ dir (families_*.golden.cs — no filename collision with any other
# wave's programs)». The claim stopped being true, and NOTHING noticed it: a
# later wave introduced the SAME TWO NAMES into `test_golden.PROGRAMS`.
#
# THE 13.08 MEASUREMENT on the merged line `integration/kir-2026-08-13`:
#
...
#     golden directory              same                  same
#     grounding snapshot            same                  same
#
# One file cannot satisfy both programs. The bytes on disk match
# `test_golden` — meaning `test_families` was not going red from "emission
# drift" but from the artifact having TWO OWNERS. Re-freezing would have
# turned the other test red, and so on in a circle: this is not drift, it is
# oscillation without convergence.
#
# The collision PRECEDED the merge — it is present at base `ba36f7bc` and
# identically on all four branches (checked with `git show` on each).
#
# COVERAGE IS NOT LOST, and this is measured, not claimed: the op's field
# set in both editions MATCHES by name (`create_type`:
# category/depth_mm/material/new_name/source_type/width_mm; `load_family`:
# path); only the values of `intent` and `path` differ, and neither of them
# selects the emitter branch. The richer edition is `test_golden`, and it
# remains the sole owner.
#
# So that the claim from the old docstring would stop being prose, a guard
# was set up: `test_golden_files_have_one_owner.py` requires that every
# `golden/*.golden.cs` file be claimed by EXACTLY ONE corpus.
