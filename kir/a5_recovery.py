...
def _validate_transition_proof(
    ...
    *,
    ...
    if phase is A5Phase.REBUILT:
        ...
        # A KNOWN outcome, not just a commit. A refusal ("Revit
        # answered, the transaction rolled back, there are no
        # witnesses") closes its own program just as reliably; UNKNOWN
        # remains illegal — its effect never finishes at all and never
        # reaches the proof.
        ...
        if "program_ids" in proof:
            ...
            # A refusal does not move the document and carries no
            # revision, so the final revision is proved by the LAST
            # RECEIPT THAT HAS ONE. Requiring it from a refusal would
            # mean requiring evidence of a write that never happened.
            with_revision = [
...
class A5Journal:
    ...
    def create(
        ...
        *,
        ...
        # 🔴 F-153: THE RECORD OF A DIRECTORY'S NAME LIVES IN ITS
        # PARENT, NOT IN ITSELF. The earlier edition fsync'd the journal
        # file and `a5_runs` itself — and stopped there. As long as
        # someone else created `out_dir`, there was no hole: they had
        # already fixed its name. But `mkdir(parents=True)` creates the
        # whole chain ITSELF, and that is the ordinary case of the FIRST
        # A5 run into a fresh output directory: then, after a power
        # loss, what disappears is not a record in the journal, but the
        # WHOLE JOURNAL along with any trace that the turn ever
        # happened — and a retry builds, a second time, what has
        # already been built.
        #
        # The list of things being created is taken BEFORE `mkdir`:
        # after it, "did not exist" is already indistinguishable from
        # "existed." We fsync the parent of EVERY thing created and stop
        # at the first one that existed before us — the rule does not
        # reach beyond the measured defect (pinned by a separate test).
        созданные: list[pathlib.Path] = []
        предок = journal_dir
        while not предок.exists():
            созданные.append(предок)
            if предок.parent == предок:            # the root: there is nowhere further up to go
                break
            предок = предок.parent
        journal_dir.mkdir(parents=True, exist_ok=True)
        закрепить = [journal_dir] + [p.parent for p in созданные]
        ...
        if hasattr(os, "O_DIRECTORY"):
            # Order FROM DEEP TO SHALLOW: the name lives in the parent,
            # so the parent is fsync'd AFTER the thing it names. We
            # remove duplicates, we keep the order — `dict.fromkeys`,
            # not `set`.
            for каталог in dict.fromkeys(закрепить):
                try:
                    directory_fd = os.open(
                        каталог, os.O_RDONLY | os.O_DIRECTORY)
                    try:
    ...
    @classmethod
    @staticmethod
    def resumable_absence_reason(
        out_dir: "str | os.PathLike[str]",
    ) -> "str | None":
        """`None` if "there are no unfinished runs" is the honest
        answer; otherwise the REASON this conclusion is doubtful.

        🔴 A COMPANION to `find_resumable`. That one returns `None` when
        the `a5_runs` directory does not exist, and that answer must not
        be dropped: finding no journal, the turn is required to start a
        new run. But `None` carries two different meanings:

            there were NO A5 runs in this directory at all -> all correct
            the `a5_runs` directory was DELETED              -> evidence of
                                                                 a past run
                                                                 is lost, and
                                                                 a new one is
                                                                 written over
        ...
        The second is visible from outside: the decompile directory
        itself exists and is not empty, meaning work DID happen here.
        This is not proof of loss — A5 is not switched on on every
        turn — but that is exactly why the suspicion must be spoken in
        words, not left as silence: it is the reader who decides it, not
        the instrument.

        The A5 protocol exists for the sake of evidence after a
        failure; a silent zero here costs more than the ordinary one.
        """
        base = pathlib.Path(out_dir)
        if (base / "a5_runs").is_dir():
            return None
        siblings = [entry.name for entry in sorted(base.iterdir())
                    if entry.name != "a5_runs"] if base.is_dir() else []
        if not siblings:
            return None
        return (f"каталога a5_runs нет, но разбор {base.name} не пуст "
                f"({len(siblings)} записей: {', '.join(siblings[:3])}"
                f"{', …' if len(siblings) > 3 else ''}) — работа здесь ШЛА. "
                f"«Незавершённых прогонов нет» может быть фактом о МАШИНЕ: "
                f"каталог журналов снесён, и новый прогон запишется поверх "
                f"пустого места")
    ...
    def find_resumable(
        ...
        *,
        ...
        if not journal_dir.is_dir():
            return None
        # The reason for this `None` is asked of
        # `resumable_absence_reason()`: the answer must not be
        # dropped — finding no journal, the turn is required to start a
        # new run — but "there were no runs" and "the directory was
        # deleted" are resolved DIFFERENTLY.
