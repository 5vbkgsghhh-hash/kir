...
from kir.authoring import _op_refs
...
def effects_enabled() -> bool:
    ...
    return env.get("KIR_EFFECTS", "").strip().lower() in {
        "1", "true", "yes", "on",
    }
    ...
    def _walk(node: Any) -> None:
        if isinstance(node, Mapping):
            ...
            for key, value in node.items():
                if key in spec.SYNTHETIC_FIELDS:  # authority: registry_base
                    continue
                _walk(value)
...
def build_dependency_graph(
    ...
    # are serialized by program order (conservative, R5).
