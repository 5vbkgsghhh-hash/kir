...
from kir.decompile.geometry_acceptance import FormAcceptanceState
from kir.idempotence_contract import Vec3
...
class KindComparison:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            # Expectations carved out of the comparison universe: they enter
            # neither `expected`, nor `missing`, nor any percentage — they are
            # published as a number so the carve-out is visible, not silent.
...
class IdempotenceReport:
    ...
    #: Expectations carved out of the comparison universe BY CONSTRUCTION: the
    #: effect of a ``modify``-family op that produced no element in ``created_ids``.
    ...
    #: Receipts for ISOLATED programs that Revit refused. Each is one host
    #: group (the radius is proven by construction), so its refusal is not
    #: fatal to the run. An empty tuple = none refused.
    isolated_failures: tuple[dict[str, Any], ...] = ()
    #: Refusals of INDIVIDUAL ops inside COMMITTED chunks (07.29, task #25).
    #: A class that used to be invisible: run #11 reported "Committed", yet
    #: 113 of 122 curtain-wall subdivision lines refused inside successful
    #: transactions and landed in missing without a single word of reason. The
    #: ops stay in the denominator — they were promised and were not built.
    op_refusals: tuple[dict[str, Any], ...] = ()
    #: Receipts for ORDINARY chunks that Revit refused with a KNOWN outcome
    #: (`refused_without_commit`). Their ops stay in the denominator — they
    #: were promised and were not built. An unknown outcome does not land
    #: here: it fails the run instead of joining this list.
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        d: dict[str, Any] = {
            ...
            # The isolated slice goes into the SAME discrepancies list: the
            # report's reader is not required to remember a second place
            # where the truth about the unbuilt lives.
            ...
            "comparable_coverage": {
                ...
                "atoms_form_inconclusive": self.atoms_form_inconclusive,
                "atoms_form_pending": form_pending,
                # Ops of refused isolated programs STAY in the denominator:
                # they were promised and were not built. Carving them out
                # would raise the percentage on the back of what does not exist.
                "isolated_refused_ops": isolated_ops,
                # The same rule for ordinary chunks: the ops of a refused
                # chunk were promised and were not built, so they stay in the
                # denominator. Carving them out would raise the percentage
                # by exactly what is absent from the model.
                "chunk_refused_ops": chunk_ops,
                # The same rule: an op refused inside a SUCCESSFUL chunk was
                # still promised and not built — it is not carved out of the denominator.
