...
READ THIS CORPUS VIA sudo OR SAY YOU COULD NOT — BUT NEVER REPORT ZERO. The
receiving directory is private-mode (0700, owned by the service user), and
this breaks instruments more quietly than expected: `pathlib.glob` over a
directory without read permission returns an EMPTY list, not an error. A
session without permissions gets "no evidence" where the truth is "cannot
see". A missing index and an empty index are different facts, and here the
environment itself conflates them. Measured 11.08.2026 on 44 real journals:
without sudo, listing yields 0 files with not a single error.
"""
...
from kir.outcome import ProgramOutcome


ACCEPTANCE_JOURNAL_SCHEMA_VERSION = "kir-acceptance-journal/3"
PREVIOUS_ACCEPTANCE_JOURNAL_SCHEMA_VERSIONS = frozenset({
    "kir-acceptance-journal/2",
})
...
def configured_evidence_root() -> pathlib.Path | None:
    ...
    named = env.get(ACCEPTANCE_EVIDENCE_DIR_ENV)
...
class AcceptanceJournal:
    ...
    def _replay(
        ...
        if schema_version not in ({ACCEPTANCE_JOURNAL_SCHEMA_VERSION}
                                  | PREVIOUS_ACCEPTANCE_JOURNAL_SCHEMA_VERSIONS):
            raise AcceptanceJournalError(
        ...
        for index, row in enumerate(rows):
            ...
            if index == 0:
                ...
                elif event == "aborted_before_artifact":
                    if schema_version != ACCEPTANCE_JOURNAL_SCHEMA_VERSION:
                        raise AcceptanceJournalError(
                            "pre-artifact abort is unsupported by this schema")
                    if finalized or artifact_binding is not None:
                        raise AcceptanceJournalError(
                            "pre-artifact abort requires prepared state")
                    if body.get("registration_digest") != registration_digest:
                        raise AcceptanceJournalError(
                            "pre-artifact abort belongs to another registration")
                    final_payload = _mapping(
                        body.get("terminal"), "pre-artifact abort payload")
                    cls._verify_pre_artifact_abort(final_payload)
                    finalized = True
                    terminal_event = "aborted_before_artifact"
    ...
    def _replay_legacy_v1(
        ...
        return AcceptanceJournalReplay(
            ...
            final_payload=final_payload,
            terminal_event=("finalized" if finalized else None),
            legacy_unbound=True,
        )
    ...
    @staticmethod
    def _verify_pre_artifact_abort(terminal: Mapping[str, Any]) -> None:
        """Record the single terminal state legal before an artifact appears.

        It is deliberately already ``finalize``: it writes a proven refusal
        while the journal is exactly in PREPARED. Rollback, unknown
        execution, witness results, or acceptance evidence cannot be
        encoded here.
        """

        outcome = _mapping(
            terminal.get("outcome"), "pre-artifact abort outcome")
        expected = {
        ...
        if outcome != expected:
            raise AcceptanceJournalError(
                "pre-artifact abort requires an exact non-started outcome")
        if terminal.get("evidence") is not None:
            raise AcceptanceJournalError(
                "pre-artifact abort cannot carry acceptance evidence")
        if terminal.get("evidence_digest") is not None:
            raise AcceptanceJournalError(
                "pre-artifact abort cannot carry an evidence digest")
        if terminal.get("execution_artifact_binding_digest") is not None:
            raise AcceptanceJournalError(
                "pre-artifact abort cannot carry an artifact binding")
        detail = terminal.get("detail")
        if detail is not None:
            _mapping(detail, "pre-artifact abort detail")
    ...
    def abort_before_artifact(
        self,
        outcome: ProgramOutcome,
        *,
        detail: Mapping[str, Any] | None = None,
    ) -> None:
        """Fsync a proven terminal refusal of PREPARED + NOT_STARTED.

        The method is deliberately unavailable to V1/V2 journals and after
        any artifact has been bound. A caller that may already have sent
        the record must use the ordinary terminal of the bound artifact
        (typically ``unconfirmed``), but never this replay-safe abort.
        """

        if not isinstance(outcome, ProgramOutcome):
            raise TypeError("journal abort outcome must be typed")
        if self.state.legacy_unbound:
            raise AcceptanceJournalError(
                "legacy unbound journal is archive-only")
        if self.state.schema_version != ACCEPTANCE_JOURNAL_SCHEMA_VERSION:
            raise AcceptanceJournalError(
                "pre-artifact abort is unsupported by this schema")
        if self.state.artifact_binding is not None:
            raise AcceptanceJournalError(
                "pre-artifact abort cannot follow artifact binding")
        if detail is not None and not isinstance(detail, Mapping):
            raise TypeError("journal abort detail must be an object")
        terminal: dict[str, Any] = {
            "outcome": outcome.to_dict(),
            "evidence": None,
        ...
        if detail:
            terminal["detail"] = dict(detail)
        self._verify_pre_artifact_abort(terminal)
        self._append_terminal(
            terminal,
            event="aborted_before_artifact",
        )
    ...
    def _append_terminal(
        ...
        *,
        ...
        with self._write_guard:
            ...
            if event == "aborted_before_artifact":
                if self.state.schema_version != ACCEPTANCE_JOURNAL_SCHEMA_VERSION:
                    raise AcceptanceJournalError(
                        "pre-artifact abort is unsupported by this schema")
                if self.state.artifact_binding is not None:
                ...
                if self.state.artifact_binding is None:
                    raise AcceptanceJournalError(
                        "terminal evidence is missing its artifact binding")
                self._verify_terminal(
                    terminal,
                    self.state.registration_digest,
                    self.state.artifact_binding,
                )
            else:
                raise AcceptanceJournalError(
                    "unsupported acceptance terminal event")
            body = {
                "schema_version": self.state.schema_version,
                "seq": self.state.sequence + 1,
                "event": event,
