...
import os
from kir import env  # noqa: E402  (a dependency-free submodule — introduces no import cycle)
import re
...
def _weak_sandbox() -> bool:
    ...
    🔴 THE TWO HALVES READ ONE NAME WITH TWO DIFFERENT DICTIONARIES — measured
    2026-08-17. Here it is `env.get(...).strip().lower()`, while the other
    half (`src/Kukai.Revit.Bridge/Security/NamespaceValidator.cs:205`) uses
    `weak == "1"` as an exact comparison and `Equals(..., OrdinalIgnoreCase)`
    for the other three — WITHOUT a `Trim()` on any of them. So a value of
    `" 1"` or `"1 "` (a space arrives from `.env` silently) gives a WEAK
    server half and a STRICT client one: what diverges is not the machines,
    but the NORMALIZATION. There is no channel in the tree that would make
    them agree, and there is nothing to build one from here — the other half
    reads the environment of a FOREIGN process on the user's machine. So the
    state is not "fixed", but NAMED: `sandbox_posture()` below, and from
    there into `/diagnostics`."""
    return env.get("KIR_WEAK_SANDBOX", "").strip().lower() in ("1", "true", "yes", "on")


def sandbox_posture() -> dict[str, str]:
    """WHAT THE CODE IS CURRENTLY FILTERED BY — as an answer you can ask for, not only a log line.

    WHY THIS IS A SEPARATE FUNCTION AND NOT `if _weak_sandbox()` AT THE CALL
    SITE. The state was already being printed — but in exactly one place and
    exactly once per process lifetime: the startup banner `main.py:322`.
    Measured 2026-08-17 from `journalctl -u kukai-backend` over 07-27..08-17:
    **224 entries "sandbox=WEAK (KUKAI_WEAK_SANDBOX)"**, and all 224 are
    visible only to someone with ssh on the box. Whoever decides whether to
    set the flag does not look at journald: they click "Complain" in the
    chat, and that button (`kukai_chat_v5.html:8973`) collects
    `/diagnostics` — where there was NOT A WORD about the sandbox. The value
    was asserted in one place and read in another; here it becomes something
    you can ask.

    WHAT EXACTLY IS TURNED OFF, BY THE NUMBER: `validate_code_safety`, when
    the flag is set, returns `None` on its FIRST line — every one of the
    `len(BLOCKED_PATTERNS)` regex rules (43 as of 2026-08-17) plus two
    contextual checks (`_getfield_reflection_violations`,
    `_typeof_assembly_violations`) passes right through, 45 in total. The
    number is not written in as a literal but computed below — otherwise it
    would go stale at the very next edit to the table.

    IN PRODUCTION THE FLAG IS SET: `KUKAI_WEAK_SANDBOX=1`, measured
    2026-08-17 by reading
    `/proc/$(systemctl show -p MainPID --value kukai-backend)/environ` (the
    same call returned 134 `KUKAI_` keys — the instrument is sound). The
    code's default here is `""`, i.e. STRICT, and it cannot be reasoned
    about from that.

    WHAT THIS FUNCTION DOES NOT KNOW AND DOES NOT PRETEND TO KNOW: the other
    half of the defense (`NamespaceValidator` in the plugin) reads the SAME
    variable from the environment of a different process on the user's
    machine. Its value is therefore not visible from here at all, so
    `detail` says "the server half", not "there is no sandbox": those are
    different claims, and we cannot prove the second one.
    """
    checks = len(BLOCKED_PATTERNS) + 2  # +2 contextual checks, see above
    if _weak_sandbox():
        return {
            "mode": "weak",
            "detail": (
                "KUKAI_WEAK_SANDBOX взведён: серверная проверка кода "
                "(validate_code_safety) отключена целиком — %d правил не "
                "применяется. Вторая половина (NamespaceValidator в плагине) "
                "читает ту же переменную на машине пользователя, отсюда её "
                "значение не наблюдаемо." % checks
            ),
        }
    return {
        "mode": "pattern-validation",
        "detail": (
            "Серверная проверка кода применяет %d правил. Состояние второй "
            "половины (NamespaceValidator в плагине) отсюда не наблюдаемо."
            % checks
        ),
    }
