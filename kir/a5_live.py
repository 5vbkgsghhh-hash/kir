...
tool handler or choose request scope; ``kir.serving`` retains dispatch,
while ``kir.a5_contract`` owns request identity.
"""
...
from kir.document_guard import (
...
# is using that type.  See ``kir/idempotence.py``/task #69 notes for the
# decision record.
...
def _op_results(exec_res: Any) -> dict[str, Any]:
    """The per-op chunk output (``__results``), or empty.

    The unwrap is SINGLE-LEVEL and deliberately mirrors
    ``collect_created_ids``: refusals are required to be read from the SAME
    map as ids, otherwise two readers of one envelope would diverge on which
    ops even exist, and the census law would end up checking its own parsing
    bug.  ``_a5_payload`` does not fit here — it descends up to three levels
    and on an envelope of ``{"ok":.., "result":{op_id:..}}`` would return the
    op map itself, after which a second ``.get("result")`` would give empty.
    """
...
def collect_op_refusals(
    ...
    """Per-op refusals — from the same ``result`` that ids are taken from.

    Under ``isolation="per_op"``, emission places a refusal next to
    successes: ``__results[oid] = {"refused": <text>, "refused_op_id":
    oid}``.  Before /3 no one read it: ``collect_created_ids`` skips a row
    with no ``id``, and the receipt stored only ``element_ids`` — and 113
    refused grid lines (rebuild #11) were left without a single word about
    the cause.

    By ``op_id``, the op's NAME is reached, along with HOW it addressed its
    target: the line "G refused" is useless to a reader, they need
    "create_curtain_grid_line on host e8152799".  The reason text is NOT
    TRUNCATED — it is exactly what named the culprit element in chunk 9's
    decompile.
    """
...
def count_ops_without_element(exec_res: Any) -> int:
    """Ops that closed WITHOUT creating an element, BY SEMANTICS.

    Only an explicit ``created: false`` (swapping a panel type in place).  A
    row with no ``id``, no ``created:false``, and no ``refused`` deliberately
    does NOT go here: that is unknown, and the census law is required to
    catch it, not hide it — exactly the "silently not created" class this
    law exists for.
    """
...
def _cleanup_covers(
    ...
    """Whether every created element is covered — by deletion OR by absence.

    A per-item deletion witness is a false negative under a CASCADE: a
    curtain grid cell leaves the model together with its host, and by the
    time the queue reaches its id there is nothing left to delete. MEASURED
    28.07 (runs #4 and #5): exactly 12 of 1236 created elements returned
    "element not found," while the final census of the stamp for both runs
    showed zero remainders.

    Strictness is preserved and even sharpened: a SURVIVING created element
    is still a cleanup failure, because the census sees it. Only what is not
    in the model and was also not deleted by us is uncovered; that state IS
    the goal of cleanup, not a violation of it.
    """
...
def _receipt_from_journal(
    ...
    """The effect receipt, including the TRANSITIONAL refusal shape.

    From 568349f6 up to this edit, a refusal was written as an ad-hoc dict
    ``{outcome, program_id, bridge_detail}`` — with no schema and no
    operation. Run #4's journal (v9) holds exactly such a record, and
    reading it as "this program never existed" would mean declaring
    uncovered a plan that is in fact closed: Revit answered, the transaction
    was rolled back, there are no elements.

    The shape is unambiguous, so it is raised into the same CommitReceipt.
    Everything else — cleanup and reconciliation receipts — are not commit
    receipts.

    🔴 WHAT EXACTLY SEPARATES OUT WHAT ISN'T OURS (29.08.2026, audit finding
    F-121). There used to be a promise here, "no witnesses," with NO CODE
    behind it: it accepted ANY mapping with `outcome=refused_without_commit`
    and a 64-hex `program_id`, while the `operation` field — which the
    transitional record does not carry at all — was SUBSTITUTED with the
    literal "rebuild". Measured: a record with its own `operation:
    "cleanup"` was raised as a rebuild receipt and closed SOMEONE ELSE'S
    coverage program (`decided == True`).

    The "no witnesses" trait is not operational — neither the transitional
    nor the cleanup record has them — and so no one ever coded it. What
    separates them is the `operation` FIELD: the transitional shape
    (``{outcome, program_id, bridge_detail}``) does not carry it, so the
    ABSENCE of the field is its trait, and presence is a foreign kind that we
    take on faith.
    """
    ...
    if not isinstance(program_id, str):
        return None
    # THE OPERATION IS READ, NOT INVENTED. No field present is a
    # transitional form, and "rebuild" is legal; a field present — we
    # trust IT, not ourselves. Requiring `operation == "rebuild"` ALWAYS
    # is impossible: it would reject exactly the record the function was
    # written to read.
    declared_operation = raw.get("operation")
    if declared_operation is None:
        operation = "rebuild"
    elif declared_operation == "rebuild":
        operation = "rebuild"
    else:
        # A cleanup / reconciliation — exactly what the docstring
        # excluded.
        return None
    try:
        return CommitReceipt(
            run_id=run_id, operation=operation, element_ids=(),
            bridge_error=True, commit_confirmed=False,
            commit_status="RolledBack", program_id=program_id)
    except Exception:  # noqa: BLE001 — a foreign shape remains foreign
        return None
...
class _A5Recovery:
    ...
    def _current_rebuild_receipts(
        ...
        for effect_id in sorted(self.journal.state.effect_receipts):
            ...
            if receipt is None:
                continue
            # RESOLVED outcomes, not only commits: a refusal closes its
            # own program just as reliably. Unknown (a timeout) does not
            # fall here — its effect never finishes at all.
    ...
    def _decided_receipts(self, operation: str) -> list[CommitReceipt]:
        """Receipts with a KNOWN outcome — confirmed and refused."""
    ...
    async def after_rebuilt(self, created_ids) -> None:
        ...
        if not phase_at_least(self.journal.state.phase, A5Phase.REBUILT):
            # PLAN COVERAGE — over the set of programs while keeping
            # strictness: every program has exactly one KNOWN outcome. A
            # commit and a refusal cover it equally; the unknown covers
            # nothing and still fails the transition.
            ...
            # The order of receipts in the proof is the plan, not the
            # order of effects: the proof must be read against the plan.
            by_program = {
