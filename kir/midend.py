...
from kir.registry_base import EffectKind, ResultSpec, SYNTHETIC_FIELDS


#: `/4` does not change the input KIR 1.0. It names a corrected SEMANTICS of
#: the `PlannedOp.result` field: under `/3`, the payload-dependent
#: `create_wall_type` was always recorded as `wall_type`, even though the
#: signed OpContract and the typecheck already distinguished
#: floor/roof/ceiling. Keeping the old name would mean giving two different
#: claims about one plan a single schema tag. All old `/3` documents remain
#: immutable archival evidence with their own digests; the new compiler does
#: not rename them. Plans with no explicit native lineage keep `/4` below.
PLAN_SCHEMA = "kir-planned-program/4"
# Explicit native ownership is signed, unlike the descriptive units passenger.
# Keep absent-lineage /4 evidence byte-for-byte; never invent an owner for it.
LINEAGED_PLAN_SCHEMA = "kir-planned-program/5"
#: /4 names payload-dependent grounding results. The lineage-bearing plan /5
#: changes its parent digest, not grounding's own fields or selector semantics.
GROUND_SCHEMA = "kir-grounded-program/4"
#: The one grammar of a stable program identity (the native type-ownership
#: marker). Every carrier matches THIS object: the compiler's typed diagnostic,
#: the plan's own check, the retained-archive reader and `emit_core.lineage_token`.
LINEAGE_FORM = re.compile(r"^[A-Za-z0-9._:-]{1,64}$")
...
class PlannedProgram:
    ...
    plan_digest: str = ""
    #: THE TABLE OF INTENT UNITS from the author's envelope (`course.unit()`).
    #:
    #: 🔴 SET UP 25.08.2026, BECAUSE THIS IS WHERE INTENT WAS ACTUALLY
    #: BREAKING OFF. The live door passes `compile_program` NOT a dict but a
    #: `PlannedProgram` (`serving.py`: `compile_input = routed_plan ...`), and
    #: the plan had no `units` field at all. `_units_of` reads `units` only
    #: from a `Mapping`, so in prod `CompileOutput.units` was ALWAYS empty,
    #: the `units` key was NEVER placed into the receipt, and an observation
    #: about arity N was addressed by the number `u0`, which means nothing to
    #: a reader.
    #:
    #: NOT INCLUDED IN `_unsigned_evidence`, AND THAT IS A DECISION. The
    #: compiler does NOT INTERPRET the table: it changes neither the
    #: emission, nor the plan, nor the grounding — it is a PASSENGER riding
    #: along to the receipt. Signing it would mean shifting the
    #: `plan_digest` of EVERY already-saved plan for the sake of a label. The
    #: cost is named: an unsigned passenger can be swapped unnoticed, and it
    #: MUST NOT be relied on as evidence — it names, it does not prove.
    units: tuple = ()
    ...
    def __post_init__(self) -> None:
        ...
        if self.lineage is not None and (
                ...
                or LINEAGE_FORM.fullmatch(self.lineage) is None):
            raise ValueError("planned lineage must be a valid non-empty stable identity")
    ...
    def evidence_schema(self) -> str:
        return LINEAGED_PLAN_SCHEMA if self.lineage is not None else PLAN_SCHEMA
...
#: 🔴 A CLOSED LIST OF PLACES WHERE GROUNDING DOES NOT HAPPEN AND THIS IS
#: TOLERATED FOR NOW. It is allowed only to SHRINK, and a line is removed by
#: the commit that fixes the place
#: (`test_the_ledger_of_ungrounded_places_holds_no_ghosts`).
#:
#: SET UP 30.08.2026 AFTER AN INCIDENT, not ahead of time. The F-103 check
#: raised a `ValueError` on `site_subregion_hosted`, the compiler caught it
#: as a PANIC, `KIR-P000`, and the 6/6 gate turned red on 12 refusals (one
#: program × six versions × two modes). The program used to compile BEFORE
#: the fix — that is, my refusal fired on legitimate input, the second time
#: in one shift.
#:
#: BUT THE FINDING IS CORRECT, AND SO THE ENTRY STAYS HERE RATHER THAN BEING
#: STRUCK OUT. Measured across all 73 golden programs, shape by shape:
#:
#:     kind=sel  by=element_id   grounded 25   stayed authoring 1  <- this one
#:     kind=sel  by=name         grounded 41   stayed authoring 0
#:     kind=sel  by=ref          grounded  8   stayed authoring 0
#:     kind=target_w (any by)    grounded  0   stayed authoring 29
#:
#: That is, of the `sel` kind, 74 out of 75 selectors get grounded, and
#: exactly one does not. This is NOT "element_id doesn't get grounded" (25
#: of those do get grounded) and not "the `target_w` kind" (its check
#: doesn't apply here at all): this is a hole specifically in
#: `create_site_subregion.host`, and it lives in `kir/ground.py` — outside
#: the holding of the smith who set up this entry. The hole does not block
#: emission: the program compiles and the golden matches, so it is tolerated.
UNGROUNDED_PLACES: dict[tuple[str, str], str] = {
    ("create_site_subregion", "host"):
        "30.08.2026: заземлитель не опускает этот host, хотя 25 таких же "
        "`sel`+`element_id` опускает. Эмиссии не мешает — голден совпадает. "
        "Чинится в `kir/ground.py`; снять строку обязан тот же коммит",
}


def _authored_selectors_left(op_name: str,
                             payload: dict[str, Any]) -> list[str]:
    """Fields DECLARED as selectors by the registry that remained authoring.

    🔴 ZERO WORK IS NOT ZERO DISCREPANCIES (30.08.2026, audit finding
    F-103). `_assert_payload_refinement` skips EVERY value that didn't
    change, before checking whether the selector fields were left
    unresolved. If NOTHING changed, there was nothing left to check, and the
    function was satisfied. So `GroundedProgram.from_ops(plan,
    plan.to_ops())` — "here's a plan, and here it is again in the role of a
    grounded one" — PASSED: the selector stayed authoring (`{by: name}`),
    not a single resolution happened, and both honesty flags reported true
    and rode into the receipt as proof that the compiler had queried the
    document.

    The flag is read from the REGISTRY, not from the shape of the value. A
    grounded selector has the shape `{"__grounded__": ...}`; an authoring
    one has `{"by": ...}`.

    🔴 NOT EVERY FIELD WITH `kind == "sel"` GETS GROUNDED, AND THIS WAS
    BOUGHT IN RED (30.08.2026). The first version asked only
    `kind == "sel"` and dropped the "curtain wall" and "curtain wall junior"
    course recipes on ALL six versions: `set_curtain_panel.panel_type` is
    declared a selector, but its `ref_kinds` is EMPTY — there is nothing to
    ground it with, and genuine grounding legitimately leaves it as
    authoring. My refusal was firing on legitimate input.

    What discriminates is the REGISTRY, the same flag the grounder itself
    uses (`ground.py`, `_how_to_get_one`: "Discriminates not by my own list,
    but by the REGISTRY: whether the parameter's `ref_kinds`
    intersects…"): a field is groundable if and only if its `ref_kinds` is
    NON-EMPTY. No second carrier of this knowledge is set up here.

    A missing field is legitimate (an optional selector), and here it is
    not a finding: grounding is not required to insert what the author
    never wrote.
    """
    from kir import spec

    op = spec.OPS.get(op_name)
    if op is None:                      # a foreign op is not our subject
        return []

    def авторский(value: Any) -> bool:
        if isinstance(value, list):
            return any(авторский(item) for item in value)
        return isinstance(value, dict) and "by" in value

    return sorted(
        pspec.name for pspec in op.params
        if pspec.kind == "sel" and pspec.ref_kinds
        and (op_name, pspec.name) not in UNGROUNDED_PLACES
        and авторский(payload.get(pspec.name))
    )
...
def _recomputed_derived_artifact(
    ...
    from kir import connect, contour, route_mep, spec
    ...
    if key == "__region__" or (key.startswith("__region_")
                               and key.endswith("__")):
        region_fields = [
            param.name for param in op_spec.params
            if param.kind == "region" and param.name in planned
        ]
        # 🔴 AN OP WITH TWO PROFILES WAS UNCALLABLE, AND THE CONTRACT STAYED
        # SILENT ABOUT IT. Measured 20.08.2026: `create_solid_blend` sits in
        # the registry, in the certificate table, and in the emitter — and
        # ANY authoring program using it died with `KIR-P000` on all six
        # versions. The cause: the derived-field declaration named EXACTLY
        # ONE `__region__`, while an op with a bottom and a top has two.
        # Grounding named the profiles by name (`__region_profile__`), the
        # contract did not know that name, and rejected it as "undeclared
        # lowering field".
        #
        # WHY IT IS FIXED RIGHT HERE, NOT IN GROUNDING. The derived-field
        # contract is the authority on WHAT grounding is allowed to add.
        # Bypassing it with a key that skips the check would mean removing
        # the only defense against "a field appeared out of nowhere". So
        # what gets expanded is the DECLARATION: for an op with N profiles,
        # EXACTLY the names of its region parameters are legal.
        #
        # A BOUNDARY HELD DELIBERATELY: for an op with ONE region, only the
        # shared `__region__` is legal — a named key there is still
        # illegal. Otherwise the same value would get two legal addresses,
        # and the next wave would pick either one at random (the class "a
        # value's kind lives in four places").
        if key == "__region__":
            if len(region_fields) != 1:
                return _NO_DERIVED_ARTIFACT
            field = region_fields[0]
        else:
            if len(region_fields) < 2:
                return _NO_DERIVED_ARTIFACT
            field = key[len("__region_"):-len("__")]
...
def _assert_payload_refinement(
    ...
    *,
    ...
    # REQUIREDNESS IS TRACKED BY THE NUMBER OF REGIONS — A SECOND CARRIER OF
    # THE SAME KNOWLEDGE. The first (`_recomputed_derived_artifact`) says
    # WHAT is legal to add; this one says what is REQUIRED to be present. On
    # 20.08 they were expanded one at a time, and fixing the first gave not
    # a green, but the NEXT refusal: the named key was already accepted,
    # while the shared one was still required. Both lists are required to
    # read the same rule, otherwise an op with two profiles is unexpressible
    # either way — the refusal just moves down a line.
    region_fields = [param.name for param in op_spec.params
                     if param.kind == "region" and param.name in planned]
    if len(region_fields) == 1:
        required_derived.add("__region__")
    elif len(region_fields) > 1:
        required_derived.update(f"__region_{name}__" for name in region_fields)
    ...
    for key in sorted(added):
        ...
        if (expected_derived is not _NO_DERIVED_ARTIFACT
                and _canonical_json(value) == _canonical_json(
                    expected_derived)):
            continue
        # A field's owners are asked of the AUTHORITY
        # (registry_base.SYNTHETIC_FIELDS), rather than carried as a pair
        # alongside the check: the pair already drifted apart once — the
        # op's parsing knew nothing about the field, and the group with the
        # door would not compile.
        if op_name in SYNTHETIC_FIELDS.get(key, frozenset()):
            ...
            if isinstance(host_op, dict) and {"p0_mm", "p1_mm"} <= set(host_op):
                # The host's shape is taken IN ITS POSITION AT THE TIME OF
                # THIS OP: a `move_elements` shift by the same program
                # BEFORE the door is folded into the expectation
                # (07.09.2026, F1). The control recomputes the law itself
                # (`geom.same_program_shift`), rather than trusting the
                # plan.
                from kir.geom import same_program_shift, shifted_host_shape
                ordered = list(grounded_by_id.values())
                upto = next((i for i, o in enumerate(ordered)
                             if o.get("id") == planned.get("id")), len(ordered))
                shift = same_program_shift(ordered, upto, str(host_id))
                expected = shifted_host_shape(host_op, shift)
                if value == expected:
                    continue
...
class GroundedProgram:
    ...
    def __post_init__(self, verification_snapshot: Any) -> None:
        ...
        # 🔴 A PROGRAM PASSED BACK TO ITSELF IN THE ROLE OF GROUNDED IS NOT A
        # GROUNDED PROGRAM (F-103). The check above answers the question
        # "did anything that changed drift apart"; this one asks "was what
        # was REQUIRED to be resolved actually resolved". Without the
        # second question, zero work read as zero discrepancies, and
        # `derived_artifacts_verified` got declared true over a program
        # that no one had ever grounded.
        #
        # The refusal cannot fire on legitimate input, and this is
        # MEASURED, not merely declared: genuine grounding (`ground.py`)
        # leaves not a single authoring selector behind — verified by
        # compiling with a snapshot resolver.
        _not_lowered = [
            (op.op_id, field)
            for op in self.ops
            for field in _authored_selectors_left(op.op_name, op.to_dict())
        ]
        if _not_lowered:
            _shown = ", ".join(f"{o}.{f}" for o, f in _not_lowered[:5])
            _rest = (f" и ещё {len(_not_lowered) - 5}"
                     if len(_not_lowered) > 5 else "")
            raise ValueError(
                "заземление не состоялось: поля-селекторы остались авторскими "
                f"({_shown}{_rest}). Программа, переданная самой себе в роли "
                "заземлённой, — не заземлённая программа")
    ...
    @property
    def selector_resolutions(self) -> int:
        """How many selectors grounding ACTUALLY resolved.

        🔴 ONE FIELD CANNOT CARRY TWO FACTS (30.08.2026, F-103).
        `selector_resolution_replayed` is true EXACTLY WHEN there ARE no
        resolutions — and that is a deliberate caution, explained in its
        own place: without actually replaying the snapshot resolver,
        claiming "replayed" would say more than the content digest proves.
        But a reader sees the name "replayed" and understands it backward.

        So the flag's polarity is NOT TOUCHED (that would tell a lie in the
        other direction), and a NUMBER is placed alongside it instead:
        "there was nothing to replay" and "there was something to replay,
        and we didn't do it" stop looking identical.

        The property is COMPUTED and is not included in `_unsigned_evidence`:
        the list of resolutions is already there, and an extra field would
        shift the `ground_digest` of every golden for the sake of a number
        that is already derivable from the evidence.
        """
