...
def _dispatch_attempt(value: str) -> str:
    if not isinstance(value, str) or not value.strip() or len(value) > 128:
        raise ValueError("dispatch attempt_id must be 1..128 characters")
    return value
...
class InMemoryOperationStore:
    ...
    async def claim_dispatch(
        ...
        *,
        ...
        """Atomically reserve the only server-to-Bridge dispatch attempt."""

        attempt_id = _dispatch_attempt(attempt_id)
        async with self._lock:
            record = self._records.get(operation_id)
            if record is None:
                raise KeyError(operation_id)
            if record.phase not in {
                ...
                return None
            record.phase = OperationPhase.DISPATCH_CLAIMED
            record.attempt_id = attempt_id
            record.updated_at = datetime.now(timezone.utc).isoformat()
            return record
...
class DatabaseOperationStore:
    ...
    async def claim_dispatch(
        ...
        *,
        ...
        attempt_id = _dispatch_attempt(attempt_id)
        row = await self._database.claim_operation_dispatch(
