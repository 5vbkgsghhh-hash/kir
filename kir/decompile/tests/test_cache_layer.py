...
* :class:`kir.compile_cache.CachedCompileClient` — verified against a
  fake compile client so the live Roslyn service (:52412) is never touched.
* :func:`kir.decompile.lift_cache.cached_lift_document_detailed` — verified
...
from kir.compile_client import CompileError, CompileResult
from kir.compile_cache import (
    CachedCompileClient,
    compile_cache_key,
)
from kir.decompile import lift_cache
...
class CanonicalJsonAgreement(unittest.TestCase):
    ...
    _MODULES = (
        "kir.midend",
        "kir.decompile.journal",
        "kir.decompile.lift_cache",
        "kir.decompile.geometry_acceptance",
        "kir.decompile.passport",
        "kir.decompile.merkle",
        "kir.acceptance_live",
        "kir.acceptance_mutation",
    )
...
class NonFiniteMakesTheLiftEntryUncacheable(unittest.TestCase):
    ...
    def test_index_with_a_non_finite_value_keys_to_a_miss(self) -> None:
        from kir.decompile.lift_cache import _index_hash
