...
from kir.decompile.fold import FidelityCanon, iter_l1_leaves
...
class AtomEscrowFormAcceptanceTests(unittest.TestCase):
    ...
    def _source_geometry(source_id: str = "9999"):
        from kir.decompile.geom_extract import extract_geometry
    ...
    def test_the_journal_says_the_comparison_volume_was_zero(self):
        """🔴 "COMPARED AND MATCHED" VERSUS "THERE WAS NOTHING TO COMPARE"
        (E-43).

        An atomic run has an empty multiset of ops BY CONSTRUCTION, and
        `_compare` honestly returns `multiset_match=True`: empty equals
        empty. But 100% agreement ON NOTHING proves no idempotence at all,
        while the A5 journal reader was seeing exactly the same pair of
        fields as a genuinely converged run.

        🔴 WHY NOT `comparison_performed=False`, AND THIS WAS PAID FOR IN RED:
        the first revision of the fix did exactly that, and the test above
        was failing — `a5_recovery` rejects a COMPARED proof with no
        comparison ("Compared proof has no comparison"), meaning an honest
        atomic run would collapse entirely. The volume is named as a
        SEPARATE field.
        """
        captured: dict = {}
        ...
        report, _ = self._run_case(recovery=_Spy())
        self.assertTrue(captured, "павлоад журнала не снят — тест пуст")
        self.assertIs(captured["comparison_volume_zero"], True)
        # NARROWNESS CONTROL: the comparison DID still TAKE PLACE (otherwise
        # `a5_recovery` will reject the proof), and the verdict stayed the
        # same.
        self.assertIs(captured["comparison_performed"], True)
        self.assertIs(captured["multiset_match"], True)
    ...
    def test_exact_post_commit_surface_raises_coverage(self):
        ...
        # 🔴 AND EXACTLY WHY THE VOLUME MUST BE NAMED (30.08.2026, E-43).
        # The three lines above themselves say "the multiset is empty" and in
        # the same breath assert "it matched". Both statements are true, and
        # together they read as a green verdict about something that was
        # never measured. This locks in that the run REPORTS the volume's
        # emptiness rather than leaving it to be inferred from the numbers.
        self.assertEqual(report.total_expected, 0)
        self.assertEqual(report.total_actual, 0)
...
class CurtainIndexRelift(unittest.TestCase):
    """The tail of the panel wave (aaa44b45, 28.07): ``run_idempotence``
    re-lifts the Δ-copy WITHOUT the curtain index — ``profile_index``/
    ``family_placement_index``/``wall_curve_index`` already reach the relift
    (see the ``curve_index`` block a bit above in the file), ``curtain_index``
    does not. A panel that became ``set_curtain_panel`` on the first lift gets
    atomized again on the relift — a FALSE idempotence discrepancy, not a real
    one.

    ``extract_curtain_topology``/``build_curtain_extract_cs`` are themselves
    already covered by ``test_curtain_extract.py``/
    ``test_lift_curtain_panel.py``; this test patches exactly the
    builder/parser BOUNDARY, to check the WIRING (read_executor ->
    lift_document) in ``kir/idempotence.py`` — not to reinvent curtain wall
    parsing, and not to depend on ``authoring.py``
    (``curtain_cell_address_cs``), which a parallel wave is fixing right now.
    """
    ...
    def _run_report(self):
        ...
        with mock.patch(
                "kir.decompile.curtain_extract.build_curtain_extract_cs",
                side_effect=lambda ids: (
                    self._MARKER + ",".join(str(i) for i in ids))), \
             mock.patch(
                "kir.decompile.curtain_extract.extract_curtain_topology",
...
class HostedOpWhoseHostIsNotRebuilt(unittest.TestCase):
    """Observed on 27.07 on SOB6.2: `create_door`, whose host is a wall with
    the `core_interior` location line, stayed an atom ("not expressible as
    create_wall"). The door lifted, the wall did not — and the A5 corpus
    carried off a dangling reference. FidelityCanon refused with "fidelity
    ref target … is absent from graph", while what reached the operator was a
    bare "internal".

    The same law that materialize.leaves_to_program already applies with its
    typed skip host_unmaterialized: an excluded host is data, not an
    exception. A reference to a discarded node discards the referrer too,
    down to a fixed point."""
    ...
    def test_chain_is_resolved_to_a_fixed_point(self):
        """The discard propagates: a tag on a door, a door on an atom-wall."""
        wall_atom = {"kind": "atom", "_id": "W", "category": "OST_Walls",
                     "category_ru": "Стены", "type_name": "T",
                     "bbox_min_mm": None, "bbox_max_mm": None,
                     "source_element_id": "10", "level_name": None,
...
class SafetyGuardTests(unittest.TestCase):
    ...
    def test_operator_declaration_is_an_alternative_proof_only_with_the_token(self):
        """A second route of proof, added 27.07 by the operator's decision.

        The naming convention is a heuristic, and it failed on a file the
        operator holds as a disposable copy. A spoken "this is a copy" cannot
        serve as proof: the rail exists for exactly this reason — so that
        nobody's word, mine included, puts a thousand elements into the wrong
        file. So an explicit declaration only counts TOGETHER with the secret
        confirm_token: two independent factors instead of one, as before."""
    ...
    def test_declaration_defaults_off_so_nothing_changes_without_it(self):
        """The flag is off by default: the previous refusal on the working
        name is intact."""
        safety = K.SafetyContext(
    ...
    def test_dry_gate_compiles_against_the_source_catalogue(self):
        """The dry gate must compile WITH the source catalog.

        On 28.07 it was compiling without it and refusing whole chunks with
        KIR-G103 — that is, reporting its own blindness rather than the
        program. The test catches exactly the wiring: the snapshot must reach
        the compiler.
        """
        leaves, doc = self._leaves_doc()
        catalogue = {"wall_types": [{"id": 1, "name": "\u0422\u0438\u043f"}]}
        seen = []
        import kir.compiler as _compiler
