...
# Imported AT THE TOP LEVEL on purpose: the fixture pulls in kir.idempotence,
# which imports the real materialize. An import from inside the test, where
# sys.modules has already been replaced by a stub, catches an ImportError out of nowhere.
from kir.decompile.tests.test_serving_idempotence import (  # noqa: E402
    _persist_decompile,
)
...
_REAL_MATERIALIZE_MODULE = sys.modules["kir.decompile.materialize"]
...
#: The suite's device. `serving.ADMIN_DEVICE` is NOT a constant: it is derived from
#: `KUKAI_ADMIN_DEVICES` at the moment of the call and equals `None` when the list is empty
#: (the fallback was removed on 15.08.2026: "we have no right to guess someone else's device"). A suite that
#: opened the MODE and did not open the LIST mocked `_turn_device_id` with
#: `None` — the gate refused, and the refusal blamed the mode, which was in fact enabled.
_НАБОРНОЕ_УСТРОЙСТВО = "dev-набор"


def _открыть_ворота(case, *, режим: str = "stage2") -> str:
    """Open BOTH gate conditions and register the rollback in cleanup. Returns the device.

    `addCleanup`, not `tearDown`: it fires even when `setUp` did not
    reach the end — a shape bought on 28.08 by fifteen errors of "the test
    left the process environment behind."
    """
    for имя, значение in (("KUKAI_KIR_DECOMPILE", режим),
                          ("KUKAI_ADMIN_DEVICES", _НАБОРНОЕ_УСТРОЙСТВО)):
        было = os.environ.get(имя)
        case.addCleanup(
            (lambda н, з: (os.environ.__setitem__(н, з) if з is not None
                           else os.environ.pop(н, None))), имя, было)
        os.environ[имя] = значение
    return _НАБОРНОЕ_УСТРОЙСТВО
...
class DecompileGate(unittest.TestCase):
    ...
    def test_stage2_admin_visible(self):
        устройство = _открыть_ворота(self)
        with mock.patch.object(serving, "_turn_device_id",
                               return_value=устройство):
...
class DecompileDispatch(unittest.TestCase):
    #: 🔴 THE ALLOW-LIST IS SET EXPLICITLY (28.08.2026), AND WITHOUT IT THE TEST HUNG.
    #:
    #: `serving.ADMIN_DEVICE` is not a constant: it is derived from
    #: `KUKAI_ADMIN_DEVICES` at the moment of the call and equals `None` when the list
    #: is empty (the fallback was removed on 15.08.2026: "we have no right to
    #: guess someone else's device"). This `setUp` opened the MODE and did not open the LIST, so
    #: `_turn_device_id` returned `None`, the gate refused, `_slow_run` was not
    #: called — and `await started.wait()` waited for an event that would never
    #: happen. The whole decompile suite stalled on this one test.
    def setUp(self):
        ...
        self.addCleanup(serving._active_run.clear)
    ...
    def test_double_start_is_refused(self):
        ...
        async def _drive(out_dir):
            # The run directory is substituted on purpose (08.09.2026): the observer
            # `_record_dead_decompile_run` names the cancelled run in
            # `status.json`, and without the substitution the file would land in the
            # default tree (`backend/data/decompile/docA` off the current directory).
            with mock.patch(
                    "kir.decompile.pipeline.run_decompile", _slow_run), \
                ...
                # 🔴 THE WAIT IS BOUNDED (28.08.2026), AND THIS IS NOT CAUTION.
                #
                # `started` is set only by `_slow_run`, and it is called only if
                # the FIRST start passed the gate. If it did not pass, the event NEVER
                # happens, and `wait()` hangs forever. Measurement 28.08: on this test
                # the whole decompile suite stalled dead — 1830 tests out of 2182
                # were run, the summary was not printed at all, and `tail` brought back
                # a progress line that reads as "no reds."
                #
                # An infinite wait is the worst kind of failure: it says
                # nothing and takes away its neighbors' right to be measured. The bound
                # turns it into a NAMED refusal that carries the reason
                # of the first start.
                try:
                    await asyncio.wait_for(started.wait(), timeout=10)
                except asyncio.TimeoutError:                  # pragma: no cover
                    raise AssertionError(
                        "первый старт не дошёл до работы за 10 с — второй "
                        "старт проверять не на чем. Ответ первого: "
                        f"{first}") from None
        ...
        with tempfile.TemporaryDirectory() as tmp:
            first, second = _run(_drive(tmp))
            from kir.decompile import pipeline as pipe
            named = pipe.read_status(tmp)
        self.assertTrue(first["ok"])
        self.assertTrue(first["started"])
        self.assertFalse(second["ok"])
        self.assertEqual(second["error"], "run_in_progress")
        # The cancelled run is named in the run directory, not in the tree.
        self.assertEqual(named["stage"], "cancelled")
        self.assertFalse(os.path.exists(
            os.path.join(os.getcwd(), "backend", "data", "decompile")))
...
class RebuildInstrument(unittest.TestCase):
    def setUp(self):
        ...
        self._dev = mock.patch.object(
            serving, "_turn_device_id", return_value=устройство)
        self._dev.start()

    def tearDown(self):
        self._dev.stop()
        os.environ.pop("KUKAI_IR_ATOM_ESCROW", None)
        sys.modules["kir.decompile.materialize"] = (
            _REAL_MATERIALIZE_MODULE)

    def test_materializer_pending_when_module_absent(self):
        # The A1×A3 integration seam: the materialize module NOW exists
        # (wave A3 has been merged), so "absence" is simulated explicitly —
        # writing None into sys.modules forces the import to raise ImportError
        # (the old sys.modules.pop relied on the file's actual absence and
        # silently stopped testing this branch after the A3 merge).
        with mock.patch.dict(
                sys.modules, {"kir.decompile.materialize": None}):
    ...
    def test_dry_run_compiles_chunks_with_mocked_materializer(self):
        ...
        from kir.decompile.tests.test_materialize import _op_leaf

        fake = types.ModuleType("kir.decompile.materialize")
        leaves = [
        ...
        sys.modules["kir.decompile.materialize"] = fake
    ...
    def test_dry_run_reuses_materializer_plan_and_reports_its_digest(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                mock.patch.object(serving, "_decompile_out_dir",
                                  return_value=tmp),
                mock.patch("kir.compiler.compile_rebuild_chunk",
    ...
    def test_atom_residual_is_executable_but_never_claims_full_fidelity(self):
        ...
        """
        from kir.decompile.tests.test_materialize import (
            _atom_leaf,
            _op_leaf,
        )
    ...
    def test_enabled_atom_escrow_reaches_dry_compile_with_pending_evidence(
        ...
        from kir.decompile.tests.test_geom_extract import (
            _element as geometry_element,
            _part as geometry_part,
            _payload as geometry_payload,
            _triangle_mesh,
        )
        from kir.decompile.tests.test_materialize import _atom_leaf
    ...
    # ── §18.4, tail of the wave: the partial-read gate stood ONLY on A5 ──────
    #
    # A rebuild from a decompile made with closed worksets builds
    # PART of the building and says nothing about it. The refusal is exactly the same as A5's, including
    # the named allow_partial carve-out — otherwise the law of contamination applies to
    # the instrument that only MEASURES, and does not apply to the one that WRITES.
...
class ScopeLeavesHostClosure(unittest.TestCase):
    """A pin for host closure of the A5 scope (live bug #14, 2026-07-21).

    A door in the scope must pull in its host wall (otherwise materialize
    fails closed with 'has no materialized host op'); hosts are not counted
    in limit_ops; foreign walls are not pulled in; the order of leaves is preserved."""
    ...
    def test_door_scope_pulls_host_wall_outside_cap(self):
        from kir.serving import _scope_leaves
        scoped = _scope_leaves(self._leaves(), limit_ops=2,
                               only_kinds=["create_door"])
        ids = [leaf["_id"] for leaf in scoped]
        # w2 is foreign.  The atom REMAINS (arch analysis 2026-07-25 §3.6): it is not
        # rebuilt, but it must be in the denominator of the metric.
    ...
    def test_cap_counts_targets_not_hosts(self):
        ...
        scoped = _scope_leaves(self._leaves(), limit_ops=1,
                               only_kinds=["create_door"])
        ids = [leaf["_id"] for leaf in scoped]
        # The cap counts target ops; neither the host nor the atom counts toward the cap.
        self.assertEqual(ids, ["w1", "d1", "a1"])

    @staticmethod
    def _cross_level_leaves():
        # A tall wall on Floor -3 hosts doors on -3/-2/-1 (live bug #17).
        return [
    ...
    def test_level_scope_pulls_cross_level_host(self):
        # The "Floor -2" scope must pull in the host wall from "Floor -3" — otherwise
        # materialize fails (live parking failure -2/-1: 0/93, 0/119).
        from kir.serving import _scope_leaves
    ...
    def test_level_scope_runs_closure_without_limit_or_kinds(self):
        # A plain level_scope (without limit/only_kinds) ALSO triggers closure
        # (previously an early return skipped it — floor 20 got lucky with its own hosts).
        from kir.serving import _scope_leaves
        scoped = _scope_leaves(self._cross_level_leaves(),
                               level_scope="Этаж -3")
        ids = sorted(leaf["_id"] for leaf in scoped)
    ...
    def test_wall_scope_pulls_nothing_extra(self):
        ...
        # Foreign doors are not pulled in; the atom remains — it has no op_name, and
        # discarding it by kind would mean hiding what was not lifted (§3.6).
        self.assertEqual([leaf["_id"] for leaf in scoped],
                         ["w1", "w2", "a1"])

    def test_atom_escrow_scope_is_stable_and_bounded_by_limit(self):
        from kir.serving import _atom_escrow_source_ids_for_scope
