...
from kir.decompile.schema import CANON_MM, L0Document
from kir.decompile.tests.fixtures_decompile import (
    make_element,
    project1_metadata,
)
from kir.contour import bulge_midpoint, radius_to_bulge
...
class DegenerateAndAtomScaleTests(unittest.TestCase):
    def test_rooms_empty_model_is_valid_and_zoned_by_discipline(self) -> None:
        ...
        # 07-28: the discipline is read from the extractor's table, not
        # from fold.py's own dictionary. Furniture there is marked
        # architectural — the earlier "unknown" was not a property of
        # the document, but a hole in the dictionary, which knew 17
        # categories out of 47.
...
class CanonIdentityIsDefiningDOF(unittest.TestCase):
    """Canon refinement (live A5 evidence 2026-07-21): the identity of
    an OP-leaf = the defining DOFs (op_name+type_name+params); anchor_mm
    is a derived quantity from raw Revit geometry with float noise
    (10/40 live walls "missed" the canon by a 0.5mm anchor drift despite
    exact p0/p1) and does NOT enter the op's identity. An ATOM has no
    params — its anchor remains its identity."""
    ...
    def test_canon_contract_versions_are_explicit(self) -> None:
        # The guard is deliberately pinned: a canon change must be a
        # decision, not a side effect. /2 → /3 (2026-07-26) —
        # dimensional grids: arc axes and radians stopped being rounded
        # on the millimeter grid (two different arced walls gave ONE
        # canon), likewise dimensionless scalars; and the version itself
        # entered canon_hash, so that the next such change would not
        # silently redefine the meaning of already-saved hashes.
        # /3 → /4 (2026-07-28) — the zone-group discipline: the layout
        # now takes it from the extractor's table instead of its own
        # 17-category dictionary, the lexicon changed entirely
        # ("architecture"→"architectural", "coordination"→"shared"),
        # and the macro discipline group enters the subtree's canon —
        # meaning both the layout AND the tree hash change for the same elements.
        ...
        #
        # fidelity-canon/4 (2026-07-29, rebuild #11): arced walls. A —
        # the 2π branch is resolved at the ARC level by a paired shift
        # of angles (_canonical_arc_branch): atan2 on reverse-read
        # returns (-pi, pi], while decompilation stored the neighboring
        # branch — the same arc, a different hash (4 of 5 live
        # discrepancies in v18). B — the endpoints of an arced wall are
        # canonicalized FROM its own arc
        # (_fidelity_arc_endpoints_from_arc): the stored p0/p1 diverged
        # from the arc of the same leaf by 0.37-0.94 mm and crossed the
        # CANON_MM grid (2 of 5). No tolerance was widened.
    ...
    def test_ring_canon_is_cyclic_invariant(self) -> None:
        # Canon refinement #2 (live floor evidence 2026-07-21): Revit
        # rebuilds the sketch and hands back the loop starting from ITS
        # OWN vertex — the ring's rotation/direction and the order of
        # hole rings are Revit-incidental, not defining DOFs.
        base = {
        ...
        # the same geometry: the outline is rotated by 2 vertices AND
        # reversed, the holes are reordered and rotated
        ...
        # the defining difference is caught: a shifted vertex = a different floor
    ...
    def test_rotation_canon_is_periodic(self) -> None:
        # Canon refinement #3 (live mezzanine furniture 2026-07-21): a
        # flip composition (hand-flip + pre-rotation) leaves an instance
        # at 360° where the original's lift gave 0° — geometrically the
        # same spot, but the raw canon counted 360.0 ≠ 0.0 (3 of 5
        # furniture "misses" were precisely this 360-wrap).
        op = {
...
class ContourFloorCanonTests(unittest.TestCase):
    """The group_by wave is no longer relevant here — this is the 07-28
    canon wave: the first live contour floor (rebuild #7, v12,
    backend/data/decompile/sob62_fas_r23_v12/idempotence_debug.json)
    built, but the canon diverged — extra_rebuilt (source_element_id
    11438487) against expected (9981227), the same leaf.

    Measured: (1) BULGE QUANTIZATION — a dimensional, dimensionless
    scalar today is rounded on a 1e-9 grid (``_FIDELITY_SCALAR_STEP``),
    and the discrepancy between "the original lift's raw floats" and
    "geometry rounded to 0.01mm after a round trip through emission"
    lands in the 6th-7th digit of bulge (~1.3e-6 on a chord of
    ~3925mm) — a grid 3 orders of magnitude finer than the noise itself
    never converges; (2) RING ROTATION — ``_canonical_ring`` in this
    file already knows cyclic start+orientation, but contour rings sit
    one level deeper than it looks (``{"shape","points_mm","arcs"}``,
    not a bare list of points directly under the ``outline``/``holes``
    key) — for create_floor_by_contour it never fires.

    Both numbers are taken VERBATIM from the dump (not invented)."""

    _ORIGIN = (0.0, 0.0, 0.0)

    # Verbatim from idempotence_debug.json, leaf source_element_id=9981227
    # (expected) / 11438487 (relifted, extra_rebuilt) — only geometry
    # and defining DOFs, L1Node requires no extraneous keys.
    _EXPECTED = {
        ...
        "params": {
            ...
            "contour": {
                ...
                "holes": [{
                    ...
                    # RING ROTATION, live: the same arc (the same chord,
                    # the same bulge sign, noise in the 6th digit), but
                    # edge 1->0 — the relift reads the loop starting
                    # from a DIFFERENT first node.
    ...
    def test_v12_extra_rebuilt_sheet_converges_after_canon_fix(self) -> None:
        """A refuting test: today's typed mismatch of leaf 9981227
        (expected) / 11438487 (relifted) — verbatim the same numbers as
        in the live failure of rebuild #7. The canon of both sides must
        converge."""
        self.assertEqual(
    ...
    def test_negative_control_genuinely_different_contour_still_diverges(
            self) -> None:
        """Negative control: move a real hole vertex by 50mm
        (deliberately more than any quantization tolerance) — the canon
        must remain DIFFERENT. A canon that always converges is not a canon."""
    ...
    def test_negative_control_genuinely_different_bulge_still_diverges(
            self) -> None:
        """Negative control: the same chord, but a noticeably different
        curvature (bulge shifted by 0.05, two orders of magnitude
        larger than the grid on this ~3925mm chord) — the canon must
        remain DIFFERENT."""
        moved = copy.deepcopy(self._RELIFTED)
...
class Codex20260729BulgeMidpointCanon(unittest.TestCase):
    """Codex review tasks/b8f3v4r97.output (session eeccfb91), seams
    #5/#6/#7 — all three on fidelity-canon/2 (contour ring canon),
    closed together by fidelity-canon/3: the arc's canon became the
    physical mid-sweep point rather than a separately quantized bulge,
    so the bulge and radius forms of one arc converge BY CONSTRUCTION,
    not by a second normalization."""
    ...
    def test_codex5_reported_example_now_diverges(self) -> None:
        """The review's verbatim example: a 4000mm chord, bulge=0.09951
        and 0.10049 — /2 gave ONE hash, even though the arc midpoints
        differ by 1.96mm (> CANON_MM=1mm). /3 must tell them apart."""
    ...
    def test_codex5_property_same_hash_implies_close_midpoints(self) -> None:
        """Property (codex wording): for ANY two arcs that converge to
        the same hash, the mid-sweep points must be CLOSER than
        CANON_MM on every coordinate. The whole range |b|<=1.5 and
        several chord lengths, including >4km (the /2 lower clamp
        "inflated the tolerance" precisely there)."""
    ...
    def test_codex6_radius_form_survives_rotation_and_reflection(self) -> None:
        """Not just head-on — after dihedral normalization (a ring
        started from a different vertex), the radius and bulge forms of
        ONE arc must remain one hash; edge reindexing is shared between
        both forms."""
        pts = [[0.0, 0.0], [4000.0, 0.0], [4000.0, 4000.0], [0.0, 4000.0]]
        b = radius_to_bulge(pts[0], pts[1], 3000.0, True, "x", "x", [])
    ...
    def test_codex6_radius_shorter_than_half_chord_fails_open(self) -> None:
        """A geometrically impossible radius-arc (radius < half the
        chord) must not crash the canon — the same fail-open as for any
        other unrecognized ring shape (a raw ring, points_mm/arcs as-is)."""
        pts = [[0.0, 0.0], [4000.0, 0.0], [4000.0, 4000.0], [0.0, 4000.0]]
    ...
    def test_codex7_major_arc_reflection_sign_property(self) -> None:
        """1<|b|<=1.5, every edge of the square, both signs: a reflected
        loop must converge with the CORRECT sign (-b) and diverge with
        the unchanged one (+b) — a negative control in the same loop,
        not separately."""
...
class Codex20260729HeightOffsetAbsentIsZero(unittest.TestCase):
    """Codex #8 (canon half — lift.py's <1mm discard stays with curtain
    wall within this wave's boundary): absent height_offset_mm must
    canonicalize as explicit 0.0 on BOTH floor branches."""
    ...
    def test_roundtrip_minus_0999_to_0999_matches_round_mm_oracle(self) -> None:
        """Codex range −0.999…0.999: any explicit value canonicalizes
        EXACTLY as absent if and only if _round_mm of the same quantity
        gives 0.0 — that is, one and the same grid for explicit and
        absent, and no explicit value is swallowed beyond this shared rule."""
