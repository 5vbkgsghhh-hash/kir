...
Convention follows ``kir/tests/test_pbt.py``: hypothesis is not in the
...
class MerkleHashProperties(unittest.TestCase):
    ...
    def test_node_origin_is_not_bbox_min_where_they_must_differ(self) -> None:
        # THIS TEST USED TO BE NAMED `..._is_rounded_bbox_min` AND
        # ASSERTED EQUALITY WITH EXACTLY WHAT `node_origin`'s
        # DOCSTRING CALLS UNFIT. It passed not by contract but by
        # fixture coincidence: a single floor at elevation 0, where
        # both values are zero. Measured 2026-08-09 on three floors:
        # for subtrees at 3000 and 6000 mm `facts.bbox_min_mm` ==
        # (0,0,0) — fold zeroes the z of 2D parameter points — while
        # `node_origin` gives (0,0,3000) and (0,0,6000). If someone
        # "fixes" `node_origin` to `bbox_min`, the old test would
        # turn green again, and localizing content relative to the
        # origin would stop being invariant under a vertical shift:
        # every lifted floor would collapse onto world zero and stop
        # matching hashes with the same floor at a different
        # elevation.
    ...
    def test_node_origin_equals_bbox_min_only_on_the_ground_floor(self) -> None:
        # The surviving half of the old assertion — but named for what it
        # actually is: a coincidence at elevation 0, not a definition of the
        # value.
