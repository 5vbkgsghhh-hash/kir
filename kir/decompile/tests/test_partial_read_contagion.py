"""§18.4 — the law of partial-read contamination. Refuting tests (§18.7 item 2).

The measurement that gave rise to the law: the EOM document was opened with 17 of 18
worksets closed — the collectors honestly returned 11 elements instead of 2016,
all category statuses `complete`, the passport reported high coverage. The C#
side MEASURES the worksets (`build_metadata_cs`), `_parse_metadata` PARSES them,
`L0Document.is_partial_read` READS them — but the L0 header was losing them:
`metadata_dict()` did not write them, `_new_metadata` and `L0JSONLReader.materialize()` did not
pass them along. No derived artifact could say that a partial model had been read.

At the time of writing, the following failed:
  * header round-trip (write→read) — the fields did not survive the write;
  * materialize — lost the fields even when they were present in the header;
  * pipeline — neither run.json, nor status.json, nor the passport carried the flag.
"""
...
class L0HeaderRoundTrip(unittest.TestCase):
    """The L0 header must carry the workset state."""
...
class PipelineRaisesTheSignal(unittest.TestCase):
    """run.json / status.json / the passport carry the partial-read flag."""
    ...
    def test_partial_read_reaches_run_json_status_and_passport(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            # §18.4: the percentage is printed only together with the flag.
