...
from kir.decompile.schema import SECTION_RECEIPT_OUTCOMES
...
CATEGORY_RU = {
    ...
    # Sections other than AR (architecture) were added together with the
    # extractor's categories on 27.07.
    ...
    "OST_Areas": "Зоны",
    # `OST_CurtaSystem` was appended to the tuple's tail in extract.py (the
    # tail of wave aaa44b45, 28.07) — the third kind of curtain-grid host.
    "OST_CurtaSystem": "Витражные системы",
    # Curtain-panel cut lines (wave 29.07): the category name is taken from
    # the CENSUS of live model v14 — «Схемы разрезки витражей», 122
    # elements, exactly as many as the lines in the same run's curtain
    # index.
    ...
    # R4 red: insulation and lining are separate bodies with their own dimension.
    ...
    # Working documentation — dimensions, tags, notes (wave 29.07). The
    # names are NOT invented: they are the localized labels from the census
    # of snapshot k2_ar_rd_v6 (13A-RD-AR-K2_v33), that is, exactly what
    # Revit itself calls them in this document. A fixture must not lie any
    ...
    # wave/opening (03.08.2026): openings as separate elements. The
    # localized names are the ones Revit prints in the manager (§18.5: a
    # localized name is only a reference column, `BuiltInCategory` is what
    # keys it regardless).
    ...
    # 22.08.2026: window tags. The name is taken from a real document's
    # census (the L0 header of snapshot `mnvnk_...`, the line
    # `{"key": "OST_WindowTags", "name": "Марки окон", "count": 660}`), not
    # translated by ear.
    "OST_WindowTags": "Марки окон",
    # 22.08.2026: reference planes. The name is taken from a real
    # document's census (the L0 header of snapshot `mnvnk_...`, the line
    # `{"key": "OST_CLines", "name": "Опорные плоскости", "count": 1474}`),
    # not translated by ear — same as window tags in the line above.
    "OST_CLines": "Опорные плоскости",
    # 04.09.2026 — the "silence becomes a refusal" wave, +13.
    "OST_PointLoads": "Точечные нагрузки",
    "OST_LineLoads": "Линейные нагрузки",
    "OST_AreaLoads": "Площадные нагрузки",
    "OST_AreaRein": "Армирование по площади",
    "OST_Topography": "Топоповерхность",
    "OST_BuildingPad": "Площадка",
    "OST_FilledRegion": "Заливка",
    "OST_MaskingRegion": "Маскировка",
    "OST_Cornices": "Карнизы",
    "OST_Reveals": "Ниши",
    "OST_EdgeSlab": "Край плиты",
    "OST_StairsLandings": "Площадки лестниц",
    "OST_PathOfTravelLines": "Пути эвакуации",
}

# Insurance against the next addition: the names table must cover the
# categories table in full. Without this check, a new category would fail
# with a `KeyError` deep inside the fixture, and the cause would read as a
# broken test rather than an unfilled row (which is exactly what happened
# on 27.07 when the sections were added).
...
CURVE_CATEGORIES = {
    ...
    "OST_DuctCurves",
    "OST_CableTray",
    # The electrical (ЭОМ) wave, 09.08: trunking is a linear `MEPCurve`
    # just like a tray, and before the op appeared, its shape played no
    # role in the fixture (an atom is an atom). Now it does: leaving
    # trunking on `bbox_only` would mean baking into the fixture a geometry
    # it never actually has, and the new lifter would be checked against a
    # lie — exactly the same argument as the line about curves below.
    "OST_Conduit",
    # Wave 29.07: lines are LINES. Leaving them on `bbox_only` would mean
    # baking into the fixture a geometry they never actually have, and any
    # downstream curve parsing would be checked against a lie.
    "OST_Lines",
...
POINT_CATEGORIES = {
    ...
    "OST_Furniture",
    "OST_GenericModel",
    # Wave 29.07: both the node element and the telephone device are
    # `FamilyInstance`, that is, `LocationPoint`. Tags, dimensions, and
    # notes are deliberately NOT included here: they have no `Location` at
    # all, live they will give `bbox_only`, and the fixture must show
    # exactly that.
...
def project1_elements(total: int = 350) -> dict[str, list[dict[str, Any]]]:
    ...
    The number of families is NOT hardcoded: the categories table grows
    (27.07: 20 -> 45, when electrical, HVAC, plumbing, and structural
    (ЭОМ/ОВ/ВК/КР) were added to it), and a hardcoded number would turn
    legitimate growth into a fixture failure.
    """
...
class FakeExtractBridge:
    ...
    def __init__(
        ...
        *,
        ...
        # Whose document the fake EXPECTS to see. A probe is recognized by
        # its exact text, and the text depends on the source: a fake that
        # does not know about the bridge would not recognize the bridge's
        # probe and would declare the window dead — meaning the source
        # wiring would stay unchecked exactly where it actually broke in
        # production.
        ...
        self.crashed = False
        # A WINDOW DROP is not the process dying, it is RETRYABLE
        # transport: the operator's network breaks, the bridge dies with
        # 1006, the window comes back with a new `ws_id` within seconds to
        # minutes. `admin_kir.bridge_callback` raises exactly that error
        # when the window is not in the list yet.
    ...
    async def __call__(self, code: str, *, timeout_ms: int) -> dict[str, Any]:
        ...
        result = {
            ...
            # Cross-section receipts (code review #12). A fake must model
            # the CONTRACT, not convenience: every element of a page is
            # asked about by every parameter exactly once, so the sum of
            # the six outcomes equals the page's size. The outcome is
            # chosen by whether a number is actually present in `params` —
            # so a fake cannot add up "by accident".
