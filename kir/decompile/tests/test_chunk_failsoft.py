# -*- coding: utf-8 -*-
"""Chunk-level fail-soft: a failed chunk yields a receipt, not a dead
run.

The fixture is rebuild #10 (v16), journal
``a5_runs/eeff119bd2c34344.state.jsonl``: 28 chunks planned, 10 ran,
9 ``Committed`` (2009 elements), the ninth ``RolledBack``, no comparison
happened at all, and cleanup tore down everything that had been built.

TEST ORDER MATTERS. The first two (``test_01`` / ``test_02``) must be
GREEN EVEN BEFORE THE FIX: they prove that the receipt contract and the
phase machine ALREADY know how to handle a refusal, and that exactly one
place needs to change — the `raise` in ``run_idempotence``. If they turn
red, the fix went to the wrong place: someone went digging in the
contracts instead of the orchestrator.
"""
...
from kir import idempotence as KI


# The ninth chunk's receipt, VERBATIM from live journal #10 (v16).
CHUNK9_RECEIPT = {
    "bridge_detail": (
...
def _valid_rebuilt_proof() -> dict:
    """Proof of the Rebuilt phase: 9 confirmed chunks + 1 refusal."""
    receipts = [
...
class ContractsAlreadyReady(unittest.TestCase):
    """GREEN BEFORE THE FIX — proof that only one place needs to
    change."""

    def test_01_refusal_receipt_is_decided(self):
        """A live refusal receipt — `refused_without_commit` and
        `decided`."""
    ...
    def test_02_rebuilt_proof_accepts_a_refused_chunk(self):
        """The Rebuilt phase's proof is valid with 9 confirmed + 1
        refused."""
...
class ChunkFailSoft(unittest.TestCase):
    ...
    def test_03_denominator_is_untouched(self):
        """The ops of the refused chunk REMAIN in the denominator."""
        ...
        # The refusal is visible in the overall list of discrepancies,
        # not only in its own corner.
    ...
    def test_04_timeout_stays_fatal(self):
        """Unknown is NOT a refusal: fail-soft does not swallow it."""
        ...
        # And exactly one envelope is recognized as a known refusal.
    ...
    def test_05a_refusal_without_revision_cannot_prove_rebuilt(self):
        """A refusal resolves the effect, but without a revision it still
        does not prove the snapshot."""
        proof = {
            "commit_receipts": [
    ...
    def test_05b_unfinished_effect_blocks_the_phase(self):
        """An unclosed write-ahead effect does NOT let the phase advance.

        Exactly the same mine as in 568349f6: a refusal whose effect is
        not closed by a receipt poisons the journal retroactively ("A5
        phase cannot advance with pending effects"). Fail-soft must leave
        it exactly where it is — otherwise the unknown starts pretending
        to be a refusal.

        The phase proofs are taken VERBATIM from live journal #10:
        inventing a SnapshotManifest by hand would mean checking our own
        fiction.
        """
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            # The proof is VALID (the same one accepted in test_02) — the
            # blocker must be specifically the unclosed effect, not the
            # shape of the proof.
    ...
    def test_06_threshold_reads_both_measures(self):
        """The threshold takes the maximum of two shares — by chunks and
        by ops."""
        self.assertEqual(KI.MAX_REFUSED_CHUNK_SHARE, 0.25)
        # Plan v16: 11 large chunks (10×250 + 215) + 17 solo chunks of one
        # op each.
        ...
        # THE LIVE CASE: one large chunk out of 28. The run must make it
        # through.
        ...
        # Three large chunks: by chunks 10.7% (would pass), by ops 27.5%
        # — this specifically catches the by-ops measurement.
        chunk_share, op_share, exceeded = KI.refusal_shares(
            [{"ops": 250}] * 3, plan)
        ...
        # Eight solo chunks of one op each: by ops 0.29% (would pass), by
        # chunks 28.6% — this catches the by-chunks measurement.
        chunk_share, op_share, exceeded = KI.refusal_shares(
            [{"ops": 1}] * 8, plan)
    ...
    def test_07_materializer_stays_host_atomic(self):
        """TRIPWIRE: not a single ref reference crosses a chunk boundary.

        Measured 29.07 on plan v16: 275 references, 0 crossing a
        boundary. A cascade of "orphaned" ops is not written precisely
        because there is nothing to orphan; if the materializer ever
        starts slicing a host group, it must fail HERE, not on live Revit
        forty minutes later.
        """
        plan = [
            {"program_id": "a" * 64, "ops": [
    ...
    def test_08_live_journal_replays_as_valid_rebuilt(self):
        """Live journal #10: 9 confirmed + 1 refused = a valid Rebuilt."""
        if not LIVE_JOURNAL.is_file():
            self.skipTest(f"нет живого журнала: {LIVE_JOURNAL}")
...
def _cross_chunk_refs(plan):
    """(op_id, ref) for every reference whose bearer lies in a DIFFERENT
    chunk."""
    owner = {}
...
# ─── task #25: the refusal of a SINGLE op inside a COMMITTED chunk ────────

class OpRefusalsInsideACommittedChunk(unittest.TestCase):
    """Run #11: the chunk is "Committed," yet 37 ops inside it failed
    silently."""
    ...
    def _report(self, *, refuse_op_ids=(), unaccounted=False):
        ...
        from kir.decompile.tests.test_idempotence import (
        ...
        class Bridge(FakeModelBridge):
            async def rebuild_runner(self, program):
                ...
                if not hit:
                    return envelope
                if unaccounted:
                    # The chunk is committed, but the outcomes do not
                    # reconcile: the ops simply vanished from the output —
                    # exactly the "silently never created" class.
                    for op in hit:
    ...
    def test_09_refusals_are_reported_without_moving_the_denominator(self):
        """The refusal is visible both in the report and in the
        discrepancies; the denominator is NOT touched."""
        ...
        # THE DENOMINATOR DOES NOT MOVE: the op was promised and not
        # built.
        ...
        # The reason, VERBATIM, is also in the overall list of
        # discrepancies.
    ...
    def test_10_an_op_without_an_outcome_kills_the_run(self):
        """AN OP WITHOUT AN OUTCOME is the unknown: fail-soft does NOT
        swallow it.

        The death must be TYPED (`ops_unaccounted`), not "internal": the
        latter would mean a bare contract exception flew out into the
        open — a silent break instead of a resolution. The unknown's
        journal trace is the same as `timeout_unconfirmed`'s — serving
        returns this envelope BEFORE `finish_effect`, so the write-ahead
        effect is left hanging (see test_05b: an unclosed effect does not
        let the phase advance).
        """
        ...
        # And the unknown's envelope is NOT a resolved refusal —
        # otherwise chunk-level fail-soft would keep going on top of an
        # unknown state.
