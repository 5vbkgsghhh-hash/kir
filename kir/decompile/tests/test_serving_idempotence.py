...
from kir.contracts import RevisionProof  # noqa: E402
from kir.open_model import (  # noqa: E402
    OpenModelProfile,
    required_grounding_pools,
)
from kir import idempotence as K  # noqa: E402
...
def _persist_decompile(
    ...
    *,
    ...
    if worksets_closed:
        # §18.4: a decompile made with closed worksets carries the flag
        # in the L0 HEADER — it is from there that the A5 gate takes it.
...
class IdempotenceGate(unittest.TestCase):
    def test_commit_status_requires_one_explicit_non_conflicting_field(self):
        self.assertIsNone(serving._explicit_commit_status({
            "error": True,
            "message": "transaction commit status: RolledBack",
        }))
        self.assertEqual(
            serving._explicit_commit_status({
                "error": True,
                "result": {"commit_status": "RolledBack"},
            }),
            "RolledBack",
        )
        self.assertIsNone(serving._explicit_commit_status({
...
class IdempotenceDispatch(unittest.TestCase):
    def setUp(self):
        # 🔴 BOTH GATE CONDITIONS ARE OPENED (28.08.2026). Only the MODE was being set,
        # but `serving.ADMIN_DEVICE` is not a constant: it is derived from
        # `KUKAI_ADMIN_DEVICES` at the moment of the call and equals `None` when the list
        # is empty (the fallback was removed on 15.08: "we have no right to guess someone else's device").
        # The mock returned `None`, the gate refused, and the refusal blamed the MODE, which was
        # in fact enabled. Thirty reds in this file — from this alone.
    ...
    def test_a5_refuses_partial_read_snapshot(self):
        # §18.4: closed worksets ⇒ the loaded leaves describe PART of
        # the model. The refusal is typed and explains what the operator should do.
        with tempfile.TemporaryDirectory() as tmp:
    ...
    def test_a5_legacy_decompile_without_workset_fields_still_runs(self):
        # A deliberate migration: old decompiles have no such fields — "no data" does not
        # turn into a refusal, otherwise the fix would retroactively break the archive.
        with tempfile.TemporaryDirectory() as tmp:
            _persist_decompile(tmp)
            header = json.loads(
    ...
    def test_sweep_csharp_bytes_are_pinned_to_the_wire_version(self):
        ...
        expected_by_version = {
            ...
            # unguarded variants (kir/gate_runner.py does not cover this
            # generator; verified separately via kir.compile_client).
            "a5-stamp-sweep/3": (
    ...
    def test_open_model_preflight_refuses_before_compile_lease_or_effect(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (mock.patch(
                    "kir.compiler.compile_rebuild_chunk") as compile_mock,
                  mock.patch.object(
    ...
    def test_rebuild_receipt_is_fsynced_before_runner_returns(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            lease.ensure_held = mock.AsyncMock()
            with (mock.patch(
                    "kir.compiler.compile_rebuild_chunk",
    ...
    def test_timeout_after_commit_stays_pending_for_reconciliation(self):
        # 🔴 THE ENVELOPE FOR AN UNKNOWN OUTCOME IS BUILT BY PROD, NOT BY THE LANGUAGE (28.08.2026).
        # The check stands on the `api.bridge_protocol` port: a hand-rolled shape here
        # already failed on 24.08 (a "matched pair" was green for any
        # value of both sides), so the envelope is taken from its host — and without
        # a host there is nowhere to take it from. A `PortMissing` in the middle of the test would read as a
        # finding about reconciling outcomes; a skip NAMES what is missing.
        from kir import ports as _п

        try:
            _п.need(_п.BRIDGE_PROTOCOL)
        except _п.PortMissing as exc:                    # pragma: no cover
            self.skipTest("конверт неизвестного исхода строит ХОЗЯИН: "
                          + str(exc))
        fingerprint = serving.DocumentFingerprint(
            "Проект — КОПИЯ A5", "", "uid-a5")
        run_id = serving.RunId("0123456789abcdef")
        ...
        # 🔴 THERE USED TO BE A HAND-ROLLED SHAPE THAT THE BRIDGE DOES NOT PRODUCE (edit 24.08.2026).
        # Here stood `{"ok": False, "state": "timeout_unconfirmed", ...}` —
        # without the `error` key, so `bridge_result.extract_error` fell through to the
        # `else state` branch and returned literally the string that
        # `serving.py` compared against. A fixture from a MATCHED PAIR: green for any
        # value of both sides, and so it did not notice that the real envelope
        # (`error: True`, `err.code = transport.execution_unknown`) was being declared
        # a PROVEN ROLLBACK. Now the envelope is built by a PROD FUNCTION.
        async def _timeout_after_commit(*_args):
            # The envelope for an unknown outcome is built by PROD — the language has none.
            from kir import ports as _п
            _unknown_operation_result = _п.need(
                _п.BRIDGE_PROTOCOL)._unknown_operation_result
            committed_ids.add("7001")
            return _unknown_operation_result(
                "op-a5-rebuild",
                "Revit не подтвердил завершение за 130с. "
                "Операция не будет повторена вслепую.")
    ...
    def test_generic_error_without_typed_rollback_stays_pending(self):
        """Error prose and zero ids are not proof that Revit rolled back."""
        ...
        async def _ambiguous_error(*_args):
            return {
                "error": True,
                "message": (
                    "transaction commit status: RolledBack (untyped prose)"),
            }
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (mock.patch(
                  ...
                  mock.patch.object(
                      serving, "_run_declarative",
                      new=mock.AsyncMock(side_effect=_ambiguous_error))):
                rebuild, *_rest = serving._a5_runners(
                    _ShimLLM(), _never_bridge, "2026",
                    stamp_scope=stamp_scope, stamp_prefix=stamp_prefix,
                    document_fingerprint=fingerprint,
        ...
        self.assertEqual(
            set(reopened.state.pending_effects), {"rebuild:000000"})
        self.assertFalse(reopened.state.effect_receipts)
...
class A5RequestHashDeltaTests(unittest.TestCase):
    """Δ is part of the request's identity: a different offset means different runs."""

    def _hash(self, delta, *, scope_digest="c" * 64):
        from kir.contracts import RevisionProof
        from kir.serving import _a5_request_hash
    ...
    def test_default_offset_keeps_the_historical_digest(self):
        from kir.idempotence import DELTA_MM
