"""A curtain wall panel = the carrier's grid CELL (design 2026-07-28).

MEASUREMENT BEFORE THE FIX (28.07, facade model SOB6.2_FAS_R23, 3130
elements read): 734 curtain wall panels, ZERO lifted. A synthetic cell
with a full side-index row became a ``no_lifter`` atom — "category is
outside the exact Part 5 lifter table," meaning the category was not in
the lifters table at all.

The tests below hold three things, each of which would break silently:

* a cell with a FULL address is lifted into ``set_curtain_panel``;
* a cell whose type equals the carrier's grid type is
  ``generator_child``, not an extra operation (a STRUCTURAL rule: type
  against type, no lists of familiar names — INVARIANT #1);
* an index of schema /1 (addresses not captured) yields a TYPED atom,
  not a guess about a (0,0) cell.
"""
...
def _legacy_index(host_id: str = HOST_ID) -> dict[str, Any]:
    """An index of schema /1 — the kind found in every decompile before
    this wave."""
...
def _document_body_only() -> L0Document:
    """A document in which the cell has ONLY the occupied panel.

    This is exactly what re-extraction of a reassembled model looks like:
    measurement ``sob62_fas_r23_v12/idempotence_debug.json`` — 20
    re-extracted curtain wall panels, of which 0 matched the index key
    (``panel_id``, the occupant), while all 20 matched
    ``host_panel_id`` (the occupied one).
    """
...
def _occupied_panels() -> list[dict[str, Any]]:
    """An occupied cell: occupant 9002 + occupied panel 9003."""
...
class CurtainCellLift(unittest.TestCase):
    ...
    def test_type_equal_to_the_hosts_own_cut_is_generator_child(self) -> None:
        """A STRUCTURAL rule: type against type, not a list of familiar
        names."""
    ...
    def test_wall_filled_cell_carries_the_bodys_type_and_body_is_child(
            self) -> None:
        """A cell with a wall is TWO Revit elements; the operation is
        exactly one."""
    ...
    def test_cell_is_found_by_the_occupied_panel_when_the_occupant_is_gone(
            self) -> None:
        """AN OCCUPIED CELL HAS TWO NAMES — the re-lift knows the second
        one.

        MEASUREMENT BEFORE THE FIX (v12, reassembly #6): the index keys
        the cell by the occupant's id, re-extraction returns the id of
        the occupied panel; the intersection of the keys is ZERO. The
        lift received None, fell through to the generic placement path,
        and produced 0 cell sheets where 20 were expected — an
        idempotence divergence out of nowhere.
        """
    ...
    def test_two_names_of_one_cell_never_make_two_operations(self) -> None:
        """A multiset: both sides in the document — the sheet is exactly
        one.

        In the source model BOTH sides are present for all 372 occupied
        cells of v12; if the second name produced its own sheet,
        reassembly would build each such cell twice.
        """
    ...
    def test_one_cell_hashes_the_same_from_either_of_its_two_names(
            self) -> None:
        """The canon must not depend on which element ended up in the
        document.

        MEASUREMENT (v12): for 20 cells lifted from both sides, ``params``
        matched byte for byte, but the canonical hash — never: the
        sheet's ``type_name`` diverged (`«Стена»` — the occupant's
        system-wrapper name — against the name of the type the cell is
        filled with). For the idempotence comparison this looked like a
        model divergence that does not exist.
        """
    ...
    def test_legacy_index_without_an_address_is_a_typed_atom(self) -> None:
        """Schema /1 does not carry addresses — and guessing (0,0) is
        not allowed."""
    ...
    def test_a_host_that_cuts_no_panel_makes_every_cell_authored(self) -> None:
        """``none`` is a READ fact, not the absence of a fact.

        A carrier without an automatic panel generates no cells at all,
        so every occupied one was assigned by the author. Telling this
        state apart from "we could not read it" is exactly the price of
        the live v4 run.
        """
    ...
    def test_without_any_curtain_index_the_panel_stays_an_honest_atom(
            self) -> None:
        """The old path is preserved verbatim: no index — no operation
        either."""
    ...
    def test_lifted_cell_compiles_to_a_program(self) -> None:
        """The op must pass the compiler's parsing — otherwise it is
        inexpressible."""
...
# ── Live index row: facade SOB6.2, Revit 2023, decompile v4 (28.07) ─────────
#
# Copied VERBATIM from backend/data/decompile/sob62_fas_r23_v4/
# curtain.index.json, carrier 8152799 — a curtain wall with a 2×2 grid.
# Only ``mullions`` are omitted (11 rows of the same kind, the cell
# lifter does not read them); everything else is byte for byte, which is
# exactly what ``LiveArtifactFixtureDoesNotDrift`` checks.
#
# Why the fixture is LIVE and not synthetic: this wave's synthetic
# fixture was green until the live run showed that the carrier's grid
# type is not read at all. The synthetic fixture checks what we
# imagined; the live row checks what the model actually returned.
...
def _live_document() -> L0Document:
    """L0 with LIVE identifiers of the carrier, the cell bodies, and the
    window panel."""
...
class TheLiveV4RowIsDiagnosedNotGuessed(unittest.TestCase):
    """What EXACTLY the live v4 decompile says about its 311 cells."""

    def test_v4_as_it_stands_lifts_nothing_and_says_why(self) -> None:
        """A REFUTING MEASUREMENT: the address made it through, the grid
        type did not.

        This is exactly the outcome the live run gave: all cells are
        atoms, and the reason must point to an UNREAD state, not to a
        fabricated "panel is stock."
        """
    ...
    def test_the_same_live_row_lifts_once_the_default_is_read(self) -> None:
        """Fix the link — and the same three cells become operations."""
        ...
        nodes = _by_source(result)
        # Two cells are filled with a WALL — their type is taken from
        # the body, and it differs from the grid type, meaning these are
        # author assignments.
        ...
        # The third cell holds EXACTLY the grid type — it is generated.
        standard = nodes["8152813"]
        self.assertEqual(standard["kind"], "atom")
        self.assertEqual(
            standard["reason"]["code"], AtomReason.GENERATOR_CHILD.value)
        # The cell bodies are also generated — they have no separate
        # operation.
    ...
    def test_the_window_cell_stays_honestly_unaddressed(self) -> None:
        """A curtain wall window is not a ``Panel``; it has no
        ``GetRefGridLines``.

        Live v4 measurement: 50 such cells. There is nothing to invent
        an address from for them, and the index row says so directly
        (``not_a_panel``), rather than staying silent.
        """
...
class LiveArtifactFixtureDoesNotDrift(unittest.TestCase):
    def test_embedded_row_matches_the_artifact_when_it_is_present(
            self) -> None:
        """The live fixture must stay live.

        The artifact is not part of the repository; when it exists on
        the machine, we check against it. Otherwise the "live row" will,
        in a month, become just as synthetic, only with a halo of
        credibility.
        """

        # 🔴 THE GUARD AND THE READER MUST LOOK AT THE SAME FILE (fix
        # 22.08.2026). `snapshot_file_exists` sees `.json.gz` too, while
        # `read_text` looked only for `.json` — the cleanup job
        # compressed `sob62_fas_r23_v4`, and the test failed with
        # `FileNotFoundError` right where the guard said "it's there."
        # This is a named class of defect, and in the same run it turned
        # up in two more files.
        from kir.decompile.snapshot_io import (open_snapshot,
                                                    snapshot_file_exists)
        if not snapshot_file_exists(_LIVE_ARTIFACT):
            self.skipTest(f"нет артефакта {_LIVE_ARTIFACT}")
        with open_snapshot(_LIVE_ARTIFACT, "rt", encoding="utf-8") as handle:
...
class TheMullionBelongsToTheGridNotToPlaceFamily(unittest.TestCase):
    """A curtain wall mullion is an element of the GRID LINE, not an
    instance placed at a point.

    MEASUREMENT 28.07 (live decompile v6, facade SOB6.2): 956 mullions —
    42% of all ops in the model — were being lifted into
    ``place_family`` with a point and a wall host. During reassembly the
    curtain wall generates ITS OWN mullions by the type's rules, and our
    956 would land on top: the op count would grow from our own
    duplicate.

    PROOF (RevitAPI.xml of the reference package, not the wiki): the
    only constructor is ``CurtainGridLine.AddMullions(Curve segment,
    MullionType, bool oneSegmentOnly)``, "add mullions on the specified
    segments of a grid"; ``Mullion`` itself has a ``LocationCurve``, not
    a point. A mullion is not placed as a family instance, even though
    as a class it is a FamilyInstance — which is why the side placement
    index honestly returns a row for it.

    The reason for the refusal is a GAP
    (``unsupported_forward_signature``), not ``generator_child``: some
    mullions are generated by the carrier's type (317 of the 956 in the
    measured model sit on carriers WITHOUT grid lines, i.e. these are
    type borders), while some could have been added by the author on a
    segment. They can only be told apart by capturing the carrier
    type's ``AUTO_MULLION_*`` parameters; until then it is more honest
    NOT to subtract them from the denominator than to subtract too many.
    """

    #: Mullion 10006235 of carrier 10006233 — a row from the LIVE v6
    #: decompile.
    ...
    def _placements(self) -> dict[str, Any]:
        """A row of the side placement index — exactly as in live v6."""
    ...
    def test_without_the_curtain_index_the_mullion_becomes_place_family(
            self) -> None:
        """The pre-state, recorded deliberately: this is what it was
        before the rule."""
    ...
    def test_the_hole_is_not_subtracted_from_honest_coverage(self) -> None:
        """generator_child is subtracted from the denominator, a gap is
        not.

        Marking the mullions as generated would be more pleasant
        (coverage would not drop), but it would be untrue: some of them
        were added by the author, and we have nothing to reproduce them
        with.
        """
