"""A metric must not flatter itself.

Two invariants of honest measurement (architecture review 2026-07-25, §3.6):

1. What FAILED to be lifted (an atom) must remain in the denominator.
   Otherwise what the compiler did not understand silently disappears from
   the score instead of lowering it.
2. A violated postcondition cannot coexist with a "success" verdict.
   In ``postconditions="report"`` mode the program commits, writing
   violations into ``__results["postcondition_violations"]`` — someone
   must read them.
"""
...
def _atom(l1_id: str, level: str = "Этаж 1") -> dict:
    """A leaf that the lift could not lift — a typed refusal."""
    return {
        "kind": "atom",
        "_id": l1_id,
        "level_name": level,
...
class AtomsStayInTheDenominator(unittest.TestCase):
    """§3.6: ``atoms_excluded`` is structurally 0 — atoms are cut before measurement."""
...
class ReportedViolationsReachTheVerdict(unittest.TestCase):
    """§3.6: nobody reads the violations from ``report`` mode."""
