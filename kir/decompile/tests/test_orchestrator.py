...
from kir.decompile.geom_extract import GeometryExtraction, extract_geometry
from kir.decompile.group_extract import (
    GROUP_INDEX_SCHEMA_VERSION,
    GroupExtraction,
)
from kir.decompile.recompile import recompile
...
# 🔴 27.08.2026: KIR LEFT THE PRODUCT TREE. The root used to be searched
# for as an ancestor that has `backend/` — that is, the KUKAI tree; a
# standalone package has no such ancestor at all, and `next()` failed
# with `StopIteration` without a word about the reason. The root is
# needed here for exactly one thing: so the subprocess CAN IMPORT `kir`.
# So it is the directory CONTAINING the package, and it is taken from
# the package itself.
...
class OfflineOrchestratorTests(unittest.TestCase):
    ...
    def test_result_is_deterministic_under_two_pythonhashseed_values(
        ...
        script = (
            "from kir.decompile.tests.test_orchestrator import "
