...
ALL THE NUMBERS BELOW ARE PRE-FIX, and this is stated HERE, not left to
anyone's memory. On 2026-08-13 `build_library` gained the rule "a claim
does not exceed the source's multiplicity" (C-RT as a construction rule,
not a property), and it REJECTS instances: on the fixture, 2 components /
6 placements become 1 / 4. So the library's composition on the corpus has
shifted, and every number in this list is the state BEFORE the fix, not
today's. A re-capture with the same instrument has been ordered and not
done: the sandbox is occupied by someone else's full run, and measuring
under someone else's run means getting a different measurement under the
same name. **Do not cite these numbers as current;
`test_component_overclaim_fixture.py` holds the rule independently of
them.**
...
from kir.decompile.merkle import DedupEntry, build_index, dedup_report
from kir.decompile.tests.test_merkle import (
    _fold,
    _grid_building,
)
...
class DominatedFilterIsAContractNotADecorationTests(unittest.TestCase):
    ...
    def test_a_dominated_entry_is_refused_not_silently_skipped(self) -> None:
        ...
        with mock.patch(
            "kir.decompile.component.dedup_report",
...
class ExecutableSavingIsVisibleWithoutRecomputingItTests(unittest.TestCase):
    ...
    **47% of discovered components cannot be executed** (316 proven of 595)
    ...
    def test_the_proven_split_matches_a_hand_count(self) -> None:
        from kir.decompile.component import prove_execution_fidelity
