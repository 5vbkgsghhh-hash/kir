...
from kir.decompile.tests.test_recompile import (
    _box_solid,
    _translated,
)
from kir.decompile.schema import GEOM_CANON_MM
# 2026-08-27: the execution wrapper is a capability of the HOST, not of
# the language. We ask the port. We do not substitute stubs: a test
# checking the WRAPPER must, without it, be SKIPPED with a named
...
class GeometryIngestionTests(unittest.TestCase):
    ...
    def test_result_bytes_are_identical_under_two_hash_seeds(self) -> None:
        script = (
            ...
            "from kir.decompile.tests.test_geom_extract "
...
class GeometryCSharpEmitterTests(unittest.TestCase):
    ...
    def test_standard_wrapper_and_static_safety_accept_emitter(self) -> None:
        ...
        if wrap_user_code is None:                          # pragma: no cover
            self.skipTest("обёртка исполнения — у хоста: " + _НЕТ_ОБЁРТКИ)
    ...
    def test_emitted_body_is_identical_under_two_hash_seeds(self) -> None:
        script = (
            "import hashlib; "
            "from kir.decompile.geom_extract "
