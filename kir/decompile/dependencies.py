...
class UnresolvedDependency:
    """One unresolved dependency — and its KIND, not just its key.

    🔴 WHY `kind` WAS ADDED (07.09.2026). A recon measurement on
    `sob62_r23_v3`: 99 definitions, 107 unresolved, and NOT ONE of the 107 had
    a kind — even though `DependencyDefinition` from the same draft carries
    one. A reader saw "107 somethings did not resolve" and could not tell a
    family apart from a system type, that is, could not name the fix.

    `None` is NOT emptiness but a named state: the row does not describe a
    definition at all (document environment, document state, side-index
    provenance). Guessing at a kind for those would have been invention.
    """
    ...
    # THE APPEND-AT-THE-TAIL LAW (the same one `L0Element.curve_kind` follows):
    # the field order above is a positional contract of already-written code,
    # and the default value keeps existing calls parseable.
    kind: "DependencyKind | None" = None

    def __post_init__(self) -> None:
        if self.kind is not None and not isinstance(self.kind, DependencyKind):
            raise DependencyManifestError("unresolved kind must be typed or None")
        if not isinstance(self.key, str) or not self.key:
    ...
    *,
    ...
    for draft in drafts:
        ...
        unresolved.append(UnresolvedDependency(
            ...
            # The kind is taken from the SAME draft as the definition on the
            # line above: there is no second source for the kind here, and
