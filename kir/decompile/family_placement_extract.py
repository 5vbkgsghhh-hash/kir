...
:class:`kir.decompile.curtain_extract.CurveState` -- reused verbatim
...
class FamilyPlacementMirrorInvariantError(FamilyPlacementPayloadError):
    ...
    A separate subclass exists for exactly ONE decision: such a row becomes
    a receipt with the typed reason ``mirror_invariant_violated``, not a
    dead run (see :meth:`FamilyPlacementExtraction.from_rows`).
    """
...
class LocationAbsence(str, Enum):
    """Why an instance HAS NO insertion point.

    "Failed to read" and "there simply isn't one here" are different claims,
    and only the second says anything about the model. Before this wave both
    situations arrived at the lifter identically (``placement_available=
    False``, two empty cells), and the only wording it could produce read
    like an admission of an oversight: "FamilyInstance has no captured
    LocationPoint and rotation".

    MEASURED 2026-07-29 (four documents, a by-name breakdown of all 7 083
    elements with this reason): 7 083 of 7 083 are ``OST_CurtainWallPanels``.
    Not one face-hosted, not one work-plane-based; moreover, all 5 060
    ``WorkPlaneBased`` instances in K2 DO HAVE a point. In other words the
    overwhelming bulk of this reason is not an oversight at all, but a
    shape that simply never has an insertion point.
    """

    #: A curtain-wall panel: its position is produced by the host's grid
    #: subdivision. ``Location`` is empty BY CONSTRUCTION, and such a panel
    #: is placed not via ``NewFamilyInstance`` but by assigning a type to a
    #: cell — a free-standing point would be a silent loss of the binding,
    #: not an approximation.
    CURTAIN_GRID_GENERATED = "curtain_grid_generated"
    #: The family is hosted on a FACE (``FamilyInstance.HostFace`` is not
    #: empty). The position can be recovered via a transform, but such a
    #: family is placed with the ``NewFamilyInstance(Reference face, ...)``
    #: overload, which we do not have — the refusal remains, but now it
    #: names the SHAPE, not an oversight.
    FACE_HOSTED = "face_hosted"
    #: A work-plane-based family with no ``Location``. MEASURED 2026-07-29:
    #: across four documents there are NONE of these — the branch is kept
    #: for completeness of the decompile, not for a known target.
    WORK_PLANE_BASED = "work_plane_based"
    #: None of the shapes was recognized, or the read threw. The one value
    #: that honestly means "we don't know".
...
def _raw_element_id(row: Any, index: int) -> str:
    """An element id from the RAW row — for a receipt about this row itself.

    A row where even the id cannot be read must still leave a trace: its
    ordinal position in the batch is the worst, but an honest, address —
    and it beats silence.
    """
...
#: Optional persisted groups. Each is written IN FULL or not written at all
#: (the same idiom as this file's ``to_dict`` and ``CurtainWallRecord.
#: to_dict``: "not applicable" is the ABSENCE of keys, not an explicit null).
#: A half-group is always an error: "the transform origin is there but the
#: basis isn't" is indistinguishable from a point.
...
def _optional_groups_are_whole(extra: set[str]) -> bool:
    """Do the extra keys break down into WHOLE optional groups?"""
    remaining = set(extra)
...
class FamilyPlacementRecord:
    ...
    # The TYPED reason for an empty point. ``None`` here means EXACTLY
    # "this decompile was captured by a schema that did not read the
    # reason" — the frozen corpus (every ``family_placement.index.json``
    # before 2026-07-29) is exactly like that, and the lifter must
    # distinguish this from "there is no reason".
    location_absence: "LocationAbsence | None" = None
    # THE THIRD SOURCE of position — ``Instance.GetTransform()``.
    #
    # The field's name was chosen so it never pretends to be an insertion
    # point: for an instance whose ``Location`` is empty, a position DOES
    # exist, but there is NO insertion point, and the two are not the same
    # thing. The invariant below forbids a row from carrying a point and a
    # transform at once — one cannot be substituted for the other
    ...
    def __post_init__(self) -> None:
        ...
        # THE ONE CLAIM in this file with NO measurement note — and it is
        # wrong in the general case. Live probes on 2026-07-21 (see
        # authoring.py:2291) showed an XOR model for mirroring relative to
        # the family's VERTICAL plane; for work-plane-based and adaptive
        # families mirrored about an arbitrary plane, Revit sets Mirrored
        # without touching either Hand or Facing. Measuring the convention
        # is separate work (it needs live Revit and a family of every
        # placement kind); until then, an invariant violation is treated as
        # a fact ABOUT THIS ELEMENT, not about the run: the subclass is
        # caught in ``from_rows`` and becomes a §18.2 receipt, the element —
        # an honest atom.
        ...
        # ── third source: a position that is NOT an insertion point ──────
        ...
        if self.placement_available and self.location_absence is not None:
            # A point exists — nothing to explain. A row carrying both means
            # the emitter is contradicting itself.
            raise FamilyPlacementPayloadError(
        ...
        if has_origin != all(vector is not None for vector in basis):
            # Half a transform is not a transform: with no basis there is
            # nothing to recover the orientation from, and "the origin is
            # there but not the rotation" looks exactly like an insertion
            # point that should not be here.
            raise FamilyPlacementPayloadError(
                "transform_origin_mm requires all three basis vectors")
        if has_origin:
            # THE FILE'S MAIN GUARD. As long as these two fields cannot stand
            # side by side, no code branch can silently promote a transform
            # into an insertion point: doing so would first require violating
            # the invariant. The ban is not a style choice: a family hosted
            # on a face is placed with the NewFamilyInstance(Reference face,
            # ...) overload, which we do not have, and a free-standing point
            # instead of a face binding is a silent loss of the binding that
            # looks like a success (§18.1).
    ...
    def from_raw(
        ...
        # A transform arrives as a FOURSOME (origin + three basis vectors)
        # or not at all: a half-group is indistinguishable from a point with
        # a broken rotation.
        ...
        if has_absence:
            try:
                ...
                # An unfamiliar word is NOT "probably unreadable": a reason
                # we don't know how to read must raise, not silently degrade
                # to the most harmless of the known values.
                raise FamilyPlacementPayloadError(
        ...
        if has_transform:
            ...
            # Basis vectors are directions, not lengths: converting them to
            # mm would be a dimensional error.
            transform_basis = [
    ...
    def from_dict(
        ...
        raw = _mapping(value, field_name)
        field_set = set(raw)
        # V1 is the mandatory core; beyond it, only WHOLE optional groups are
        # allowed. This way the frozen corpus (V1 and V1+curve) loads
        # unchanged, and new groups require neither a new schema version nor
        # an enumeration of combinations.
        if (_PERSISTED_FIELDS_V1 <= field_set
                and _optional_groups_are_whole(field_set - _PERSISTED_FIELDS_V1)):
            row = raw
        else:
            # No legal shape matches: fall back to the full field set so the
            # error names exactly what is missing or extra — the same
            # V1/V2 fallback path as in GroupInstanceRecord.from_dict.
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        # The same idiom: "not applicable" is the absence of keys. A row
        # with a point has no reason for its absence and cannot have one,
        # and writing it an explicit null would claim the question makes
        # sense.
...
class FamilyPlacementExtraction:
    ...
    §18.2: the stage's answer IS the pair ``records`` + ``failures``. The
    second key is mandatory in the persisted form too: without it there is
    no way to tell "the panel cannot be read" from "nobody asked about the
    panel".
    """
    ...
    def from_rows(
        ...
        *,
        ...
        """Parse rows ROW BY ROW: a broken row is a receipt, not a death.

        This used to be a generator inside the constructor: the very first
        ``FamilyPlacementPayloadError`` exception went into the pipeline's
        general ``except`` and turned into an "internal decompile error" —
        with no element named, none of the batch's other 200 rows, and none
        of the other batches. One panel used to bring down the decompile of
        the whole building (audit finding M6, 28.07).
        """
    ...
    def load_jsonl(cls, path: str | Path) -> "FamilyPlacementExtraction":
        ...
        # The same row-by-row quarantine as on the live path: a broken row
        # is not entitled to take the rest of the file down with it.
        return cls.from_rows(rows)

    @classmethod
    def from_dict(cls, value: Any) -> "FamilyPlacementExtraction":
        # ``failures`` is OPTIONAL on read and REQUIRED on write. Decompiles
        # made before this wave do not carry the key: declaring them unreadable
        # would mean retroactively destroying the archive for the sake of
        # form. The same deliberate migration as for the working-set fields
        # in §18.4.
...
def parse_family_placement_failures(
    ...
    """Index receipts, keyed by element_id; ``None`` means an empty dict.

    Needed by the LIFTER (§18.2/M5): an element whose id is present here
    must be turned into an atom with the reason FROM THE RECEIPT, not with
    a faceless "absent from side index". The input shape is the same as for
    :func:`parse_family_placement_index`: the object itself, a persisted
    envelope, or a bare index (which has no receipts).
    """
    if value is None:
        return {}
    if isinstance(value, FamilyPlacementExtraction):
        ...
        if "schema_version" in value or "family_placement_index" in value:
            ...
            except Exception:  # noqa: BLE001 — a broken receipt must not break the lifter
                return {}
        else:
            return {}
    else:
...
# :data:`kir.decompile.curtain_extract.CURTAIN_EXTRACT_HELPER_CS`'s
...
def build_family_placement_extract_cs(
    ...
    *,
    ...
    :func:`kir.decompile.curve_extract.build_curve_extract_cs`): elapsed
    ...
    ``Document.Title``. IT WAS EXACTLY THIS STAGE that produced the
    measurement on 30.07: a snapshot of linked electrical, taken from a
    plumbing window, returned 1837 receipts out of 2650 elements (1770
    ``element_unresolved``) — the collector was looking for the link id in
    the host document. And 20 times the host DID ANSWER: link element
    ``1442277`` (``OST_ElectricalEquipment``) got the family
    ``Tee - Generic`` — a PLUMBING tee. An empty receipt loudly says "did
    not read"; a row like this lies silently, and there is nothing to
    refute it with.
    """
