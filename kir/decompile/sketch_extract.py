...
from kir.decompile.side_contract import (
    SideFailureReason, legacy_typed_reason, optional_failures,
    source_binding_cs,
)
...
class SketchPayloadError(SketchExtractionError):
    ...
    🔴 `typed_reason` WAS ESTABLISHED ON 20.08.2026, AND THIS IS A CURE FOR
    SHAPE 11, NOT A CONVENIENCE. Every contour-parsing refusal used to be
    recorded with the ONE typed reason `profile_topology_unsupported`,
    because it is inferred from the text's PREFIX, and the prefix they share
    is the same ("exact profile topology unavailable: …"). The distinction
    survived only in the free-form tail of the string — meaning the reader
    was asked to interpret a code that never carried the distinction.

    Now the reason travels WITH the refusal. Whoever did not supply one gets
    the old inference from the prefix, and no old artifact moves.
    """

    def __init__(self, *args, typed_reason=None):
        super().__init__(*args)
...
class RailingPathRecord:
    """One railing capture: path, host, and base level.

    THREE INDEPENDENT READS, not one. Each has its own availability, because
    each has its own guard on the Revit side: an unread host must leave a
    read path in place. This exact separation was what was missing when a
    rotation read carried away the point of EVERY room in every model
    (cb9c3b65) — a shared ``try`` around the element is that very bug.

    ``plane_z_mm`` is the path plane's elevation. ``__ReadChain`` hands back
    points in [x,y], and without this field the elevation would disappear
    silently: on a 59-storey tower, the 41st floor's railing would land on
    the ground, and no one would notice, because the contour would be
    correct.

    ``has_host is None`` means "could not be read", while ``False`` means
    "read, and there is no host". These are DIFFERENT facts, and collapsing
    them into one ``bool`` would mean lying about the second one.
    """
    ...
    def __post_init__(self) -> None:
        ...
        # A host id with no host itself is not "almost data" but a
        # contradiction: had it come from the live model, the lift would
        # bind the railing to an element it itself just said had no host.
...
class ProfileFailure:
    """One §18.2 receipt of the profile index.

    ``typed_reason`` was added by this wave: before it, sketch was the only
    stage WITH NO budget (three full-model passes in one call with a 30 s
    timeout and ``retries=0``), and so the only one whose cut had nothing to
    be named by at all.

    The field is NO LONGER OPTIONAL in substance: if the constructor did not
    supply one, the type is inferred from the string by the compatibility
    table. The earlier argument — "strings like 'dependent Sketch count is
    2' are observations about the element, and labelling them with a type
    would be a lie" — was true in its observation and wrong in its
    conclusion: what followed from it was not the absence of a type but the
    absence of a second CLASS of reasons. The class now exists (see
    :class:`SideFailureKind`), the observation is still called an
    observation, and it stays aggregable at the same time. While there was
    no type, these 227 rows fell into no breakdown of the passport at all.
    """
    ...
    def __post_init__(self) -> None:
        ...
        if self.typed_reason is None:
            # The inference stands HERE, not at five construction sites: the
            # law "a refusal must have a reason" must hold regardless of
            # which extractor branch created it, or the next branch added
            # will give birth to a reasonless receipt all over again.
...
class ProfileExtraction:
    ...
    #: Appended AT THE END deliberately: existing positional constructions
    #: ``ProfileExtraction(records, paths, failures)`` must remain valid, or
    #: the "added a field" change would silently shuffle the receipts.
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            # ABSENT when there are no railings — exactly like ``slopes`` on
            # a roof. A building with no railings must land on disk
            # byte-for-byte the same way it did before this wave, or "we
            # changed nothing" would become an unverifiable claim, and the
            # index version would have had to change and invalidate every
            # existing resume.
    ...
    def from_dict(cls, value: Any) -> "ProfileExtraction":
        root = _exact_fields(value, {
            ...
            # Optional on READING: every already-recorded sketch.index.json
            # lacks this key and must still be readable. Without optional,
            # the "added an index" change would break resume on every
            # existing snapshot at once.
    ...
    for index, loop in enumerate(loops):
        ...
        if not _point_in_loop(loop.points_mm[0], exterior):
            raise SketchPayloadError(
                ...
                "cannot represent")
        holes.append(loop)
    # 🔴 AN ISLAND INSIDE A HOLE IS MATERIAL, NOT A VOID, AND BEFORE 20.08 IT
    # PASSED SILENTLY. The check above requires a loop to lie inside the
    # OUTER contour; an island SATISFIES this condition and got recorded as
    # a second hole — a solid slab got a void in its place, with `ok` and
    # not a single diagnostic. Verified by construction: a 200x200 contour,
    # a 60x60 hole, a 20x20 island -> "PASSED, 2 holes".
    #
    # The cost of honesty is named: across the corpus (67 decompiles, 686
    # profiles with holes, 358 with two or more) islands number ZERO,
    # meaning today's refusal does not take away a SINGLE element. It
    # changes a silently wrong answer into a named absence — exactly the
    # trade the cardinal invariant demands.
    #
    # Cost is O(h^2) with h = the number of holes: a corpus maximum of 5 (6 rings).
    for outer_index, outer in enumerate(holes):
        for inner_index, inner in enumerate(holes):
            if outer_index == inner_index:
                continue
            if _point_in_loop(inner.points_mm[0], outer):
                raise SketchPayloadError(
                    "profile has an island inside a hole: the inner loop is "
                    "MATERIAL, not a void, and the side schema has no way to "
                    "say so",
                    typed_reason=SideFailureReason.PROFILE_ISLAND_IN_HOLE)
...
def _parse_railing_block(
    ...
    """Parse the three INDEPENDENT reads of a railing.

    Each of the three (path, host, base level) is either read or refused BY
    NAME — and one refusal does not touch the other two. This is exactly
    why there are three separate blocks here, not one ``try``: a shared
    guard around the element is the very defect that carried away the point
    of every room.

    An unreadable value is NEVER swapped for a default: an unread host
    yields ``has_host=None`` (not ``False``) plus a receipt, because
    "could not look" and "looked, there is no host" are different facts
    about the building.
    """
    block = _exact_fields(
    ...
    if path_available:
        ...
        # CUT, not DETERMINATION: a railing HAS a path always, and if we did
        # not read it, that is on us. The class is deliberately self-critical.
        failures.append(ProfileFailure(
    ...
    if base_available:
        ...
        # A disputed case (the parameter is absent OR the read failed —
        # indistinguishable from outside), and the rule for a disputed case
        # is one: into CUT.
        failures.append(ProfileFailure(
...
def extract_sketch_profiles(payload: Any) -> ProfileExtraction:
    ...
    root = _exact_fields(
        ...
        # §18.2: ``failures`` is the second half of the response. Optional
        # only when READING already-recorded payloads (the emitter always
        # writes it).
    ...
    for index, raw_failure in enumerate(
            optional_failures(root.get("failures"),
                              "Sketch extraction.failures",
                              SketchPayloadError)):
        row = _exact_fields(
            raw_failure,
            {"element_id", "reason", "typed_reason", "elapsed_ms"},
    ...
    # Appended AT THE END (a ceiling is a sketch element, railings are path
    # elements). This is a HARD refusal for the whole stage, not for an
    # element: a category the C# has started to hand back but parsing does
    # not accept kills the entire batch at once. So the list must grow
    # BEFORE the collector, not after.
    allowed_categories = {
    ...
    for element_index, raw_element in enumerate(elements):
        ...
        if available:
            ...
            try:
                ...
                # A reason named AT THE SITE OF THE REFUSAL outranks one
                # inferred from the prefix: otherwise today's compatibility
                # table would rewrite tomorrow's receipts.
                failures.append(ProfileFailure(
                    element_id,
                    f"exact profile topology unavailable: {exc}"[:300],
                    typed_reason=getattr(exc, "typed_reason", None),
                ))
            continue
...
Func<FootPrintRoof, Dictionary<string, object>> __FootPrintRoofLoops = (__roof) =>
{
    ...
    try
    {
        ...
        // 🔴 НЕПРОЧИТАННЫЙ УКЛОН — НЕ РОВНОЕ РЕБРО (F-076, 02.09.2026).
        //
        // Чтение `get_DefinesSlope`/`get_SlopeAngle` стояло под ПУСТЫМ
        // `catch`, а `__sourceLoops.Add`/`__slopeLoops.Add` — ПОСЛЕ него.
        // Значит законная скатная кровля, у которой хоть одно ребро не
        // прочиталось, уезжала доступным контуром без этого ската: запись
        // `ProfileIndexRecord` с полностью None-уклонами ПОБАЙТОВО равна
        // записи плоской кровли (доказано F-075), ключа `slopes` в ней нет.
        // Кровля пересобиралась более плоской, чем источник, и в квитанциях
        // этого не было видно ничем.
        //
        // Лечится fail-closed, как уже лечится тяжёлая форма в
        // `geom_extract` («частичная геометрия — не геометрия»): контур
        // объявляется НЕДОСТУПНЫМ с названной причиной, и кровля остаётся
        // честным атомом. Частичный набор уклонов молчаливо неотличим от
        // полного, а атом с причиной — отличим всегда.
        bool __slopeReadFailed = false;
        ...
        foreach (ModelCurveArray __modelCurveArray in __profiles)
        {
            ...
            foreach (ModelCurve __modelCurve in __modelCurveArray)
            {
                ...
                catch (Exception __slopeException)
                {
                    __slopeReadFailed = true;
                    __result["reason"] = "roof slope read failed: "
                        + __skClassName(__slopeException);
                }
                if (__slopeReadFailed) return __result;
            }
            __sourceLoops.Add(__curves);
    ...
    (__element, __category) =>
{
    ...
    // 🔴 ОТКАЗ СКАТНОГО ЧТЕНИЯ НЕ ПЕРЕКРЫВАЕТСЯ СЛЕПЫМ К СКАТУ ЗАПАСНЫМ
    // (RV-20, 04.09.2026). Скаты умеет читать РОВНО ОДИН путь —
    // `__FootPrintRoofLoops`; `__DependentSketchLoops` не несёт их вовсе.
    // Здесь стоял безусловный переход к запасному по ЛЮБОЙ причине отказа
    // основного, и если запасной удавался, строка уезжала
    // `profile_available=true`, `slopes=null`, `reason=null` — то есть
    // ПОБАЙТОВО как честная плоская кровля (доказано F-075). Скатная кровля
    // пересобиралась плоской, и ни одна строка об этом не говорила.
    //
    // Причина у основного пути ЕСТЬ (`__profile["reason"]` — «roof slope read
    // failed: …» и родня), её просто затирал успех запасного. Поэтому она
    // едет в строку, а строка остаётся НЕДОСТУПНОЙ: это ровно то лекарство,
    // которым F-076 лечит частичный набор уклонов двумя десятками строк выше
    // («частичный набор молчаливо неотличим от полного, а атом с причиной —
    // отличим всегда»). Запасной путь его обходил, оставаясь снаружи.
    //
    // ЦЕНА НАЗВАНА: плоская `FootPrintRoof`, у которой `GetProfiles` бросил,
    // а зависимый эскиз читается, СТАНОВИТСЯ АТОМОМ вместо контура. Иначе
    // нельзя: слепой к скату путь не умеет сказать, был ли скат, и его «нет
    // уклонов» неотличимо от «уклонов не спрашивали».
    if (__category == "OST_Roofs")
    {
        var __footprint = __element as FootPrintRoof;
        if (__footprint != null)
        {
            __profile = __FootPrintRoofLoops(__footprint);
            if (!((bool)__profile["ok"]))
            {
                __row["reason"] = "roof pitch path refused and no slope-blind "
                    + "fallback may stand in for it: "
                    + (__profile["reason"] == null
                        ? "reason not recorded"
                        : __profile["reason"].ToString());
                return __row;
            }
        }
    }
...
// Заказанный id, которого не забрал ни один коллектор этой стадии, — не
// молчание, а факт: стадия его НЕ ЧИТАЕТ. Это граница ОХВАТА, и код обязан
// говорить именно её.
//
// До 12.08.2026 здесь стоял `element_unresolved`, чей контракт объявляет
// «не нашёлся В ДОКУМЕНТЕ… мог прийти из чужого разбора». Замер на
// `k2_ar_rd_v7`: из 173 таких id в L0 того же прогона нашлись ВСЕ 173
// (контроль: 8 из 8 настоящих найдены, 0 из 5 выдуманных — прибор
// различает). 172 из них — `OST_StairsRailing`, категория ЗАПРОШЕННАЯ,
// у которой 31 элемент получал честную квитанцию, а остальные падали сюда.
// Одна категория, две квитанции, и вторая говорила неправду о первой.
foreach (string __requestedId in __skRequestedIds)
{
    if (!__skSeen.Contains(__requestedId))
        __skFail(__requestedId, "element_not_claimed", "element_not_claimed",
                 null);
}
...
def build_sketch_extract_cs(
    ...
    *,
    ...
    ``element_ids`` is a page of L0 ids for floors/roofs/stairs. An empty
    sequence means "the whole document" and remains a legitimate form for a
    direct call outside the pipeline; the pipeline always passes a page.

    Both budgets are cooperative fuses, NOT preemption (the same discipline
    as :func:`curve_extract.build_curve_extract_cs` and
    :func:`curtain_extract.build_curtain_extract_cs`). One blocking API call
    can still overrun its budget, but its partial result is discarded, the
    overrun leaves as a typed receipt ``time_budget_exceeded`` /
    ``call_budget_exhausted``, and every remaining id still ends up
    accounted for.

    Before this wave the stage was the only one with no budget: three
    full-model passes in one call with a 30 s bridge timeout and
    ``retries=0`` (M9 of the 28.07 audit).

    SIX COLLECTORS AND TWO ``GetElement`` CALLS IN ONE BODY — and all eight
    must look at ONE document. The stage reads a sketch through dependent
    elements (``GetDependentElements`` → ``GetElement``), and a single read
    against the host is enough for a floor's profile from a link to turn
    out to be the profile of someone else's floor.

    ``link_title`` — read not the HOST, but its LINK with the given
    ``Document.Title``. There is one source for the WHOLE body: documents
    have separate id spaces, so a link's id, asked of the host, either is
    not found (a receipt out of nowhere) or finds a FOREIGN element with the
    same number — and then the stage silently records a foreign row as its
    own. The measurement of 30.07 on Snowdon's linked electrical work
    produced both outcomes at once.
    """
