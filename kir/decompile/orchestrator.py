...
from kir.decompile.group_extract import GroupExtraction, parse_group_index
from kir.decompile.l1_schema import L1Node
from kir.decompile.lift_cache import cached_lift_document_detailed
from kir.decompile.name import NameResult, name_document
from kir.decompile.honesty import (
    BuildStatuses,
    EquivalenceClaim,
    EquivalenceScope,
)
from kir.decompile.passport import Passport, build_passport
from kir.decompile.schema import L0Document
from kir.decompile.verify import VerifyResult, verify_document
...
def decompile(
    ...
    *,
    ...
    # 🔴 JOIN INDEX (22.08.2026). Before this fix the orchestrator did not
    # accept it AT ALL, and the already-written lifter `lift.lift_joins` had
    # no path to reach the program: the shape was ready, but there was no
    # wire — the third case of this kind in one day.
    join_index: Any = None,
    ...
    lift_result = cached_lift_document_detailed(
        ...
        # 🔴 THE GROUP INDEX REACHED FOLD BUT DID NOT REACH LIFT, and this is
        # exactly the seam that left the join lifter written yet
        # unreachable (`test_reverse_entrypoints_exist`, the advance journal).
        # `retained_groups` is parsed one line above and fed to BOTH: to
        # fold — as a BOUNDARY, to lift — as the source of `create_group`
        # operations.
