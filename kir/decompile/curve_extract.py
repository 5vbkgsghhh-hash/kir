...
The ``arc`` record reuses :class:`kir.decompile.recompile.ArcCurve` for
...
from kir.decompile.recompile import ArcCurve, GeometrySchemaError
...
def build_curve_extract_cs(
    ...
    *,
    ...
    ``Document.Title``. There is one source for the WHOLE body: documents have
    separate id spaces, so an id from a link, asked of the host, either is not
    found (a receipt on empty ground) or finds a FOREIGN element that shares
    the same number — and then the stage silently records the foreign
    element's name as its own. A 30.07 measurement on Snowdon's linked
    electrical model produced both outcomes at once.
    """
