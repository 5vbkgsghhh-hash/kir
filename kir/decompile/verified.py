...
'verified' flag (R7)", and ``test_verified.py``'s header repeats it as V7.
...
def verified_enabled() -> bool:
    ...
    return env.get("KIR_VERIFIED", "").strip().lower() in {
        "1", "true", "yes", "on",
    }
...
def prove_round_trip(library: Any, tree: Any) -> Proof:
    ...
    from kir.decompile.component import assert_round_trip
    from kir.decompile.merkle import merkle_hash
...
def prove_transition(program: Any, tree_a: Any, tree_b: Any) -> Proof:
    ...
    from kir.decompile.rebuild import BuildingState, assert_transition
    from kir.decompile.merkle import merkle_hash
...
def prove_preservation(tree: Any, nodes: Any) -> Proof:
    ...
    from kir.decompile.fold import assert_preservation
    from kir.decompile.merkle import merkle_hash
...
def prove_merge_clean(result: Any) -> Proof:
    ...
    from kir.decompile.merge3 import MergeResult
...
def prove_refinement(op_certificate: Any) -> Proof:
    ...
    from kir.translation_cert import OpCertificate, assert_refined
...
def verify_equivalence(claim: Any, proof: Proof) -> "Verified[Any]":
    ...
    FAILED verdict — a contradiction, R4).  The ``Verified`` binds to the
