...
class AtomReason(str, Enum):
    ...
    #
    # MEASURED 19.08.2026, BECAUSE "unused" AND "never occurs" ARE
    # DIFFERENT CLAIMS, and the line above only established the first one. Across
    # the entire store (54 trees, 293 178 atoms with a reason) this code occurs
    # **0 times**, and it has not a single producer anywhere in `kukai/` — that
    # is, it is unreachable BY CONSTRUCTION, not merely rare. `decompile/` does
    # not import `registry_base.KINDS` at all, so the lifter has no way to say
    # "the kind is not in the registry"; the gap this code names belongs to the
    # family of QUERIES (`query_count(kind=...)`), not to lifting.
    #
    # 🔴 THE TRIGGER WAS A HYPOTHESIS THAT MEASUREMENT REFUTED: that elements
    # whose category has no kind silently get the neighboring reason about an
    # OPERATION. Checked on the tower: all 4 479 `OST_TelephoneDevices` carry
    # `unsupported_forward_signature`, and `registry_op_gap` is also 0 across
    # the corpus. Not one element falls into the wrong reason.
    ...
    # §18.1 consequence: a non-Line LocationCurve has no way to be expressed —
    # neither by the op's arc parameter nor by an exact arc from the side
    # index. A separate code is needed precisely because the alternative was
    # a CHORD: a silent straightening passed verify as exact (only the
    # endpoints are compared) and looked like a success. The code is NOT part
    # of _SHAPE_REFUSALS: this is a refusal about geometry that cannot be
    # discarded, not about a shape that another op is willing to accept.
    CURVE_KIND_UNSUPPORTED = "curve_kind_unsupported"
    # THE OP EXISTS, BUT IT HAS NO INPUTS IN THE READING — a third state that
    # was not in this enumeration before 29.07, and its absence cost a correct
    # diagnosis.
    #
    # `no_lifter` means "there is no operation at all for this", and the
    # reason ranking reads it exactly that way: a row with 13 905 dimensions
    # under this code tells the next stage "write create_dimension". But
    # create_dimension IS WRITTEN and has sat in spec.OPS since 28.07 —
    # alongside create_tag and create_text. What's missing is not an op, but
    # fields: the required inputs of all three (`in_view`, `refs`/`target`,
    # `at`/`line_at`, `content`) are simply absent as fields in the frozen L0
    # 1.0 row (see schema.L0Element — it has no kind-owner field at all).
    #
    # The difference is not cosmetic but addressable: `no_lifter` sends you to
    # work on the operation registry, `source_contract_gap` — to the READING.
    # 29.07 already drew exactly this distinction for the placement stage
    # ("a stage's silence is not a fact about the element"), and the cost
    # there was measured as one population standing twice in the ranking
    # under two different names.
    #
    # The code is NOT part of _SHAPE_REFUSALS: this is a refusal not about the
    # element's shape, and the second op will not pick it up.
