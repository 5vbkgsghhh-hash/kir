...
def _raw_element_id(row: Any, index: int) -> str:
    """A group id from the RAW row — for a receipt about this row itself."""
...
class GroupExtraction:
    ...
    §18.2: ``failures`` is the second half of the answer, not an appendix
    to it. The stage is whole-model (a group has no L0 category), so its
    answer cannot be checked against a list of requested ids; all the more
    reason a group cut off by the budget or left unread by the API must be
    visible BY NAME.
    """
    ...
    def from_rows(
        ...
        *,
        ...
        """Row-by-row quarantine: a broken row is a receipt, not a dead run."""
    ...
    def from_dict(cls, value: Any) -> "GroupExtraction":
        ...
        # captured before this wave do not carry the key (the same deliberate
        # migration as for the working-set fields in §18.4).
...
# all"): the pipeline registers this stage with ``whole_model=True`` and an
...
#
# This line used to read the other way around, and it cost more than
# anything else in the file. It said: "the observed bridge dialect, in which
# there is no angle, is a limitation of the BRIDGE, not the contract;
# LocationPoint returns Rotation directly, so it is honest to return both."
# The first half is correct as an observation, the conclusion is not: the
# angle was missing from the dialect not because the bridge could not do it,
# but because THIS READ DOES NOT EXIST FOR A GROUP. Autodesk writes this
# itself, verbatim and identically across all six supported versions
# (RevitAPI.xml,
...
#
# The cost of the inference — MEASURED on 13A-RD-AR-K2_v33: 2846 of 2941
# groups (96.77%) never made it into the index at all, all with one line
# "group read failed: InvalidOperationException". Exactly 95 nested groups
# survived, the ones whose ``Location`` is empty, so the forbidden read was
# never even attempted.
#
# The moral is general and worth keeping here: THE OBSERVED DIALECT IS DATA.
# A missing field is explained, not eliminated; "the bridge didn't finish
# reading" and "the API doesn't return this" are indistinguishable by eye and
# differ only by reading the documentation for the specific member.
#
# ``reference_level_id``/``origin_level_offset_ft`` remain an inseparable pair
# of evidence: the offset is measured from ``Level.Elevation`` to that same
# live group point.  The point and the offset go on the wire in RAW internal
# feet; the strict parser owns the unit conversion.
#
...
def build_group_extract_cs(
    *,
    ...
    ``Document.Title``. The stage is whole-model, and that makes its case
    WORSE than paged ones: it does not place an order, so there is nothing to
    check against, and the host's groups would have arrived disguised as the
    link's groups — without a single receipt, with a perfect row count.

    A MEASUREMENT on 30.07 (``snowdon_elec_v1``) showed ZERO groups and zero
    receipts, and that is NOT proof of correctness: the host document
    (plumbing) simply had no model groups. An empty answer here is
    indistinguishable from a correct one — which is exactly why the source
    must be given, not inferred from the result.
    """
