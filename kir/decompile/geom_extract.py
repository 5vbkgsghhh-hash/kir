...
class GeometryStore:
    ...
    def __init__(self) -> None:
        ...
        #: address -> fingerprints of DIFFERENT raw definitions that landed
        #: under this address. More than one means a MERGE: the canon judged
        #: them to be the same, and the difference between them is lost.
        #: Nobody has ever measured this number (F-071).
        self._raw_digests: dict[str, set[str]] = {}
    ...
    def add(
        ...
        # 🔴 A CANON MERGE IS COUNTED, NOT HUSHED UP (F-071). The guard above
        # catches a SHA-256 collision — an event that never happens. The
        # real and ROUTINE event is different: two DIFFERENT geometries
        # produce ONE canonical form (a difference smaller than
        # `GEOM_CANON_MM`), `existing == canonical`, there is no exception —
        # and the difference between them vanishes without a trace. That is
        # exactly what the canon exists for, so refusing here is wrong; what
        # must be done is to COUNT how many distinct definitions merged into
        # one address.
        raw_digest = hashlib.sha256(json.dumps(
            validated.to_dict(), ensure_ascii=False, allow_nan=False,
            separators=(",", ":"), sort_keys=True).encode("utf-8")).hexdigest()
        self._raw_digests.setdefault(geo_hash, set()).add(raw_digest)
        if existing is None:
            self._canonical[geo_hash] = canonical
            # 🔴 THE RAW FORM WAS STORED, BUT THE QUANTIZED FORM WAS
            # FINGERPRINTED (F-071), and the content diverged from its own
            # ADDRESS. The consequence was measured at runtime: two meshes
            # differing by 0.2 mm produce one address, and the store hands
            # back whichever one arrived FIRST —
            #
            #     0.0 arrives then 0.2 -> returns z=0.0
            #     0.2 arrives then 0.0 -> returns z=0.2
            #
            # meaning the same building would be assembled from DIFFERENT
            # geometry depending on traversal order. We now store exactly
            # what the hash addresses: the answer stops depending on arrival
            # order and becomes equal to its own address. Two close
            # geometries do not thereby become different — the canon
            # forbids it — but both honestly get the ONE same one.
            self._definitions[geo_hash] = StoredGeometry(
                geo_hash,
                _validated_geometry(json.loads(canonical.decode("utf-8"))))
        return geo_hash

    def quantisation_collisions(self) -> dict[str, int]:
        """Address -> how many DIFFERENT raw definitions merged into it beyond
        the first one. An empty dict means "there were no merges".

        🔴 WITHOUT THIS COUNT, THE FIX WOULD HAVE TRADED A SILENT SUBSTITUTION
        FOR A SILENT ROUNDING. The store now answers everyone identically —
        but the fact that it answered with something OTHER than what was
        asked for must remain askable. Nobody measures this number today.
        """

        return {geo_hash: len(digests) - 1
                for geo_hash, digests in sorted(self._raw_digests.items())
                if len(digests) > 1}
            ...
            __budgetExceeded) =>
{
    ...
    foreach (GeometryObject __object in __geometry)
    {
        ...
        // 🔴 ЧЕТВЁРТЫЙ РОД НЕ СУЩЕСТВОВАЛ ДЛЯ ЭТОГО ОБХОДА (F-072, 02.09.2026).
        //
        // Выше разбирались ТРИ рода `GeometryObject` — Solid, GeometryInstance,
        // Mesh, — и цикл заканчивался БЕЗ else и без ошибки. Всякий иной
        // законный род (Curve у `ModelCurve`, PolyLine, Point, Face, Edge)
        // проходил насквозь молча, `__parts` оставался пуст, и строка уезжала
        // `status="empty"` -> Tier A. То есть у элемента, чья геометрия ЕСТЬ,
        // улика пересборки говорила «геометрии нет» — самый неверный из
        // доступных ответов, и он неотличим от честно пустого элемента.
        //
        // Отказ намеренно НЕ типизирован причиной из `GeometryFailureReason`:
        // типизированная причина обязана нести `elapsed_ms` и означает срез
        // ПО БЮДЖЕТУ, а здесь срез по ВОЗМОЖНОСТЯМ обхода. Род называется
        // именем класса, чтобы следующая волна знала, какие роды заводить, а
        // не гадала по числу.
        __errors.Add("geometry_object_kind_unsupported: "
...
def build_geometry_extract_cs(
    ...
    *,
    ...
    have different id spaces, so a link id asked of the host either is not
    found (a receipt out of nowhere), or finds a FOREIGN element with the
    same number — and then the stage silently records someone else's row as
    its own. A measurement on 30.07 against Snowdon's linked electrical
    model produced both outcomes at once.
    """
    ...
    # The helpers also carry a substitution (the weight ceiling), so they
    # too go through the check for an unresolved placeholder: a guard that
    # only looks at the body would silently let `__GX_WEIGHT_CEILING__`
    # through into live Revit.
