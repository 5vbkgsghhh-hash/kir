...
from kir.decompile.dependencies import (
    DependencyManifest,
    TargetContract,
    build_dependency_manifest,
)
from kir.decompile.fold import TreeNode, iter_l1_leaves
...
from kir.decompile.name import NameResult, ShapeClassification
...
from kir.decompile.recompile import (
    GbSolid,
    GeometryNode,
    GeometrySchemaError,
    GmMesh,
...
from kir.decompile.schema import L0Document, PASSPORT_INJECT_TOKENS
from kir.decompile.verify import NodeVerdict, VerifyResult
...
def assemble_passport(
    ...
    *,
    ...
    payload = {
        ...
        # DOCUMENT IDENTITY, NOT ITS NAME (15.08.2026, forward-only).
        #
        # Before this line the passport carried exactly ONE field out of the
        # triple `(title, path_name, project_uid)` — the name. A consumer
        # comparing a batch against an already-standing building
        # (`clash/existing.py`) could only rely on `doc_name`, and two
        # documents with the same title are indistinguishable: the finding
        # was marked `proven=False`, and the receipt honestly warned that
        # the comparison was against a PAST state.
        #
        # The fact is ALREADY READ by the extractor and ALREADY sits in L0
        # (`L0Document.identity`, `extract.py:1074-1085`), and the live
        # fingerprint takes the very same Revit field —
        # `doc.ProjectInformation.UniqueId` (`open_model.py:1908`). The two
        # sides are comparable; it simply never reached here.
        #
        # 🔴 WHAT THIS MEANS, AND WHAT IT DOES NOT. `project_information_unique_id`
        # is PROVENANCE, not authority: Autodesk contracts `Element.UniqueId`
        # to be unique WITHIN a document, so Save As carries the same value
        # (`decompile/identity.py`, `AUTHORITATIVE_DOCUMENT_IDENTITY_SOURCES`).
        # Hence the asymmetry the consumer relies on: a match does NOT prove
        # "the same file", but a MISMATCH proves "a different file". A
        # refutation here is stronger than a confirmation, and that is a
        # legitimate win.
        #
        # forward-only, and stated plainly: 4 of 71 stored L0 records carry
        # `identity`, the rest were captured before it existed and cannot
        # carry it. `None` here means "this decompile was captured earlier",
        # not "the document has no identity".
        "document_identity": (
