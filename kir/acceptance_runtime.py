...
from kir.bridge_result import extract_error
from kir.contracts import DocumentFingerprint, ElementIdentityProof
from kir.midend import GroundedProgram
from kir.outcome import AcceptanceState, ProgramOutcome, WitnessState
...
def execution_operation_identity(
    binding: ExecutionArtifactBinding,
    wrapped_source: str,
) -> OperationIdentity:
    """Derive the exact identity that the Bridge host will assign on dispatch.

    If there is no identity for an LLM-tool call, the host deliberately
    derives the identity of a regular KIR write from the durable acceptance
    run. The same pure rule here lets the building journal bind the id
    *before* transport, without importing the host's code and without
    guessing the identity from a later receipt.
    """

    if not isinstance(binding, ExecutionArtifactBinding):
        raise TypeError("operation identity requires an artifact binding")
    binding.require_transport_exact(
        wrapped_source,
    ...
    return OperationIdentity.for_payload(
        ...
        tool_name=(
            f"{binding.execution_lane}:{binding.tool}:{binding.op}"
        ),
        method="execute",
        params={
            "code": wrapped_source,
...
class AcceptanceSession:
    ...
    def abort_before_artifact(
        ...
        *,
        ...
        """Persist a proven not-started refusal while the state is PREPARED."""

        self.journal.abort_before_artifact(outcome, detail=detail)
    ...
    def evidence_wire(self, evidence: AcceptanceEvidence) -> dict[str, Any]:
        ...
        payload["journal"] = {
            "schema_version": self.journal.state.schema_version,
    ...
    def registration_wire(self) -> dict[str, Any]:
        ...
        payload = {
            "schema_version": self.journal.state.schema_version,
            "state": (
                "aborted_before_artifact"
                if self.journal.state.terminal_event
                == "aborted_before_artifact"
                else "finalized" if self.journal.state.finalized
...
async def prepare_acceptance(
    ...
    *,
    ...
    # Both lookups are DATA ABOUT THE MODEL from the same snapshot that
    # already grounded the program, and both are read BEFORE the effect:
    # the predicate is pre-registered by construction, not justified
    # afterward.
