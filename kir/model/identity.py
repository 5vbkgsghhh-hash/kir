...
"WE ALREADY HAVE IDENTITY IN L0" — TRUE ABOUT WRITING AND FALSE ABOUT READING
(measured 11.08.2026, tree `aecf6cff` + this wave).

The producer and the consumer of these facts are gated by DIFFERENT
decisions, and that is deliberate, not forgotten:

* WRITES — unconditionally. The C# extraction block (`extract.py`,
  `__DocumentIdentityFact` / `__DocumentIdentityAuthoritative`) is gated by
  no flag at all; that module's only environment variable is
  `KUKAI_DECOMPILE_OUT`, and that is the output path. Identity facts will
  ride into L0 starting with the very first live extraction.
* READS — only behind a flag that is off. `resolve_element_identity` and
  `identity_context_from_l0` are called from EXACTLY TWO PLACES:
  `building_graph.py:961` and `building_graph.py:992`, and this whole
  module is gated by `building_graph_enabled()`
  (`KUKAI_IR_BUILDING_GRAPH`, default OFF). On the live path, identity types
  are parsed by the schema, but identity RESOLUTION is performed by no one.

The consequence this paragraph is written for: while the flag is off, the
field keeps growing and no one reads it. That is the cost of one
re-extraction, not a defect — but the claim "identity already works" is
false, and checking it takes two greps by the names above, not a day.

A caveat about hits that do NOT count: `.identity` in
`decompile/dependencies.py` and `decompile/passport.py` is a DIFFERENT type
(`DependencyIdentity`, `definition.identity`), not the `L0Document.identity`
field.
"""
