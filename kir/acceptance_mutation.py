...
from kir.contracts import DocumentFingerprint, ElementIdentityProof
from kir.document_guard import bind_read_to_document
from kir.emit_utils import cs_element_id_literal, cs_string_literal
...
_UNMEASURED_WRITES: Mapping[str, str] = {
    ...
    # 🔴 HERE, AND NOT IN FULL ACCEPTANCE, AND THE DIFFERENCE IS NOT FORMAL
    # (18.08.2026).
    #
    # The op HAS a witness, and a strong one: it re-reads
    # `AreElementsJoined(doc, a, b)` after Regenerate — exactly the relation
    # it promised (certificate `proven=True` on six versions, 0 vacuous).
    # But this witness is INSIDE the transaction, i.e. the op's own claim.
    # Independent acceptance is a SECOND reader, and it has nothing to read
    # the join with: the `acceptance_probe` cannot ask `JoinGeometryUtils`,
    # and adding this without a live run would mean declaring a check that
    # does not exist.
    #
    # Writing it here is reversible (the upper bound is lost). Writing it
    # into full acceptance without a probe would be irreversible:
    # acceptance would wait for a fact it does not observe, and would fail
    # an HONEST build.
    "join_elements": (
        "меняет ОТНОШЕНИЕ двух уже стоящих элементов, а не состав документа: "
        "instance-перепись его не видит по построению. Свидетель опа "
        "перечитывает AreElementsJoined в транзакции, но НЕЗАВИСИМОГО зонда "
        "для JoinGeometryUtils пока нет — объявлять проверку, которой нет, "
        "запрещено"),
    # ── FOUR CATALOG AND VIEW OPS (24.08.2026) ──────────────────────────────
    # The precedent, verbatim, is `create_type` and `load_family` above: the
    # result is not an instance, and the instance census does not see it BY
    # CONSTRUCTION. The acceptance probe does not yet expose an independent
    # locator for a new type/material/view, and declaring a check that does
    # not exist is forbidden here.
    "create_wall_type": (
        "создаёт WallType/FloorType/RoofType/CeilingType — ElementType, "
        "исключённый из instance-census; независимый локатор нового типа "
        "пока не выводится без доверия к receipt"),
    "transfer_family": (
        "везёт Family/FamilySymbol из соседнего документа; результат не "
        "экземпляр, и независимый локатор пока не выводится"),
    "transfer_material": (
        "везёт Material из соседнего документа — элемент библиотеки, а не "
        "экземпляр здания; instance-перепись его не видит по построению, "
        "а независимого зонда, читающего материал в цели, пока нет"),
    "create_floor_plan": (
        "создаёт ВИД, а не элемент здания: instance-перепись видов не "
        "считает вовсе"),
}
...
def derive_mutation_expectation(planned: PlannedProgram) -> MutationExpectation:
    ...
    for op in planned.to_ops():
        ...
        if name == "set_param":
            ...
            value = op["value"]
            kind = value["type"]
            # 🔴 A REFERENCE VALUE IS LEGITIMATE, YET ACCEPTANCE WAS FAILING
            # ON IT WITH AN UNTYPED EXCEPTION (F-096, stage 1).
            #
            # `set_param` with a reference (material, phase, workset) is a
            # CAPABILITY of the tree: the program compiles, the C# carries
            # `Set(__rf_*.Id)`, grounding checks the name against the
            # offline pool. But here `value["v"]` is a STRING (the
            # material's name), and it was landing in `expected_number`,
            # while `MutationClaim` required a finite NUMBER for every
            # non-string kind: `expected_number must be finite`.
            # `MutationAcceptanceError` is a `ValueError`, and NOT an
            # `AcceptanceRuntimeError`, so the refusal traveled ABOVE
            # `prepare_acceptance` and reached the author as a bare
            # exception — BEFORE the one independent re-read.
            #
            # THE BLINDNESS HERE IS HONEST, NOT AN EXCUSE, and this is
            # verifiable: the probe below reads EXACTLY THREE storage kinds
            # (String/Integer/Double) and has no branch at all for
            # `StorageType.ElementId`, meaning there is NOTHING to re-read
            # what was just written with. Declaring a check that does not
            # exist is forbidden here (the same argument as for
            # `_UNMEASURED_WRITES`).
            #
            # 🔴 WHAT THIS STAGE DOES NOT DO, AND THIS IS A DEBT, NOT A
            # DECISION: a reference `set_param` STOPS BEING CHECKABLE. Stage
            # 2 is a fourth probe branch (`AsElementId()` plus a second read
            # of the name), its own field for the read-back ElementId in
            # the observation (the name is deliberately NOT written here:
            # an instrument grepping for it must read the code, not this
            # remark), its own claim branch, the `ref`/`int_ref` keys in
            # the `wanted_storage` table below (absent there today, and
            # after a fix of the claim alone, would raise a `KeyError`
            # there — an untyped refusal in place of a typed one), and a
            # name comparison after the write. It touches EMISSION and
            # therefore goes through as a separate commit through the 6/6
            # gate.
            if kind in ("ref", "int_ref"):
                blind.append(_blind(
                    op_id, name,
                    f"значение — ссылка на {value.get('pool', '?')} "
                    f"«{value.get('v')}»; независимый зонд не читает "
                    f"StorageType.ElementId, перечитать записанное нечем"))
                continue
            row = {
                "target_id": target_id,
