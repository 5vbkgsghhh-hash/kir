...
from kir.decompile.l1_schema import L1Node, _node_refs
...
# The ceiling of chunk-level fail-soft. Above it, the run refuses entirely:
# a share of the model built on top of a quarter of refusals is no longer
# "a percentage of compiler quality", but a percentage of who-knows-what.
# The threshold is checked BOTH by the number of chunks AND by the number
# of ops (see the note in run_idempotence: one of the two numbers always
# lies — a solo chunk carries one op, a large one up to 250).
...
from kir.idempotence_report import (
    IdempotenceReport,
    KindComparison,
    MetricTotals,
    _ratio_pct,
...
#: The effect of an op in the ``modify`` family does not create an
#: element: Revit keeps the element id, the id does not land in
#: ``created_ids``, and re-extraction asks EXACTLY about ``created_ids``.
#: Such an effect is invisible BY CONSTRUCTION of the comparison universe —
#: no matter how much the elevator gets fixed.
#:
#: Measured on rebuild #6 (sob62_fas_r23_v11): 54 expected
#: ``set_curtain_panel``, of which 44 change the panel's type IN PLACE and
#: are absent from re-extraction, while 10 produce an element and ARE
#: present there.  Counting all 54 as missing would lie about the 44;
#: excluding all 54 would hide the 10 (exactly the way the
#: ``place_family`` carve-out hid 15 live flip-guard misses, see above).
#: So the rule checks OBSERVABILITY, not the op's name.
def _assigned_type_names(leaf: L1Node) -> set[str]:
    """The types that the leaf ASSIGNS (selectors ``{"by":…, "value":…}``).

    Structurally, not by a list of parameter names: for ``set_curtain_panel``
    that is ``panel_type``, for future modify-ops their own selectors, and
    the rule will not need editing alongside the registry.
    """
...
def modify_outside_universe(
    ...
    """Indices of expected leaves whose effect is invisible to the comparison universe.

    A leaf lands here only if BOTH conditions hold:
    1. its op belongs to the ``modify`` family (a rule from the registry,
       not a list);
    2. re-extraction has NOT A SINGLE element carrying the assigned type —
       that is, the effect produced no element that landed in
       ``created_ids``.

    The second condition is exactly the safeguard against concealment: a
    panel that DID produce an element remains an honest shortfall and is
    visible as ``missing``.
    """
    # `spec` is imported at module level (line 56). A repeated local import
    # made the name LOCAL for the whole function — the same shadowing that
    # brought down the refusal handler via `json` (measured 02.08) — and
    # the try/except around it was dead protection: an unreachable registry
    # would have kept the module from loading at all.
...
def _jsonable(value: Any) -> Any:
    """Index -> JSON, whatever it may be: dumping evidence has no right to
    crash the run over an unfamiliar type."""
...
def _compare(
    ...
    for index, (leaf, h) in enumerate(zip(expected, expected_leaf_hashes)):
        kind = _op_name_of(leaf)
        if index in outside:
            # An expectation outside the universe: neither in the
            # denominator nor in the shortfall.
...
def collect_created_ids(rebuild_results: Sequence[Mapping[str, Any]]) -> list[str]:
    ...
    for envelope in rebuild_results:
        ...
        for value in payload.values():
            if isinstance(value, Mapping):
                # `id` is the op's element IDENTITY (the census law), not
                # evidence of creation. An op that MODIFIED an existing
                # element says so explicitly (`created: false`), and its id
                # does not go here: A5 deletes by this list, and a foreign
                # element in it would mean deleting something not ours.
                #
                # MEASURED 28.07 (rebuild #5): a curtain panel can go
                # either way — a wall type produces a NEW element, a panel
                # type changes in place. The key is absent for all other
                # ops, and their behavior is unchanged.
                if value.get("created") is False:
                    continue
                # 🔴 FIELDS ARE QUERIED FROM THE REGISTRY, NOT ENUMERATED
                # HERE (04.09.2026). `value.get("id") or
                # value.get("element_id")` used to stand here — A LITERAL,
                # that is, a second carrier of the knowledge "what the op
                # calls the thing it created". The first carrier is the
                # registry (`ResultSpec.identity_field`), and
                # `created_ledger.created_keys()` already queries it,
                # explicitly declaring the list "complete BY
                # CONSTRUCTION". The two carriers diverged the very day
                # `create_railing` became MANY: its field is
                # `railing_ids`, and the MEASUREMENT showed `[]` — on
                # rollback NOT ONE stair railing got deleted, even though
                # two had been created. `segment_ids` suffered the same.
                # The cost of an error here is at its worst: the list
                # feeds A5-deletion of a live model, and an omission
                # leaves ORPHANS, while an extra name would delete
                # something not ours.
                #
                # `element_id` is kept in addition to the registry ones:
                # it is not a field of the op, it is the shape of the
                # bridge's receipt, and the registry knows nothing about
                # it.
                from kir.address import created_identity_fields  # noqa: PLC0415

                for поле in (*created_identity_fields(), "element_id"):
                    created = value.get(поле)
                    # A multiple result arrives as a LIST: skipping the
                    # tail would mean deleting one railing out of two — an
                    # outcome worse than either alternative, and a silent
                    # one.
                    for один in (created if isinstance(created, (list, tuple))
                                 else (created,)):
                        if isinstance(один, str) and один.strip():
                            ids.add(один.strip())
                        elif isinstance(один, int) and not isinstance(один, bool):
                            ids.add(str(один))
...
def _stamp_census(envelope: Any) -> Optional[dict[str, Any]]:
    """The stamp census from the cleanup response, if it is complete and readable.

    A census is MORE AUTHORITATIVE than per-item deletion witnesses, because
    it answers the real question of cleanup: "is there ANYTHING of ours
    left in the model at all". A per-item pass answers a narrower one: "did
    Revit confirm the deletion of each id" — and on a cascade it is
    falsely negative.

    MEASURED 28.07 (run #4): of 1236 created, 12 elements disappeared
    TOGETHER WITH THEIR HOSTS (curtain panels leave along with their
    host), their deletion came back with the refusal "element not found" —
    and cleanup declared itself unsuccessful, discarding the whole chunk
    on top of that: 1108/1236 instead of 1224/1236. Meanwhile the same
    run's final stamp census: found 0, remaining 0, witnesses_complete
    true — nothing was left in the model.
    """
...
def curtain_hosts_needing_isolation(curtain_index: Any) -> frozenset[str]:
    """Curtain-wall hosts whose group cannot be let into a shared chunk.

    THE REASON WAS MEASURED, not derived. A live rebuild of the SOB6.2
    façade (28.07, run v7) rolled back a chunk of 250 ops in full:
    ...
    Both named elements are AUTO-PANELS that Revit generates itself when a
    curtain-wall-type wall is created: for such a type the «Curtain Panel»
    parameter (AUTO_PANEL_WALL) points to a LOADABLE FAMILY, and Revit
    instantiates it in every cell. One such wall, placed by probe P1 on
    its own, builds; several in one transaction do not.

    This cannot be held by per-op isolation BY CONSTRUCTION: per the
    Revit API documentation for ``SubTransaction.Commit`` — «the changes
    are not permanently committed … only when the active transaction is
    committed», and the regeneration failure arrives deferred, at the
    parent's Commit. So the only lever is CHUNK SIZE.

    THE TRAIT IS STRUCTURAL, with not a single name (INVARIANT #1): the
    host's panel-division type is read (``default_panel_state == "ok"``)
    AND it is instantiated in the cell as a FAMILY INSTANCE with no body
    wall. When «Curtain Panel» points to a WALL type, the cell is occupied
    by a wall (the panel has ``host_panel_id``), and there is no arbitrary
    family code in the generation.

    MEASUREMENT OF THE TRAIT (façade v6/v7): it selects 8 and 9 hosts out
    of 1201/1203 — exactly the grille ones, not one false positive. The
    ninth on v7 is a wall left in the model by probe P1.
    """
...
def _isolation_from_artifacts(
    ...
    """Who to isolate: what the caller passed, otherwise — from the nearby parse.

    No index — no isolation: this is exactly the previous behavior, and it
    is VISIBLE in the report (``isolated_groups``), not implied.
    """

    return _isolation_source(debug_dir, explicit)[0]
...
def _isolation_source(
    ...
    """Who to isolate AND WHY EXACTLY THAT MANY. The second part is not decoration.

    🔴 AN EMPTY SET HERE HAD FOUR DIFFERENT REASONS AND ONE APPEARANCE
    (29.08.2026, `MUTE_SOURCES` debt). Of the four, TWO are a genuine
    answer, and they rightfully stay silent; the other two were not an
    answer at all:

        the caller named the set themself        -> AN ANSWER (the bare value is it)
        the index was read, nothing to isolate    -> AN ANSWER
        no parse catalog was supplied              -> "wasn't asked", NOT an answer
        the index was NOT READ (OSError/ValueError) -> NOT an answer, and an expensive one

    The last one is decisive: isolation exists so that the radius of a
    refusal equals ONE group. Silently not isolating means silently
    returning the radius to the whole package, and `isolated_groups: 0` in
    the report is indistinguishable from an honest zero.

    A PAIR is returned, because the count and the reason are different
    quantities: the count answers "how many were isolated", the reason
    answers "why not more". The previous docstring pointed to
    `isolated_groups` as the carrier of both; it exists (`materialize.py`),
    but it is a NUMBER and cannot tell the reasons apart.
    """
    if explicit is not None:
        return frozenset(explicit), ""
    if not debug_dir:
        return frozenset(), ("каталог разбора не подан: изоляция по артефактам "
                             "НЕ СПРАШИВАЛАСЬ — это факт о вызове, не о здании")
    path = pathlib.Path(debug_dir) / "curtain.index.json"
    # 🔴 READ WITH THE SAME THING THAT CHECKED EXISTENCE (28.08.2026). Only
    # HALF of a move used to stand here: the check already knew about
    # compression (`snapshot_file_exists` sees `curtain.index.json.gz`
    # too), while the read did not (`path.read_text`). On a COOLED-DOWN
    # parse this produced not a refusal but SILENCE: `OSError` was caught,
    # the function returned an empty set, and curtain-wall hosts were not
    # isolated — the wrong conclusion, looking like its own.
    #
    # This tree's own guard found the defect
    # (`decompile/tests/test_snapshot_readers_know_gzip.py`), and it could
    # NOT have found it earlier: after the split it counted the root by
    # stepping upward and walked `/opt` instead of the package. The
    # instrument's broken path WAS HIDING the real finding.
    from kir.decompile.snapshot_io import read_snapshot_text, snapshot_file_exists
    if not snapshot_file_exists(path):
        return frozenset(), (
            f"{path.name} нет рядом с разбором: изолировать нечего ЛИБО "
            "ступень захвата витража не снималась — ПО ЭТОМУ ФАЙЛУ НЕ РАЗЛИЧИТЬ")
    try:
        payload = json.loads(read_snapshot_text(path))
    except (OSError, ValueError) as exc:
        return frozenset(), (
            f"{path.name} НЕ ПРОЧИТАН ({type(exc).__name__}): изоляция не "
            "применена, радиус отказа равен всей упаковке")
...
def _isolated_programs(
    ...
    """``program_id`` of isolated programs -> a receipt draft.

    An isolated program is recognized by its makeup, not by ordinal
    number: it contains an op on an ISOLATED host. The reverse is
    impossible by construction of the materializer — such groups are
    pulled out of the shared package — and that is exactly why the radius
    of a refusal here equals one group.
    """
...
def refusal_shares(
    ...
    """``(chunk share, op share, whether the threshold is exceeded)``.

    Both measures are mandatory, because one of them always lies: a solo
    chunk carries ONE op, a large one up to 250. Seven failed solo chunks
    out of 28 programs is 25% of chunks and 0.26% of the plan (failing the
    run would be a lie in one direction); three failed large chunks are
    10.7% of chunks and 27.5% of the plan (letting them through would be a
    lie in the other). The threshold takes the maximum of the two.
    """
...
def _refused_without_commit(envelope: Any) -> bool:
    """A KNOWN negative chunk outcome — read from a field, not from text.

    The field is set by exactly the ``serving`` branch that writes the
    ``refused_without_commit`` receipt to the journal (Revit answered, the
    transaction was rolled back, not one id was created). One source of
    truth, two readers: the journal and the orchestrator.

    Parsing ``bridge_detail`` by substring instead would not be allowed:
    ``timeout_unconfirmed`` also arrives with ``ok=false``, but there NO
    answer exists and Revit might have committed — such a chunk must fail
    the run, otherwise fail-soft would silently build the remainder on top
    of an unknown document state.
    """
...
def _bridge_detail(envelope: Any) -> str:
    """The bridge's answer, VERBATIM.

    Truncation is a separate class of loss: on 28.07 a refusal message was
    cut twice (to 300 and to 160 characters) and carried off with it the
    ids of the culprit elements, forcing the live run to be repeated for
    the sake of one line. Here the text is not cut at all: the receipt
    exists precisely so that the next reader does not have to run Revit
    again.
    """

    if isinstance(envelope, Mapping):
        # ORDER MATTERS: in the serving envelope, the `error` field holds
        # a CODE ("rebuild_exec"), while Revit's text lives in
        # `bridge_detail`. Asking `error` first, the receipt would carry a
        # code instead of the evidence.
...
async def run_idempotence(
    ...
    *,
    ...
    # The reason travels ALONGSIDE the count: `isolated_groups` is a
    # number, and it cannot name the reason for emptiness (see
    # `_isolation_source`).
    isolate, isolation_note = _isolation_source(debug_dir, isolate_source_ids)
    try:
    ...
    if dry_run:
        ...
        for index, program in enumerate(planned_programs):
            ...
            # The source catalog is mandatory: without it, grounding does
            # not resolve name-based selectors, and a KIR-G103 refusal
            # talks about the gate's blindness rather than about the
            # program (serving.source_catalogue_snapshot).
    ...
    # The refusal accumulators live OUTSIDE the try: an error at a later
    # phase (re-extract, comparison, cleanup) must not erase refusals
    # already witnessed — otherwise the failure report stays silent about
    # part of the plan that Revit rejected, and the next reader has to run
    # live Revit again for a line we already had.
    ...
    try:
        ...
        for index, program in enumerate(planned_programs):
            ...
            if isinstance(envelope, Mapping):
                # Refusals are collected BEFORE the ok check: a chunk could
                # have committed with failed ops inside it, and that is
                # exactly the case this field was introduced for.
                ...
                # AN OP WITH NO OUTCOME IS UNKNOWNNESS, not a refusal. The
                # same discipline as with timeout_unconfirmed: the chunk
                # committed, but no one knows what happened to the
                # unaccounted ops, and building the remainder on top of
                # this is not allowed. Chunk-level fail-soft deliberately
                # does NOT apply here — the envelope has no `outcome`, so
                # the fatal path fires below.
            ...
            if not (isinstance(envelope, Mapping)
                ...
                if (escrow_record is not None
                    ...
                    continue
                # A CARVE-OUT EXACTLY MATCHING THE PROVEN RADIUS. An
                # isolated program is ONE host group (that is how the
                # materializer built it), so its refusal cannot drag
                # neighbors down with it: there are none in it. A refusal
                # of an ORDINARY chunk is still fatal — there the radius
                # equals the whole program, and "carry on" would mean
                # silently losing up to 250 ops.
                #
                # THE REASON WAS MEASURED (run #3, 28.07): six large
                # chunks Committed (1226 elements), solo #1 Committed,
                # solo #2 rolled back with «Не удалось сформировать тип» —
                # and the run died, and cleanup tore down 1099 elements
                # already built. Exactly one host out of 1200 cost the
                # entire rebuild.
                ...
                if receipt is not None:
                    ...
                    continue
                # CHUNK-LEVEL FAIL-SOFT (29.07). The dividing line is NOT
                # "chunk or solo" but WHETHER THE OUTCOME IS KNOWN. Revit
                # answered, the transaction was rolled back, there are no
                # witnesses — the program is closed, and the run must
                # still reach a number. When there is no answer
                # (`timeout_unconfirmed`) — Revit might have committed,
                # and building on top of an unknown state is not allowed:
                # there it is still a fatal refusal.
                #
                # THE REASON WAS MEASURED (rebuilds #9/#10, 28-29.07): the
                # ninth chunk out of 28 rolled back, the run died, cleanup
                # tore down 2009 elements already built, and there was no
                # comparison at all (comparison_performed=false). Twelve
                # live experiments (E1-E12) failed to find a minimal
                # reproducer — meaning the number must survive an unknown
                # trap.
        ...
        # THE THRESHOLD. "Reach a number" must not turn into "we built a
        # quarter and called it a percentage". The threshold is held ON
        # BOTH measures: by the number of chunks (as agreed) and by the
        # share of ops — seven failed one-op solo programs is 0.26% of the
        # plan, while three failed large chunks is 27%, and one of the two
        # numbers always lies. Exceeding either fails the run entirely.
        if chunk_failures:
        ...
        if escrow_created_ids:
            try:
                ...
                from kir.decompile.schema import EXTRACT_BATCH
        ...
        # The re-lift's side context (live floor evidence 2026-07-21:
        # 12/12 recreated floors were atomized — lift_document with no
        # indices has no sketch profiles; the same fate awaited hosted
        # doors with no family_placement and arcs with no curve). A
        # re-lift must see the SAME side context as the original lift.
        # Every index is fail-open: its absence degrades to honest
        # atomization (as before), but does not crash the measurement.
        ...
        if debug_dir:
            try:
                # Do NOT import json here: the module already imports it at
                # file level, and a repeated `import` inside the function
                # makes the name LOCAL for the whole function — and
                # `json.dumps` in the refusal handler above (the
                # materialize_plan_refused branch) was failing with
                # UnboundLocalError. Measured 02.08 live on the tower:
                # instead of the honest "materialized chunks did not pass
                # the typed KIR plan" with a list of failures, what came
                # back was materialize_failed(UnboundLocalError) — that is,
                # the import's shadow turned a precise diagnosis into a
                # crash and hid the real cause.
                import os as _os
                ...
                with open(_os.path.join(debug_dir, "idempotence_debug.json"),
                    ...
                    json.dump({
                        ...
                        # The curtain-wall index of the COPY. Without it,
                        # the hole "the cell is in re-extraction, but the
                        # leaf is not" cannot be proven post-mortem:
                        # Δ-elements are deleted in cleanup, and there is
                        # nothing left to re-ask a live model. Fail-open:
                        # null is a fact about the run.
        ...
        if recovery is not None:
            await recovery.after_compared({
                **body,
                ...
                # 🔴 THE VOLUME IS NAMED RIGHT NEXT TO THE VERDICT
                # (30.08.2026, E-43). `_compare` on an empty rig honestly
                # gives `multiset_match=True` — empty equals empty — but
                # 100% agreement over NOTHING proves no idempotence. The
                # reader must be able to tell "compared and it matched"
                # from "there was nothing to compare", and here that is
                # said with a NUMBER, not inferred from silence.
                #
                # NOT via `comparison_performed=False`: a run where all the
                # work lives in atoms has an empty op multiset BY
                # CONSTRUCTION, and `a5_recovery` rejects a COMPARED-proof
                # with no comparison — an honest atomic run would fail
                # outright.
                "comparison_volume_zero": (
                    metrics.total_expected == 0 and metrics.total_actual == 0),
                "datums_skipped": datums_skipped,
                "atoms_excluded": atoms_excluded,
                "atoms_escrowed": atoms_escrowed,
                "atoms_form_accepted": form_accepted,
        ...
        if keep_complete_delta:
            # The operator asked to SEE the reconstruction ONCE — we leave
            # the Δ-elements at +200m (visible next to the original),
            # cleanup is not run. Their exact run prefix is returned to
            # the caller; only an explicit preview+confirm call to
            # cleanup_stamps for this run can delete them.
            cleanup_ok, cleanup_detail = True, (
                # The distance is taken from the actual Δ, not from a
                # constant: the offset became a parameter, and a
                # hard-coded "+200m" would tell the operator an untruth
                # about where to look for the copy.
        ...
        if not keep_complete_delta and sweep_runner is not None:
            try:
                ...
                if (not cleanup_ok and sweep_ok and census is not None
                    ...
                    # A per-item pass came up short on elements, while the
                    # census says nothing of OURS is left in the model.
                    # This is not a softening of the law but an answer to
                    # its real question: an element deleted in cascade
                    # along with its host IS ABSENT — exactly what cleanup
                    # was for.
                    cleanup_ok = True
                    cleanup_detail += (
