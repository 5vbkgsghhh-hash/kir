...
def _payload_to_result(payload: dict) -> CompileResult:
    ...
    # 🔴 `is True`, NOT `bool(...)` (2026-09-04). This used to read
    # `bool(payload.get("success", False))`, and that LAUNDERED a falsehood: an
    # entry with `"success": "false"` came back to the caller as an already-
    # honest boolean `True`, indistinguishable from a real success. Measured
    # before the fix: a row out of the LRU memory came back as
    # `CompileResult(success=True, errors=[CS0246])` — "compiled" with a named
    # compile error in the very same object.
    #
    # The only entry this cache is entitled to hand back as a success is one
    # where success is recorded as the boolean `True`; everything else is not
    # a verdict but garbage, and reads as "not a success".
    return CompileResult(success=payload.get("success") is True, errors=errors)
...
class CachedCompileClient:
    ...
    def _lookup(self, key: str) -> Optional[dict]:
        # 🔴 THE OTHER END OF THE SAME CHECK (2026-09-04). `_disk_get` used to
        # run an entry through `_valid_payload`, and memory did NOT: the disk
        # was locked, MEMORY WAS OPEN. The poisoned entry came out of memory
        # exactly for that reason (measured: the first call stores
        # `"success": "false"`, the second hands it right back). `_store`'s
        # lock closes the entrance; this one closes the exit; it is one check,
        # and both ends must call it, or it only guards one of the two doors.
        payload = self._lru_get(key)
        if payload is not None:
            if self._valid_payload(payload):
                return payload
            logger.warning(
                "compile cache: in-memory entry for %s failed validation and "
                "was dropped instead of reused", key)
    ...
    def _store(self, key: str, result: CompileResult) -> None:
        ...
        #
        # 🔴 `is not True`, NOT `not result.success` (2026-09-04). The previous
        # line asked for TRUTHINESS, not a VERDICT, and a verdict of the string
        # `"false"` passed straight through it: a cached falsehood outlives
        # the error itself — on disk it survives both a service restart and
        # the fix for whatever caused it. Only a success recorded as the
        # boolean is cached; anything we did not understand is not cached at
        # all.
