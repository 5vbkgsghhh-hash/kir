...
ELEMENT_REJECTED: dict[str, str] = {
    ...
    # ВТОРОЙ МАРШ — причина СВОЯ, а не «как у лестницы выше». У той концы
    # расставляет Revit, потому что элемент составной; здесь элемент один,
    # но `p0_mm`/`p1_mm` — ЗАКАЗАННАЯ ось марша, а построенный `StairsRun`
    # Revit подгоняет под правила самой лестницы (число подступенков,
    # ширина, стыковка с площадкой). Плюс замер волны второго марша: из
    # `StairsRun` привязка обратно НЕ ЧИТАЕТСЯ ничем, поэтому «адрес
    # построенного элемента» здесь неоткуда взять даже после постройки.
    "create_stairs_run": (
