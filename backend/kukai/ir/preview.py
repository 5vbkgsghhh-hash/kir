...
def _coincident_wall_ids(walls: Mapping[str, Mapping[str, Any]]) -> set[str]:
    ...
    🔴 УРОВЕНЬ — ЧАСТЬ ЛИЧНОСТИ, И ЭТО НЕ ОСТОРОЖНОСТЬ, А ЗАМЕР (15.08.2026).
    На пути ПРОГРАММЫ точка двумерна: `p0_mm`/`p1_mm` несут только план, а
    отметку несёт отдельное поле `level`. Ключ без уровня поэтому склеивает
    типовой этаж сам с собой на всех отметках. Замер на обычной башне —
    5 этажей × 8 стен: **помечено 40 из 40**, то есть единственное наблюдение
    о сборке, доезжавшее до модели, было ложным на 100 % на любом многоэтажном
    здании.

    Путь МОДЕЛИ (`build_model_preview`) этим не болел и болеть не мог: там
    концы трёхмерные и отметка уже внутри точки. Один предикат отвечал на два
    разных вопроса, потому что у входов разная размерность, — тот самый перекос
    входа, который шапка вызова объявляет закрытым.

    Уровень берётся из той же записи, что и геометрия (`walls[oid]["level"]`,
    кладётся `level_key_of(op)` рядом с `p0`/`p1`). Стена без уровня получает
    отдельный сегмент `None` и сравнивается только с такими же: смешивать её
    с уровнем «Этаж 1» значило бы угадывать за автора.
    """
