...
#: Поля, которые компилятор ПРИДЕЛЫВАЕТ САМ, — имя поля -> опы, которым оно
#: принадлежит. **ЭТО ЕДИНСТВЕННАЯ ВЛАСТЬ; читателей четверо.**
#:
#: ПОЧЕМУ ЗАВЕДЕНО (замер 12.08.2026). `__host_wall__` жил ЛИТЕРАЛОМ в
#: четырёх местах — писатель `compiler.hosted_offset_check`, читатели
#: `midend`, `effects`, `authoring`, — и ничто не заставляло их совпадать.
#: Пятое место, где его знать были ОБЯЗАНЫ, литерала не имело: разбор опа
#: (`compiler._validate_op`). Цена ровно этого пробела — этаж со стенами И
#: дверьми не собирался в группу ВООБЩЕ: члены группы проходят план дважды
#: (второй раз из `ground._ground_members`), первый проход приделывал поле,
#: второй отказывал ему KIR-P003 «неизвестное поле». Наш именной класс
#: дефекта в чистом виде — величина рождается в одном месте и перечитывается
#: в другом, и ничто не сводит их вместе.
#:
#: ПОЧЕМУ РАЗБОР ЕГО СНИМАЕТ, А НЕ ПРИНИМАЕТ. Различить «поле приделал наш
#: планировщик» и «поле принесла чужая программа» на входе НЕЧЕМ: JSON
#: выглядит одинаково. Дальше — ровно то, что ПРОВЕРЕНО, без округления в
#: свою пользу:
#:
#:   * поле не имеет авторского имени: принять его на входе значит выдать
#:     языку слот, которого у языка нет, — а слот, которого нельзя написать,
#:     нельзя и перепутать;
#:   * значение ПЕРЕВЫВОДИТСЯ, но НЕ безусловно. Площадок две, и обе с
#:     условием: `compiler._parse_and_check_internal` — когда концы стены
#:     литеральные; `ground` — когда стена адресована И разрешена
#:     (`ground.py:1405-1415`). Путь вне обоих условий я НЕ ПОСТРОИЛ — и
#:     недостижимость его НЕ ДОКАЗАЛ.
#:
#: Отсюда выбор по нашему же порядку лекарств: **убрать возможность дешевле,
#: чем доказывать недостижимость.** Прямая проверка это подтвердила: мутация
#: «принимать поле вместо снятия» НЕ покраснила ни один тест — то есть
#: разница accept/strip на построенных путях НЕ НАБЛЮДАЕМА, и обосновывать
#: снятие «иначе подделка доедет до эмиттера» было бы шире доказанного.
#: Снятие берётся не потому, что доказан вред приёма, а потому, что снятие
#: не требует доказательства вовсе.
#:
#: Там, где хост — существующий элемент (`element_id`), поле не
#: приделывается вообще (`authoring._emit_hosted` читает его только при
#: `host.by == "ref"`), и снимать нечего.
#:
#: Имя вида `__x__` выбрано не для красоты: у языка нет ни одного
#: авторского слота с таким именем, поэтому пересечься с полем реестра
#: нечем — это проверяет `test_synthetic_fields_have_one_authority.py`.
#: Форма хоста (`p0_mm`/`p1_mm`/`arc` стены), снятая планом с НАСТОЯЩЕЙ стены
#: той же программы и переданная эмиттеру. Единственный литерал этого имени
#: во всём дереве — здесь.
