...
ПОРЯДОК ПЛЕЙБУКА — ПО ЗАМЕРЕННОЙ ЧАСТОТЕ, а не по вкусу: 586 живых отказов из
очереди (`kir_rejections.jsonl`, замер 30.07) дают «слишком много опов» 210 раз
(35.8%) — это самый частый отказ системы с большим отрывом, и он стоит первым.
...
#: Ментальная модель, которой у новичка нет. Источники: kukai/ir/CLAUDE.md
...
HOW_A_PRO_STARTS: tuple[str, ...] = (
    ...
    "ЧТЕНИЕ ОКУПАЕТСЯ НЕМЕДЛЕННО. `query_types` стоит доли раунда, а "
    "неугаданный типоразмер — целого. Замер живой очереди: «несколько "
    "вариантов» по window_symbols и levels плюс «уровень не найден» — 64 "
    "отказа из 586, и все они снимаются одним чтением каталога.",
    ...
    f"программы — {MAX_OPS_PER_PROGRAM} операций ДО раскрытия макросов; он мал "
    "намеренно, и его нарушение — самый частый отказ системы (210 из 586). "
    "Повторяющееся собирают макросом, этапы разводят по программам, у каждой "
    "своя квитанция.",
)
...
TECHNIQUES: tuple[tuple[str, str], ...] = (
    ...
    ("СВОБОДНАЯ ФОРМА",
     "Чего не выразить контуром — `create_directshape` треугольным мешем. "
     "Плата названа честно: геометрия БЕЗ BIM-смысла, ни типа, ни параметров, "
     "ни строки в спецификации. Стены, перекрытия, кровли, колонны, балки и "
     "лестницы делают своими операциями всегда, когда задача ими выражается; "
     "категорий walls/floors/roofs/columns у меша нет вовсе, чтобы он не мог "
     "выдать себя за них."),
...
#: Порядок — по замеренной частоте в живой очереди (586 отказов, 30.07).
REFUSAL_PLAYBOOK: tuple[tuple[str, str], ...] = (
    ("KIR-L001 «слишком много опов» — САМЫЙ ЧАСТЫЙ ОТКАЗ СИСТЕМЫ",
     "210 из 586 живых отказов (35.8%), больше чем все остальные вместе. "
     f"Бюджет — {MAX_OPS_PER_PROGRAM} операций ДО раскрытия макросов. Чинится "
     "не уговорами, а формой: регулярное с вычислением — в `program_py`, "
     "тождественный повтор — в макрос, разнородное — в несколько программ. "
     "Бюджет меряет ПЕРЕЧИСЛЕНИЕ, поэтому скрипт под него не попадает: там "
     "перечисляет не автор. Если ты пишешь двадцать первую операцию, ты "
     "выбрал не ту форму ещё в начале."),

    ("ЧИТАЙ СНАЧАЛА `err.fix`",
     "Он собран детерминированно из полей отказа (`op_id`, `field_name`, "
     "`expected`/`got`, `suggested_replacement`) и часто несёт готовую "
     "подстановку — но СВЕРЯЙ `applicability`: `machine-applicable` ставят "
     "как есть, `maybe-incorrect` проверяют по смыслу (см. разбор 3: там "
     "подсказка компилятора построила бы здание неверно)."),

    ("БУКВА КОДА = СТАДИЯ = ЧТО ЧИНИТЬ",
     "P — форма JSON (неизвестное или пропущенное поле). T — тип или предел "
     "значения. G — заземление: селектор не разрешился. L — план и лимиты. "
     "M — макрос. E — операция не поддержана этой версией Revit. X — уже "
     "рантайм: Revit исполнял и отказал."),

    ("G-СЕМЕЙСТВО «несколько вариантов» / «не найдено»",
     "64 из 586 живых отказов. Кандидатов больше одного — умолчание "
     "невозможно, и диагностика УЖЕ несёт `candidates=[{id, name}, ...]`: "
     "повтори тем же вызовом с {\"by\":\"element_id\",\"value\":<id>}. Второй "
     "запрос каталога не нужен. Селектор один на программу — его место в "
     "`defaults`."),

    ("KIR-T002 «вне границ»",
     "Значение вне допустимого диапазона; отказ называет expected и got. "
     "Прежде чем зажимать в границу, спроси, ту ли величину ты задаёшь: "
     "живой случай — `height_offset_mm: 57000` при границах ±15000, где "
     "правильным ответом был отдельный уровень, а не подсказанные 15000."),

    ("KIR-P003 «неизвестное поле»",
     "Схема закрыта намеренно: поля, которого нет, не игнорируют — им "
     "отказывают. Обычно это поле от другой операции; сверься со схемой "
     "именно этого опа, а не с тем, как «должно быть»."),

    ("KIR-X004 «постусловия нарушены»",
     "Revit ПОСТРОИЛ, перечитал и не увидел заявленного — транзакция "
     "откачена, в модели не осталось ничего. 40 таких в живой телеметрии. "
     "Диагностика перечисляет нарушенное с осью: дословно из прогона башни — "
     "«leg_sw: level binding mismatch (topology)». Ось говорит, где искать: "
     "(topology) — привязка к уровню или хосту, (geometry) — координаты, "
     "(semantic) — параметры. Повторять то же самое бессмысленно."),

    ("`kir.unconfirmed` — ЕДИНСТВЕННЫЙ ОТКАЗ, КОТОРЫЙ НЕЛЬЗЯ ПОВТОРЯТЬ",
     "`retryable=false` стоит там не случайно: исполнение не подтвердилось, "
     "но запись МОГЛА закоммититься. Слепой повтор создаст дубликаты в живой "
     "модели. Сначала перечитай модель, потом решай. Остальные отказы KIR "
     "retryable — причина названа, следующий ход её чинит."),

    ("ОТКАЗ С `handoff`",
     "Не твоя ошибка: задача вне области KIR, и назван маршрут (recipe-path). "
     "Переписывать программу бессмысленно."),

    ("`op_refusals` В КВИТАНЦИИ",
     "Дословная причина ПО КАЖДОЙ операции: программа могла отказать "
     "частично. Держи закон переписи — создано + отказано + без элемента == "
     "всего операций; сумма не сошлась — ход не успешен, чем бы ни казался."),

    ("ЧИНИ ТОЧЕЧНО",
     "Отказ называет `op_id` и поле. Остальная программа была верна — "
     "переписывать её целиком значит выбросить оплаченную работу."),
)
...
def build_skill_text() -> str:
    ...
    for title, steps, programs in WALKTHROUGHS:
        out.append("")
        out.append(f"  {title}")
