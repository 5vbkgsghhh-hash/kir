...
TECHNIQUES: tuple[tuple[str, str], ...] = (
    ...
    ("КООРДИНАТЫ И ЕДИНИЦЫ",
     "Всё в МИЛЛИМЕТРАХ — перевод во внутренние единицы принадлежит "
     "компилятору, поэтому переводить самому нечего и незачем. Размерность "
     "точки зависит от операции и названа в схеме: сверься с ней, а не с "
     "привычкой — плоская точка там, где ждут пространственную, это "
     "типизированный отказ, а не мелочь."),
...
REFUSAL_PLAYBOOK: tuple[tuple[str, str], ...] = (
    ("KIR-L001 «слишком много опов» — САМЫЙ ЧАСТЫЙ ОТКАЗ СИСТЕМЫ",
     ...
     "не уговорами, а формой: повторяющееся — в макрос, разнородное — в "
     "несколько программ. Если ты пишешь двадцать первую операцию, ты выбрал "
     "не ту форму ещё в начале."),
