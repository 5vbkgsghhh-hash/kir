...
#: Ментальная модель, которой у новичка нет. Источники: инженерные
#: правила KIR (Mission, The two directions, Invariants, «Compiles is not builds»),
...
#: ПОВТОР, ПОКАЗАННЫЙ, А НЕ ОПИСАННЫЙ (14.08.2026).
#:
#: ЗАМЕР, ВЫНУДИВШИЙ ЭТОТ БЛОК. В постоянном описании (28 218 символов) слово
#: `track` стояло ОДИН раз — прозой, и НИ РАЗУ JSON'ом; полных программ было
#: две, и повтора не показывала ни одна. Раздел про повтор при этом сильный, с
#: числами («160 операций против одного трека»). Для ЛЛМ проработанный пример
#: плотнее абзаца, и соотношение было перевёрнуто: рассказываем — и не
#: показываем ровно то, что дороже всего стоит на настоящем здании.
#:
#: ПРОГРАММА ПРОВЕРЕНА КОМПИЛЯТОРОМ, А НЕ НАПИСАНА ПО ПАМЯТИ: `plan_program`
#: принимает её и разворачивает в 6 опов. Пример, который не компилируется,
#: учит неверному с той же плотностью, с какой верный учит верному.
...
#: ОТКАЗ ЭТОЙ ЖЕ ПРОГРАММЫ, СНЯТЫЙ ПРОГОНОМ, А НЕ СОЧИНЁННЫЙ. Получен из той
#: же программы с треком `x` до узла 5 вместо 6. Пара «оп + его типичный
#: отказ» стоит дороже двух отдельных примеров: модель встретит этот отказ
#: раньше, чем любой другой у `series`, и он называет И предмет, И границу.
...
#: ПОВОД СПРОСИТЬ КОНТРАКТ, А НЕ УПОМИНАНИЕ, ЧТО ТАКАЯ ФУНКЦИЯ ЕСТЬ.
#: Замер 14.08: описание называет `spec(` пять раз, и ОБА субъекта арены
#: вызвали его НОЛЬ раз. Канал построен, материал в нём есть (ловушки опов,
#: замеренные живьём), встречи не происходит — потому что ни одна строка не
#: говорит, КОГДА звать.
SPEC_TRIGGER_RU: str = (
...
#: ПРИЁМЫ, ОСТАЮЩИЕСЯ В ПОСТОЯННОМ ТЕКСТЕ. Род списка — ЗАКРЫТЫЙ, НО НЕ
#: ПОЛНЫЙ: сюда попадает то, что решает ФОРМУ высказывания, и добавление
#: строки требует того же довода, что снятие.
#:
#: Почему эти три, а не другие шесть. «Свободная форма» решает, каким ОПОМ
#: вообще писать (меш против тела против контура); «одиночки и опасные» —
#: какие опы обязаны ехать своей программой, то есть форму ПАЧКИ; «толщины и
#: сечения» — читать ли каталог перед письмом. Ошибка здесь стоит переписи
#: всей программы. Остальные шесть отвечают на «я упёрся в конкретное» —
#: их спрашивают ПОСЛЕ выбора формы, и потому им место в канале спроса.
#: 🔴 «АДРЕСАЦИЯ ВНУТРИ ПРОГРАММЫ» ВЕРНУЛАСЬ СЮДА ПОСЛЕ КРАСНОГО, и это
#: правильный порядок: сначала правило разместило её в канале спроса, потом
#: `test_tool_doc.test_ref_rule_matches_the_compiler` покраснел, потому что
#: правило `ref` (что можно адресовать ссылкой, а `type`/`symbol` — нельзя)
#: решает, КАК пишется программа, а не «что делать, когда упёрся». Ратчет
#: оказался точнее моего разбиения — ровно за этим он и стоит.
...
def build_techniques_text() -> str:
    """Приёмы по ситуациям — ПО ЗАПРОСУ, а не в каждом ходу.

    Дом вытесненного §4 (15.08.2026). Собирается из `TECHNIQUES`, то есть из
    того же объекта, который раньше печатался в описание: второй экземпляр
    разошёлся бы с первым молча, а разойтись ему было бы где — приёмы правят
    чаще любого другого блока курса.
    """
    # ФОРМАТ ОДНОСТРОЧНЫЙ — тот же, каким блок печатался в описании. Первая
    # редакция разнесла ситуацию и совет по строкам с пустой между ними и
    # вышла за потолок урока (3 561 при 3 300), выведенный из канала песочницы
    # и потому неподнимаемый. Красота изложения, не влезающая в канал, — это
    # усечение, а усечённый совет хуже отсутствующего.
    moved = [(s, a) for s, a in TECHNIQUES
             if s not in _TECHNIQUES_IN_PERMANENT_TEXT]
    out = [f"{SECTION_TITLES[3]} — {len(moved)} из {len(TECHNIQUES)}. "
           f"Остальные решают ФОРМУ и стоят в постоянном тексте."]
    for situation, advice in moved:
...
def build_skill_text() -> str:
    ...
    # 🔴 §4 ВЫТЕСНЕН В КАНАЛ СПРОСА 15.08.2026 — по той же причине и тем же
    # приёмом, что литералы §5 шестью днями раньше.
    #
    # ЗАМЕР ДО ПРАВКИ: описание инструмента 30 427 символов при потолке
    # 30 000 — потолок был ПРОБИТ, и не одной правкой, а накоплением: восемь
    # способностей приехали в постоянный текст за один день. Поднять потолок
    # значило бы переложить нашу неаккуратность на бюджет мышления модели,
    # который платится КАЖДЫМ ходом.
    #
    # ПОЧЕМУ ИМЕННО ЭТОТ БЛОК, А НЕ СОСЕДНИЙ. Правило размещения одно:
    # в постоянном тексте стоит то, без чего модель выберет НЕ ТУ ФОРМУ.
    # §1 (устройство), §2 (с чего начинают), §3 (форма повтора) решают именно
    # форму и остаются. §4 — ситуационный справочник «попал в X — делай Y»:
    # он нужен ПОСЛЕ того, как форма выбрана и автор упёрся в конкретное, то
    # есть ровно тогда, когда его можно спросить. Это не сокращение курса:
    # текст целиком доступен одной строкой `course("приёмы")`, и указатель
    # ниже называет её в том же месте, где раньше стоял сам блок.
    out.append("")
    ...
    for situation, advice in kept:
        out.append(f"  • {situation} — {advice}")
    out.append(f"  • ОСТАЛЬНЫЕ {len(TECHNIQUES) - len(kept)} ПРИЁМОВ "
               f"(адресация в программе, тип и семейство, пределы макросов, "
               f"уровни, хост, координаты) — course(\"приёмы\"), печатает в "
               f"квитанцию этого хода.")
    ...
    # ПОВТОР ПОКАЗАН, А НЕ ТОЛЬКО ОПИСАН. Стоит здесь, а не в `course`, по той
    # же причине, что и пара выше: предмет — САМ ТЕКСТ программы, и вынести его
    # в канал спроса значило бы вынести то, ради чего блок написан. Цена
    # замерена ПОСЛЕ письма, а не оценкой: описание 28 218 -> 29 565 символов
    # (+1 347, +4.8% описания; +1.8% хода при схеме 45 216, то есть ~422 токена
    # при поставщицких 3.19 симв/токен). Потолок, объявленный до письма, был
    # 3 000 символов — уложились вдвое.
