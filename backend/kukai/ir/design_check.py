...
def spatial_model_from_program(
    ...
    *,
    ...
    # 🔴 РАЗБИЕНИЕ СТРОИТСЯ ПО УРОВНЯМ СО СТЕНАМИ, А НЕ ПО УРОВНЯМ С КОМНАТАМИ.
    #
    # Прежняя редакция шла `room_ops.items()`, то есть замкнутость считалась
    # ТОЛЬКО там, где о ней попросила комната. Но замкнутость есть факт о
    # СТЕНАХ, а не о комнатах, и от этого зависел целый класс ответов.
    #
    # Замер 15.08.2026, четыре программы, отличающиеся ровно сборкой:
    #
    #     4 стены замкнуто, комнат нет     правил 0/20, HAB000 «нет помещений»
    #     4 стены дыра 1500 мм, комнат нет правил 0/20, HAB000 — ТО ЖЕ САМОЕ
    #     4 стены замкнуто + комната       правил 8/20
    #     4 стены дыра + комната           правил 5/20, HAB060 «нет измеримого
    #                                      контура» — дефект НАЗВАН
    #
    # То есть аппарат умеет видеть незамкнутый контур и молчит ровно в том
    # случае, в котором сломалась «полосатая стена»: стены есть, комнат нет.
    # Отдельным замером тем же прибором: замкнутая коробка даёт 1 грань 24 м² и
    # называет ЧЕТЫРЕ стены, её замкнувшие; дыра в 1500 мм — 0 граней.
    #
    # Правило НЕ добавлено и вердикт НЕ меняется: без комнат правилам судить
    # по-прежнему нечего. Меняется одно — свидетель теперь НЕСЁТ ЧИСЛО
    # (`partition_faces`), которого раньше не существовало, и по нему можно
    # сказать «твои стены ничего не замкнули». Поле было в `BuildWitness` с
    # самого начала и просто не заполнялось на этом пути.
    for level_id in sorted(set(room_ops) | set(segs_by_level)):
        level_rooms = room_ops.get(level_id, ())
        if level_id not in elevations:
            # ЗАЩИТНАЯ СТРОКА, И ЭТО НАЗВАНО ЧЕСТНО: недостижима на сегодняшнем
            # входе. Стена, чей уровень не разрешается по программе, до
            # `segs_by_level` не доходит вовсе — её отбрасывает чтение выше,
            # с уже названной причиной `wall_level_unresolved` (`BuildNote`).
            #
            # Первая редакция этой правки завела здесь ВТОРУЮ причину
            # (`level_elevation_unknown`) для того, у чего причина уже есть, —
            # и тест это поймал. Два имени одного факта разошлись бы на первом
            # же читателе, который считает по одному из них.
            continue
