...
def instrument_refusal_line(refused: int, subject: str = "настоящий документ",
                            why: str = "") -> str:
    """Машинная строка об отказе ПРИБОРА — ТОТАЛЬНАЯ, ноль тоже печатается.

    Решение владельца 16.08.2026: ворота обязаны различать три исхода —
    зелено · красно · ОТКАЗ ПРИБОРА, — и третий невозможно спутать с первым.
    Отсюда тотальность: функция не возвращает пустую строку НИ ПРИ КАКОМ
    входе, потому что отсутствие строки читается как «вопрос не задавали»,
    а ноль — как «задали, отвечать нечем».

    Вынесена из двух веток печати именно для того, чтобы у контроля был
    предмет: ветку с живым корпусом офлайн не исполнить (нужен и корпус, и
    Roslyn), а тотальность помощника проверяется без единой зависимости.
    """
...
async def main() -> int:
    ...
    if not real_profiles:
        ...
        # 🔴 МАШИННАЯ СТРОКА, ПЕЧАТАЕМАЯ В ОБЕИХ ВЕТКАХ, ВКЛЮЧАЯ НОЛЬ.
        # Прозу выше человек читает, а сводку прогона собирает греп, и он
        # обязан отличить «не проверяли» от «проверили и чисто» БЕЗ разбора
        # русского текста. Решение владельца 16.08.2026: отказ прибора
        # обязан быть невозможно спутать с зелёным, поэтому счётчик есть
        # всегда, а не только когда он ненулевой.
        ...
        # Парная строка к отказу выше. Ноль печатается ИМЕННО ЗДЕСЬ, а не
        # опускается: читающий сводку обязан видеть, что вопрос ЗАДАВАЛСЯ и
        # ответ был «нечего добавить», а не что строки нет вовсе.
