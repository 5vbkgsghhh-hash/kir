...
def _forms(ospec: spec.OpSpec, p: spec.ParamSpec) -> tuple[str, ...]:
    ...
    if p.kind == "sel_list":
        # Множественное число рода `sel`: те же формы, что у одиночного, минус
        # `family_type` (он живёт только у place_family.symbol и списком не
        # бывает). Своя ветка, а не объединение с `sel`, потому что автору
        # надо СКАЗАТЬ, что здесь список — это и делает вызывающий.
        forms = ["name", "element_id", "default"]
        if p.ref_kinds:
            forms.append("ref")
        return tuple(forms)
...
def _by_name_next_move(ospec: spec.OpSpec, p: spec.ParamSpec,
    ...
    if ospec.name in spec.SOLO_OPS:
        # ДВА СОЛО-ОПА — ДВА РАЗНЫХ СЛЕДУЮЩИХ ХОДА, И ЭТО НЕ ВЕЖЛИВОСТЬ
        # (10.08.2026). Совет «сошлись ПО ИМЕНИ» верен для уровня марша:
        # `base_level` — селектор рода `sel`, у него форма `name` есть. У
        # площадки же ссылка попадает в `stairs` рода `target_w`, а у него
        # формы `name` НЕТ И БЫТЬ НЕ МОЖЕТ (`_forms`: цель записи
        # разрешается до эмиссии, а не в C#). Прежний текст посылал бы
        # автора писать `stairs="Этаж 1"` — совет, ведущий во второй отказ
        # подряд, то есть ровно та «яма», ради которой замер 04.08 этот
        # помощник и завёл.
        if p.kind in ("target_w", "refs_w", "targets_w"):
...
#: Как заполняется слот-селектор — ОДНО определение на двух читателей.
#:
#: Читатель первый — отказ языка (`_selector_error` ниже). Читатель второй —
#: документация модели (`tool_doc._selector_forms_in_python`). Раньше это
#: жило ЛОКАЛЬНОЙ переменной внутри отказа, и дока, не имея к ней доступа,
#: описывала диалект СВОИМИ словами: двадцать пять ручных словарей-селекторов
#: в отрендеренном тексте и ни одного упоминания короткой формы, которую
#: язык принимает с самого начала. Классическое «объявлено здесь, прочитано
#: там»; лечится не сверкой двух копий, а тем, что копия одна.
#:
#: Порядок ключей — от САМОЙ КОРОТКОЙ формы к самой громоздкой: читатель
#: берёт первую, и первой должна лежать та, которую не в чем ошибиться.
...
def _docstring(ospec: spec.OpSpec) -> str:
    ...
    if not ospec.params:
        lines.append("    (нет)")
    # РЕЕСТР МОЖЕТ НЕ ЗНАТЬ, ЧТО ОБЯЗАТЕЛЬНО, И ТОГДА ОБЯЗАН СКАЗАТЬ ЭТО ВСЛУХ.
    #
    # ЗАМЕР 13.08.2026 (аудит оси языка). `p.required` выражает обязательность
    # БЕЗУСЛОВНУЮ, и другой у реестра нет: у `OpSpec` одиннадцать полей, и ни
    # одно не выражает отношения между параметрами. Условная обязательность
    # («точка требует level, кривая требует host») живёт рукописными ветвями
    # компилятора и в реестр не поднимается.
    #
    # Для опа, у которого обязательным не объявлен НИ ОДИН параметр, этот
    # текст читался как «можно вызвать пустым» — и это НЕВЕРНО, а не неполно:
    # `place_family` с одним `symbol` отвергается `KIR-P007` («не задано
    # положение»). Такой оп в реестре ровно один из 69 (замерено), но условие
    # проверяется ВЫВОДОМ, а не именем: следующий такой оп получит эту строку
    # сам, без правки здесь.
    #
    # Почему сказано ровно столько. Назвать ЗДЕСЬ, какие поля условны, значило
    # бы выписать второй экземпляр правила, живущего в компиляторе, — тот самый
    # дефект, ради запрета которого существует `SOLO_OPS` одной строкой. Само
    # правило приезжает `OP_NOTES` ниже, у него один источник.
    ...
    # ПРИЧИНА «НЕ ОПИРАЙСЯ БЕЗ ПРОВЕРКИ» ЖИВЁТ ЗДЕСЬ, А НЕ В ОПИСАНИИ
    # ИНСТРУМЕНТА (09.08.2026, слияние ночных волн).
    #
    # Замер, вынудивший перенос: на 62 операциях описание выросло до 30 440
    # при потолке 30 000, и потолок не вкусовой — он равен объявленным
    # ~10 000 токенов контекста модели (3.00 символа на токен, замерено на
    # этом же тексте). Докстрока самого теста запрещает его поднимать:
    # «дальше текст обязан не расти, а вытеснять сам себя».
    #
    # Вытеснение сделано по цене, а не по важности. СПИСОК непроверенных опов
    # остаётся в описании — модель обязана знать, что оп непроверен, ещё до
    # того, как выберет его. А ПРИЧИНА нужна ровно тогда, когда модель уже
    # спросила про этот оп, и приезжает тем же `spec(op)` / `__doc__`, что и
    # весь остальной контракт. Постоянно загружаемый текст платится КАЖДЫЙ
    # ход; докстрока — только на том ходу, где её спросили.
    #
    # Импорт ленивый нарочно: `tool_doc` тянет `skill`, а тот `macros` и
    # `compiler`, и модульный импорт здесь замкнул бы граф.
    from kukai.ir.tool_doc import OP_NOTES, UNPROVEN
    # ЛОВУШКИ САМОГО ОПА — ВТОРАЯ ПОЛОВИНА ТОГО ЖЕ ХОДА, ЧТО И ПРИЧИНА НИЖЕ
    # (09.08.2026). Ловушка, которая называет один оп и нужна только после
    # того, как он ВЫБРАН (устройство группы, требование конверта у `delete`,
    # семейство отвода у трассы), стояла в описании инструмента и платилась
    # КАЖДЫМ ходом — включая все те, где этого опа в программе нет. Здесь она
    # платится ровно тем ходом, где её спросили. Ни один замер при переезде не
    # потерян: `test_tool_doc` держит обе стороны шва — текст обязан БЫТЬ
    # здесь и обязан ОТСУТСТВОВАТЬ в описании.
    for note in OP_NOTES.get(ospec.name, ()):
        lines += ["", "ЛОВУШКА ЭТОГО ОПА (замерена живьём):", f"    {note}"]
    entry = UNPROVEN.get(ospec.name)
    if entry is not None:
        # `.reason`, а не сама запись: журнал везёт ещё вердикт и две даты —
        # это учёт ДЛЯ НАС, и модели он не адресован. Печать записи целиком
        # отдала бы автору `Entry(verdict=…, decided_on=…)` вместо совета.
...
def _call_head(ospec: spec.OpSpec) -> str:
    """ОДНА строка вызова: обязательные, `*`, необязательные, `id`.

    Вынесена из `_call_form` затем, что её просит второй читатель —
    `course.spec()`, печатающий контракт опа по запросу модели. Собранная там
    заново, эта строка была бы вторым мнением о порядке слотов и разошлась бы
    с отказом `_bind_refusal` ровно тогда, когда оба текста читают подряд.
    """
