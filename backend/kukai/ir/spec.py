...
# Журнал пробелов — тот же примитив, что у приёмки и у тёмных модулей: форма
# строки проверяется ПРИ ИМПОРТЕ, поэтому запись без даты, срока и причины
# физически не заводится. `record_ratchet` не импортирует ни реестра, ни
# приёмки (проверено: у него только datetime и typing), цикла нет.
# ИМЕНА БЕЗ ПСЕВДОНИМОВ, И ЭТО НЕ СТИЛЬ. Храповик журналов находит объявления
# СКАНОМ ИСХОДНИКА по образцу `ИМЯ = Ledger(` (tests/test_record_ratchet.py).
# Первая редакция этой строки импортировала `Ledger as _Ledger` — журнал
# построился, зарегистрировался в `ALL_LEDGERS` и СТАРЕЛ БЫ МОЛЧА: сканер его
# не видел, а тест, который это ловит, в тот момент падал по чужой причине
# (утечка env в conftest). Ровно седьмая общая форма, поймавшая своего же
# автора через час после того, как он её записал.
from kukai.ir.record_ratchet import CLOSE_BY, Entry, Ledger  # noqa: E402
...
#: `create_stairs_run` (15.08.2026) — ТРЕТИЙ жилец, и он здесь по ЗАМЕРУ, а не
#: по симметрии с площадкой. RevitAPI.xml пишет у `StairsRun.CreateStraightRun`
#: дословно и одинаково на всех шести версиях: `InvalidOperationException` —
#: «The stairs element represented by stairsId is not in an active
#: StairsEditScope. New components cannot be added to it.» Та же фраза, что у
#: `CreateSketchedLanding`, и тот же вывод: маршу нужна область правки, а две
#: области одновременно Revit не открывает («there already is a stairs edit
#: mode active in the document»). Значит второй марш — ОТДЕЛЬНАЯ ПРОГРАММА, и
#: невыразимость соседства двух лестничных опов есть закон REVIT, а не наш
#: вкус. Открыть область на УЖЕ СТОЯЩЕЙ лестнице позволяет одноаргументный
#: `StairsEditScope.Start(ElementId)` (6/6), тот же, которым живёт площадка.
SOLO_OPS: frozenset[str] = frozenset({
    "create_stairs", "create_stairs_landing", "create_stairs_run"})

#: ВЕРСИОННО-ХРУПКИЕ ПАРЫ (оп, ПОЛЕ) — ОТДЕЛЬНЫЙ СПИСОК, А НЕ ЖИЛЬЦЫ `SOLO_OPS`.
#:
#: РОД СПИСКА: **ЗАКРЫТЫЙ, НО НЕ ПОЛНЫЙ.** Выведен из прогона ворот
#: `k2_ar_rd_v7` от 2026-08-13 (`tools/compile_gate_offline.py`, 889 проверок).
#: Полон ОТНОСИТЕЛЬНО ЭТОГО ПРОГОНА и НЕ полон относительно API: оп, чья
#: эмиссия откажет на паре, здесь не названной, поедет в общем куске и унесёт
#: соседей. Храповик на это — `test_version_fragile_asks_the_emitter.py`.
#:
#: 🔴 ПОПОЛНЕН 15.08.2026 ПЯТЬЮ ПАРАМИ, И ИСТОЧНИК СМЕНИЛСЯ. Прежний ратчет
#: обходил РОВНО `authoring.py`, а тела 32 эмиттеров из 63 уехали в двенадцать
#: спутников `*_emit.py`, оставив четырёхстрочные обёртки. Метод был верен —
#: спросить место определения, — а охват стал 6 % нового кода: четыре места
#: отказа по версии были невидимы, три не названы нигде. Ратчет теперь ходит
#: по `authoring` И по спутникам, привязывая место к опу ПО ЦЕПОЧКЕ ВЫЗОВОВ
#: (два места из четырёх лежат в частных помощниках, а `struct_emit`
#: обслуживает шесть опов — приписать место всему модулю значило бы объявить
#: хрупкими пять непричастных).
#:
#: ЦЕНА ПОПОЛНЕНИЯ ЗАМЕРЕНА, А НЕ ПРЕДПОЛОЖЕНА. Единственный вызывающий —
#: `decompile/materialize.py:1830` (нарезка чанка). Четыре из пяти опов в
#: `decompile/` не упоминаются НИ РАЗУ, то есть материализатор их не породит
#: по построению; `create_foundation` упоминается, но по корпусу из 52
#: деревьев встречается **0 раз**. Итог: пополнение стоит ноль чанков на
#: замеренных данных и ограничивает потерю до 249 соседей там, где оп
#: появится.
#:
#: ПОЧЕМУ НЕ В `SOLO_OPS`. Тот список — множество ИМЁН, и его обоснование
#: своё: области правки лестницы (`StairsEditScope`). Здесь причина другая —
#: отсутствующая перегрузка API на старой версии, — и ключ другой: ПОЛЕ, а не
#: имя. Замерено 13.08 на башне: по полю соло уезжают 125 опов, по имени 333.
#: Слить два рода в один предикат значило бы потерять различие, которое
#: понадобится на первом же не-версионном отказе.
#:
#: ЗАЧЕМ ВООБЩЕ. Отказ эмиссии роняет ПРОГРАММУ целиком, а размер куска (250)
#: назначен нами. Замерено: ТРИ опа, невыразимых на Revit 2021, унесли с собой
#: 2 742 совместимых — отношение 1 : 914. Соло-нарезка возвращает их ценой
#: ~125 дополнительных программ, то есть 22 возвращённых опа за программу.
#:
#: ЧЕСТНАЯ ОГОВОРКА: 81 из 125 — `create_ceiling`, и они соло ВСЁ РАВНО не
#: соберутся на 2021 (`Ceiling.Create` с 2022, legacy-пути нет ни на одной из
#: шести). Выигрыш не в том, что потолки построятся, а в том, что они
#: перестанут уносить соседей.
#:
#: ПОЛЕ `None` означает «оп хрупок целиком, независимо от параметров».
VERSION_FRAGILE: frozenset[tuple[str, str | None]] = frozenset({
    ("create_ceiling", None),
    ("create_floor", "holes"),
    ("create_floor_by_contour", "contour.holes"),
    # ── добор 15.08.2026: спутники `*_emit.py`, найдено обходом, не глазами ──
    # ТА ЖЕ ПАРА, ЧТО У ПЕРЕКРЫТИЯ, И ЭТО СКАЗАНО В САМОМ ЭМИТТЕРЕ:
    # `struct_emit._emit_foundation_slab` документирует «the SAME
    # EMIT_UNSUPPORTED refusal create_floor gives for holes pre-2022»
    # (`Floor.Create` с отверстиями — с 2022, у `NewFloor` их нет).
    # Плита фундамента и есть структурное перекрытие по контуру, поэтому
    # отсутствие этой пары рядом с двумя соседними было пробелом учёта,
    # а не решением (`struct_emit.py:227`).
    ("create_foundation", "holes"),
    # ОТКАЗ НА ЧЕТЫРЁХ ВЕРСИЯХ ИЗ ШЕСТИ (`ver < "2025"`, `datum_emit.py:511`):
    # весь API многоэтажного марша типизирован `ISet<ElementId>`, а
    # развёрнутый на флоте плагин на net48 не связывает `System.dll` — тело
    # не соберётся У ПОЛЬЗОВАТЕЛЯ (CS0012), хотя собирается у нас. Оп с
    # отказом на 2/3 поддерживаемых версий обязан ехать соло: иначе он
    # уносит чанк на каждой из четырёх.
    ("create_multistory_stairs", "levels"),
    # ТРИ НАГРУЗКИ КР — ХРУПКИ ЦЕЛИКОМ (поле `None`), а не по параметру:
    # `analysis_emit._version_guard` отказывает `KIR-E003` при
    # `ver > _FREE_LOAD_LAST_VER` («2023»), то есть на 2024/2025/2026 —
    # ПОЛОВИНЕ поддерживаемых версий. Развилки эмиссии нет по построению:
    # альтернативы — построить нагрузку на чужом носителе либо промолчать,
    # и обе читаются снаружи как успех. Поле `None`, потому что решает
    # версия, а не параметр: guard стоит ДО разбора чего бы то ни было.
...
def is_version_fragile(op_name: str, params: Mapping[str, Any] | None) -> bool:
    """Хрупок ли ЭТОТ оп с ЭТИМИ параметрами — а не «есть ли имя в списке».

    Ключ пары — ПОЛЕ, поэтому `create_floor` без `holes` едет в общем куске
    вместе со 154 своими собратьями, а не уходит соло из-за имени.
    """
    values = params or {}
...
# ═════════════════════════════════════════════════════════════════════════
# ЧИТАЮЩАЯ СТОРОНА ОТСТАЁТ ОТ ПИШУЩЕЙ, И ОТСТАВАНИЕ ТЕПЕРЬ ЗАМКНУТО
#
# ЗАМЕР 12.08.2026. Пишущие опы реестра порождают 61 категорию переписи;
# носители словаря разделов (`KINDS[*].collector_cs` и
# `extract._CATEGORY_SPECS`) называют 42 из них. Оставшиеся 19 — ниже.
# Следствие видно снаружи: 22 пишущих опа из 65 не получают раздела вовсе,
# и семь из них — КР (три нагрузки, ферма, балочная система, армирование
# площади, ленточный фундамент). Автор раздела КР видит в своём списке ТРИ
# операции, а остальные семь стоят под заголовком «раздел не выведен».
#
# ЭТО ТОТ ЖЕ КЛАСС, ЧТО И `query_types.pool`, ПРОСТО В ДРУГОЙ ВАЛЮТЕ.
# Там KIR умеет ПИСАТЬ в шесть пулов, которых не умеет ЧИТАТЬ; здесь он
# умеет писать в 19 категорий, которых не называет ни один читающий
# носитель. Обе стороны — списки, которые ведут порознь, и ничто не
# заставляло их сойтись. Средство одно и то же: множество, которое обязана
# покрыть читающая сторона, ВЫВОДИТСЯ из пишущей, а всякий непокрытый член
# — ИМЕНОВАННЫЙ отказ. Пустого третьего варианта нет, и это держит тест
# (`tests/test_write_side_read_side_parity.py`).
#
# ЧЕГО ЭТОТ ЖУРНАЛ НЕ ДЕЛАЕТ. Он не трогает СУДЬЮ ПРИЁМКИ: категорию
# результата судья знает точно у всех трёх нагрузок (`exact`, верхние и
# нижние границы целы — проверено `derive_expectation` на программах, уже
# принятых воротами 6/6). Третья опора кардинального инварианта у них НЕ
# потеряна, и заявлять обратное было бы враньём в дорогую сторону. Слепота
# судьи — отдельный, УЖЕ замкнутый учёт: `acceptance._OPS_BLIND` плюс
# `tests/test_registry_category_accounting.py`, где неназванных ноль.
#
# ПОЧЕМУ НЕЛЬЗЯ ПРОСТО ДОПИСАТЬ СТРОКИ В ТАБЛИЦУ ИЗВЛЕЧЕНИЯ. Строка там
# утверждает, что конвейер эту категорию ЧИТАЕТ. Дописать её без
# экстрактора значит завести ровно тот дефект, ради запрета которого этот
# журнал и существует: величина объявлена в одном месте и читается в
# другом, и ничто не заставляет их совпасть.
# ═════════════════════════════════════════════════════════════════════════

#: Категория -> почему её раздел не выводится. Ключ — категория, а не оп:
#: оп получает раздел через свои категории, и решение принимается там, где
#: пробел настоящий.
CATEGORIES_WITHOUT_DISCIPLINE = Ledger(
    "spec.CATEGORIES_WITHOUT_DISCIPLINE",
    {
        # ── Аналитические нагрузки: у конвейера нет стадии расчётной модели.
        # Категорию РЕЗУЛЬТАТА судья приёмки знает точно у всех трёх (её
        # задаёт сам класс API), поэтому здесь пробел ровно один: читать их
        # некому.
        "OST_PointLoads": Entry(
            CLOSE_BY, "2026-08-12", "2026-09-11",
            "create_point_load: у таблицы извлечения нет ни одной строки "
            "расчётной модели, поэтому раздел КР этой категории вывести "
            "неоткуда; закрывает строка + экстрактор нагрузок"),
        "OST_LineLoads": Entry(
            CLOSE_BY, "2026-08-12", "2026-09-11",
            "create_line_load: та же стадия расчётной модели, которой у "
            "конвейера нет; отдельная строка, потому что решение "
            "принимается по категории, а не по стадии"),
        "OST_AreaLoads": Entry(
            CLOSE_BY, "2026-08-12", "2026-09-11",
            "create_area_load: та же отсутствующая стадия расчётной модели; "
            "оп при этом эмитируется на пяти версиях из шести (KIR-E003 на "
            "2024 — перегрузка API, а не наш пробел)"),
        # ── Площадка и рельеф: стадии участка у конвейера нет вовсе.
        "OST_Topography": Entry(
            CLOSE_BY, "2026-08-12", "2026-09-11",
            "create_topography(surface) и create_site_subregion: стадии "
            "участка в таблице извлечения нет ни одной строки, поэтому "
            "поверхность рельефа не читается ни одним разбором"),
        "OST_Toposolid": Entry(
            CLOSE_BY, "2026-08-12", "2026-09-11",
        ...
        "OST_BuildingPad": Entry(
            CLOSE_BY, "2026-08-12", "2026-09-11",
            "create_building_pad: та же отсутствующая стадия участка; пул "
            "типов площадки вдобавок не читается `query_types` — тот же "
            "разрыв, что и здесь, но в валюте пулов"),
        "OST_Site": Entry(
            CLOSE_BY, "2026-08-12", "2026-09-11",
        ...
        # ── Профили по хозяину: стадии обходов у конвейера нет.
        "OST_Cornices": Entry(
            CLOSE_BY, "2026-08-12", "2026-09-11",
            "create_wall_sweep(карниз): стенные и краевые профили не "
            "извлекаются ни одной строкой таблицы, хотя оп эмитируется на "
            "всех шести версиях"),
        "OST_Reveals": Entry(
            CLOSE_BY, "2026-08-12", "2026-09-11",
            "create_wall_sweep(руст): вторая категория того же опа, и "
            "решает её КАТЕГОРИЯ РАЗРЕШЁННОГО ТИПА, то есть выбрать одну "
            "нельзя ни здесь, ни у судьи"),
        "OST_EdgeSlab": Entry(
            CLOSE_BY, "2026-08-12", "2026-09-11",
            "create_slab_edge: краевой профиль перекрытия той же "
            "отсутствующей стадии обходов; категория у судьи известна "
            "точно, не читается только снаружи"),
        # ── Плоская детализация: живёт в ВИДЕ, а разборы читают модель.
        "OST_FilledRegion": Entry(
            CLOSE_BY, "2026-08-12", "2026-09-11",
            "create_filled_region: заливка принадлежит виду, а таблица "
            "извлечения собрана по модельным категориям; закрывает стадия "
            "видовой детализации, а не строка"),
        "OST_MaskingRegion": Entry(
            CLOSE_BY, "2026-08-12", "2026-09-11",
            "create_filled_region: маскирующая область — ТОТ ЖЕ "
            "FilledRegion с другим типом, и `IsMasking` читается только "
            "после создания, поэтому строк здесь две, а не одна"),
        "OST_PathOfTravelLines": Entry(
            CLOSE_BY, "2026-08-12", "2026-09-11",
            "create_path_of_travel: путь эвакуации тоже принадлежит виду; "
            "у судьи приёмки категория известна точно, а читающего "
            "носителя нет ни одного"),
        # ── Лестничное хозяйство: таблица знает марш и ограждение марша,
        # но не контейнер и не площадку.
        ...
        "OST_StairsLandings": Entry(
            CLOSE_BY, "2026-08-12", "2026-09-11",
            "create_stairs_landing: площадка — компонент на уже стоящем "
            "хозяине, и таблица извлечения строки для неё не имеет, хотя "
            "марш читает"),
        "OST_Railings": Entry(
            CLOSE_BY, "2026-08-12", "2026-09-11",
...
# Ниже 44 строки на 66 пишущих опов, и недостающий 22 — не пробел: КАЖДЫЙ
...
#      у каждого причина словами и дата                             11
