...
def toposolid_version_refusal(op, ver: str):
    """Отказ по версии для толщи рельефа — или None. Живёт РЯДОМ с порогом.

    ЗАЧЕМ ОТДЕЛЬНОЙ ФУНКЦИЕЙ, А НЕ СТРОКОЙ В ЭМИТТЕРЕ (13.08.2026, найдено
    ЖИВЫМ прогоном на Revit 2023). Проверка стояла ТОЛЬКО в эмиссии, а
    заземление пула бежит раньше — и на 2023 автор получал

        KIR-G104  «toposolid_types: пусто в модели»

    вместо правды. Пул там пуст НЕ потому, что в документе не завели тип, а
    потому, что на Revit 2021-2023 толщ рельефа не существует вовсе: класса
    `ToposolidType` нет в эталонных сборках этих трёх версий (проверено по
    поверхности API; `open_model.py` знал это всё время и обходил через
    сравнение имени типа СТРОКОЙ — иначе снапшот не собрался бы).

    Цена была не в неточности, а в невыполнимости совета: «пусто в модели»
    читается как «заведи тип», и ЛЛМ уходила заводить тип толщи рельефа на
    версии, где толщ рельефа не бывает. Ни одна из названных дорог не вела.

    Правило, которое отсюда следует и стоит шире этого опа: **версия — факт о
    мире, пул — факт о документе; когда первое ОБЪЯСНЯЕТ второе, отвечает
    первое.** Замерено, что таких пулов ровно ОДИН из 36, поэтому здесь
    адресная функция, а не механизм: таблица «способность → версия» стала бы
    третьей копией знания, уже живущего в эмиттерах.
    """
    from kukai.ir.authoring import EMIT_UNSUPPORTED
