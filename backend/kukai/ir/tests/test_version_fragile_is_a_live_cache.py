...
class TheCacheAgreesWithTheGateInBothDirections(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        #: Опы, которые прогон за этим ответом ДЕЙСТВИТЕЛЬНО содержал
        #: (`facts.op_histogram` его дерева). Область охвата авторитета.
    ...
    def test_no_pair_outlived_its_reason(self):
        ...
        🔴 СУДИТСЯ ТОЛЬКО ТО, ЧТО ПРОГОН МОГ НАБЛЮДАТЬ (поправка 15.08.2026).

        Ответ ворот снят с ОДНОГО здания (`k2_ar_rd_v7`, 150 программ,
        3 отказа), и его `facts.op_histogram` называет **13 опов**. Пары, чьего
        опа в прогоне не было вовсе, этот ответ судить не может: ноль отказов
        там означает «не с чем было отказать», а не «ворота не отказывают».

        Замер, ради которого поправка и появилась: добор `VERSION_FRAGILE`
        пятью парами из спутников `*_emit.py` (фундамент с отверстиями, марш
        многоэтажной лестницы, три нагрузки КР) окрасил эту проверку — при том
        что ни одного из пяти опов в здании НЕТ. Обвинение было о ДАННЫХ, а
        читалось как о списке; это форма 4 канона — ноль из корпуса требует
        доказательства, что корпус был достижим.

        Сторона «пропущено» поправки НЕ требует и не получает: там ворота
        ГОВОРЯТ, а список молчит, и молчание списка виновно при любом составе
        прогона.
        """
        ...
        # Непросуженное НЕ молчит: иначе область охвата тихо расширялась бы
        # каждой новой парой, и однажды судить было бы нечего.
        if unjudgeable:
    ...
    def test_the_scope_of_the_answer_is_declared_and_non_empty(self):
        """Охват обязан быть ОБЪЯВЛЕН. Ответ без списка своих опов позволяет
        предыдущей проверке судить на неизвестной области — и она позеленеет
        или покраснеет по причине, которой никто не увидит."""
        self.assertTrue(
            self.ops_in_run,
        ...
        # Все пары, которые ответ НАЗЫВАЕТ, обязаны лежать внутри охвата —
        # иначе охват описывает не тот прогон, из которого взят ответ.
