...
class ThePlanDigestCannotSeeIdentityProofs(unittest.TestCase):
    ...
    def test_the_surviving_arms_are_the_ones_that_can_fire(self):
        ...
        # КОММЕНТАРИИ ВЫРЕЗАНЫ, И ЭТО ВАЖНО. Прежняя версия искала снятое
        # выражение в СЫРОМ исходнике — а комментарий на его месте цитирует
        # его дословно, поэтому проверка проходила бы и при возврате кода.
        # Хуже: она была привязана к переносу строки, то есть ломалась от
        # переформатирования и молчала о сути.
        code = "\n".join(
            line for line in inspect.getsource(
                S._handle_revit_ir_inner).splitlines()
            if not line.lstrip().startswith("#"))
        self.assertIn("if guarded_out is None or not guarded_out.ok:", code)
        self.assertNotIn("guarded_out.planned", code,
