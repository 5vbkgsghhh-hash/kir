...
class LevelGuardMustNotClaimDriftForAWrongType(unittest.TestCase):
    ...
    def test_guard_distinguishes_absent_from_wrong_type(self):
        ...
        cs = self._beam_cs()
        # Класс читается помощником __ClassName из преамбулы, а не
        # обращением к среде выполнения за типом: та форма записи
        # отвергается валидатором моста версий до 06.07.2026 (живой
        # отказ 04.08.2026). Контракт прежний — ВТОРАЯ ветка обязана
        # назвать реальный класс, иначе отличить «исчез» от «не тот
        # класс» снова будет неоткуда.
