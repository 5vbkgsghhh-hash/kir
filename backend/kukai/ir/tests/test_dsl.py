...
from kukai.ir.tests.test_sdk import _SAMPLES, _sample  # noqa: E402
...
_EXTRA: dict[str, dict] = {
    ...
    # create_ceiling (09.08): форма задаётся ЛИБО прямой ломаной `outline`,
    # ЛИБО эскизом `contour`, и «ни того ни другого» — тот же типизированный
    # KIR-P007. Взаимную обязательность реестр выразить не может, поэтому
    # ни одно из двух полей не помечено required, и корпусу нужен явный
    # выбор — ровно как у place_family выше.
    "create_ceiling": {"outline": [[0.0, 0.0], [6000.0, 0.0],
                                   [6000.0, 4000.0], [0.0, 4000.0]]},
    # create_stairs (09.08): марш задаётся ЛИБО прямыми концами, ЛИБО
    # винтом `spiral`, и «ни того ни другого» — тот же типизированный
    # KIR-P007. Ни одно из полей не помечено required именно потому, что
    # обязательность ВЗАИМНА, и корпусу нужен явный выбор.
    "create_stairs": {"p0_mm": [0.0, 0.0], "p1_mm": [5000.0, 0.0]},
    # create_extrusion_roof (09.08): `start_mm` и `end_mm` — не два
    # независимых числа, а КОНЦЫ ОТРЕЗКА вдоль нормали рабочей плоскости.
    # Общая таблица образцов даёт роду `num` одно и то же значение, то есть
    # ход НУЛЕВОЙ длины, и компилятор законно отказывает KIR-T002. Причина
    # та же, по которой `_BY_PARAM` вообще существует: вырожденный образец
    # проверяет не то, что задумано.
    "create_extrusion_roof": {"start_mm": 0.0, "end_mm": 12000.0},
    # create_face_wall (10.08): `face_normal` — род `pt_xyz`, но НЕ точка:
    # это НАПРАВЛЕНИЕ, а общий образец рода даёт нулевой вектор, то есть
    # вырожденность ПО ОПРЕДЕЛЕНИЮ, и компилятор законно отказывает KIR-T002.
    # Причина та же, что у `create_extrusion_roof` строкой выше: образец,
    # вырожденный для СМЫСЛА параметра, проверяет не то, что задумано. Наклон
    # здесь не украшение — Revit строит стену по грани ТОЛЬКО по наклонной
    # (его собственное правило: нормаль не вертикальна и не горизонтальна).
...
def _args_for(ospec: spec.OpSpec) -> dict:
    ...
    for p in ospec.params:
        ...
        if p.name == "p1_mm":
            ...
            # ОДИН ИСТОЧНИК ОБРАЗЦА (09.08): `_sample` знает и про enum, и про
            # границы САМОГО параметра. Отдельная ветка enum здесь была вторым
            # домом той же таблицы и, что важнее, не умела отсекать число по
            # границам — на `sweep_deg` (1..360) корпус получал 1000.0 и
            # обвинял оп в том, что тот «не строит принимаемый JSON».
