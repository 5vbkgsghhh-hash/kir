...
def test_the_saving_is_real_and_large(flat: dict) -> None:
    ...
    Порог 2.5x назначен НИЖЕ замеренного (3.04x на 03.08) — тест сторожит
    регресс, а не фиксирует сегодняшнее число. Если вынос перестанет окупаться,
    честнее выбросить его целиком, чем возить `$defs` ради приличия.
    """
    stats = measure(flat)
...
def test_flat_schema_is_untouched_by_this_module(flat: dict) -> None:
    """Генератор остаётся плоским, пока переключатель не проверен на стенде.

    Открытый риск назван в шапке `schema_dedup.py`: неизвестно, читает ли
    рабочая модель `$defs`/`$ref` так же хорошо. Constrained decoding в проде
    выключен, схема едет текстом — значит вопрос про ПОНИМАНИЕ, и решает его
    A/B на `mission_bench`, а не наше удобство. Пока не решён — прод плоский,
    и этот тест не даёт переключить умолчание молча.
    """
    assert "$defs" not in flat, (
        "program_schema() начал отдавать схему со ссылками — это смена того, "
        "что видит модель; она допустима ТОЛЬКО после замера на стенде, и "
        "тогда этот тест правится осознанно вместе с ним")
