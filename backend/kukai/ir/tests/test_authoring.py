...
class QueryTypes(unittest.TestCase):
    ...
    def test_all_twenty_five_pools_compile_offline(self):
        ...
        27 -> 35 (12.08.2026): восемь пулов, в которые компилятор УМЕЛ ПИСАТЬ,
        не умея читать. Множество взято у РЕЕСТРА — `OpSpec.grounded` по всем
        опам минус эти choices, — а не из списка задач: прежняя запись называла
        ШЕСТЬ, потому что выводилась из нужд тридцати НЕПРОВЕРЕННЫХ опов, а
        `create_ceiling` и `create_railing` давно проверены и в тот срез не
        попадали. Данные снимок собирал уже, идиому коллектора каждого — тоже
        (`open_model.__AddPool`), описание инструмента не выросло ни на символ.

        И ЭТОТ ТЕСТ ПОЙМАЛ РОВНО ТО, РАДИ ЧЕГО ЧИСЛО ДЕРЖИТСЯ РУКАМИ. Волна
        пулов гоняла только `test_tool_doc`, `test_any_query`, `test_schema_*`
        и `test_grounded_program` — `test_authoring` в тот срез не входил, и
        расхождение 35 против 27 всплыло лишь на ПОЛНОМ наборе, тремя часами
        позже. Радиус изменения — граф импорта, а не подкаталог, и цена этого
        урока здесь ровно одна строка."""
