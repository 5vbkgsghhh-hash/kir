...
class ToolDescriptionCoversRegistry(unittest.TestCase):
    ...
    def test_description_stays_small_next_to_the_schema(self):
        ...
        """
        # 🔴 ПОТОЛОК НЕ ПОДНЯТ, И ЭТО РЕШЕНИЕ ДИРЕКТОРА 15.08.2026.
        #
        # Он был ПРОБИТ накоплением: восемь способностей приехали в постоянный
        # текст за один день, и замер дал 30 427 при потолке 30 000. Первым
        # ходом я поднял потолок до 30 600 с арифметикой окупаемости — и это
        # был неверный ход: потолок платится КАЖДЫМ обращением, и поднять его
        # значит переложить нашу неаккуратность на бюджет мышления модели.
        #
        # Вместо подъёма — ПЕРЕРАСПРЕДЕЛЕНИЕ по правилу «в постоянном тексте
        # только то, без чего выберут не ту форму»: шесть ситуационных приёмов
        # §4 уехали в `course("приёмы")`, три решающих форму остались.
        # Замер: 30 427 -> 28 361 (-2 066), все восемь способностей названы,
        # контроль на результат зелёный (14 фактов из 14).
