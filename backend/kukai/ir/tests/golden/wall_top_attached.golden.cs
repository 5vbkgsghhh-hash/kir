...
using (Transaction __t = new Transaction(doc, "KIR: стена, приаттаченная к верхнему уровню"))
{
    try
    {
        ...
        if (__lv_WT == null) { __t.RollBack(); return __Refuse("WT", (__lv_raw_WT == null ? "уровень не найден (модель изменилась после grounding)" : "id уровня резолвится не в Level, а в " + __lv_raw_WT.GetType().Name + " — причина (дрейф модели или неверный id) не определена рантаймом")); }
