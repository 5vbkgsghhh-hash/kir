...
using (Transaction __t = new Transaction(doc, "KIR: дуговая стена (скруглённый угол r325)"))
{
    try
    {
        ...
        if (__lv_WA == null) { __t.RollBack(); return __Refuse("WA", (__lv_raw_WA == null ? "уровень не найден (модель изменилась после grounding)" : "id уровня резолвится не в Level, а в " + __lv_raw_WA.GetType().Name + " — причина (дрейф модели или неверный id) не определена рантаймом")); }
