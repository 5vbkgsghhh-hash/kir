...
using (Transaction __t = new Transaction(doc, "KIR: дверь с зеркалом и створкой"))
{
    try
    {
        ...
        try { Parameter __cm = __el_W1.get_Parameter(BuiltInParameter.ALL_MODEL_INSTANCE_COMMENTS); if (__cm != null && !__cm.IsReadOnly) __cm.Set("kir:fbe2471b:W1"); } catch { }
        ...
        XYZ __pt_D1 = new XYZ(U(2000.0), U(0.0), __hl_D1.Elevation + U(0.0));
        ...
        XYZ __pt_Win1 = new XYZ(U(5000.0), U(0.0), __hl_Win1.Elevation + U(900.0));
        ...
        // post D1
        {
            ...
            if (__el_D1.Mirrored != true)
                __post.Add("D1: mirrored state mismatch (semantic)");
            if (__el_D1.HandFlipped != true)
                __post.Add("D1: hand flip state mismatch (semantic)"
                    + (__el_D1.CanFlipHand ? "" : " — семейство не допускает флипа"));
            if (__el_D1.FacingFlipped != false)
                __post.Add("D1: facing flip state mismatch (semantic)"
                    + (__el_D1.CanFlipFacing ? "" : " — семейство не допускает флипа"));
        }
        // post Win1
        {
            ...
            if (__el_Win1.FacingFlipped != true)
                __post.Add("Win1: facing flip state mismatch (semantic)"
                    + (__el_Win1.CanFlipFacing ? "" : " — семейство не допускает флипа"));
        }
        if (__post.Count > 0)
        {
            __t.RollBack();
...
// witness D1
{
    ...
    try { var __fi2 = __el_D1 as FamilyInstance;
        if (__fi2 != null) {
            __rb["mirrored"] = __fi2.Mirrored;
            __rb["hand_flipped"] = __fi2.HandFlipped;
            __rb["facing_flipped"] = __fi2.FacingFlipped;
        } } catch { }
    __results["D1"] = __rb;
}
...
// witness Win1
{
    ...
    try { var __fi2 = __el_Win1 as FamilyInstance;
        if (__fi2 != null) {
            __rb["mirrored"] = __fi2.Mirrored;
            __rb["hand_flipped"] = __fi2.HandFlipped;
            __rb["facing_flipped"] = __fi2.FacingFlipped;
