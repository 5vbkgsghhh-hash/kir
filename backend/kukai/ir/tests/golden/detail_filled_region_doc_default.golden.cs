...
double[] __fu0_F1 = new double[] { 500.0, 3500.0, 3500.0, 500.0 };
double[] __fv0_F1 = new double[] { 500.0, 500.0, 1700.0, 1700.0 };
double[] __fum_F1 = new double[] { 2000.0, 3500.0, 2000.0, 500.0 };
double[] __fvm_F1 = new double[] { 500.0, 1100.0, 1700.0, 1100.0 };
double[] __fu1_F1 = new double[] { 3500.0, 3500.0, 500.0, 500.0 };
double[] __fv1_F1 = new double[] { 500.0, 1700.0, 1700.0, 500.0 };
using (Transaction __t = new Transaction(doc, "KIR: прямоугольная заливка типом документа по умолчанию"))
{
    try
    {
        ...
        __frt_F1 = doc.GetElement(doc.GetDefaultElementTypeId(ElementTypeGroup.FilledRegionType)) as FilledRegionType;
        if (__frt_F1 == null) { __t.RollBack(); return __Refuse("F1", "в документе нет типа заливки по умолчанию — назовите type"); }
        ...
        __ol_F1.Append(Line.CreateBound(__vp_F1(500.0, 500.0), __vp_F1(3500.0, 500.0)));
        __ol_F1.Append(Line.CreateBound(__vp_F1(3500.0, 500.0), __vp_F1(3500.0, 1700.0)));
        __ol_F1.Append(Line.CreateBound(__vp_F1(3500.0, 1700.0), __vp_F1(500.0, 1700.0)));
        __ol_F1.Append(Line.CreateBound(__vp_F1(500.0, 1700.0), __vp_F1(500.0, 500.0)));
        ...
        try { Parameter __cm = __el_F1.get_Parameter(BuiltInParameter.ALL_MODEL_INSTANCE_COMMENTS); if (__cm != null && !__cm.IsReadOnly) __cm.Set("kir:2e6f5ba4:F1"); } catch { }
        ...
        // post F1
        {
            ...
            try
            {
                foreach (CurveLoop __frCl_F1 in __el_F1.GetBoundaries())
                {
                    ...
                    foreach (Curve __frCv_F1 in __frCl_F1)
                    {
                        ...
                        for (int __frK_F1 = 0; __frK_F1 < 4; __frK_F1++)
                        {
            ...
            for (int __frJ_F1 = 0; __frJ_F1 < 4; __frJ_F1++)
                if (__frHit_F1[__frJ_F1] != 1) __frExact_F1 = false;
            if (!__frRead_F1)
                __post.Add("F1: граница заливки нечитаема — GetBoundaries бросил (geometry)");
            else if (__frLoops_F1 != 1 || __frCurves_F1 != 4)
                __post.Add("F1: прочитано " + __frLoops_F1 + " петель / " + __frCurves_F1 + " рёбер вместо 1/4 (geometry)");
