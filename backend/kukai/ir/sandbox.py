...
class SandboxPolicy:
    ...
    #: склеенные без собственной семантики. Язык от склейки не меняется:
    #: скрипт, написанный под `kukai.ir.dsl`, работает здесь без единой правки
    #: — объекты функций те же.
    #:
    #: **СОСТАВ ЗДЕСЬ НЕ ПЕРЕЧИСЛЯЕТСЯ, И ЭТО РЕШЕНИЕ, А НЕ ЛЕНЬ.** Строка
    #: врала дважды: сперва называла четыре имени при шести, потом семь при
    #: восьми (`phase` подключили и не дописали). Перечень, живущий копией
    #: рядом с оригиналом, расходится с ним всегда — вопрос только в том,
    #: заметят ли. Спрашивай `course.SANDBOX_NAMES` — там он один и по
    #: определению верен; сколько их сейчас, печатает
    #: `tests/test_sandbox_names_are_declared.py`, который ещё и требует,
    #: чтобы каждое имя было либо описано модели, либо объявлено тёмным с
    #: причиной.
    #:
