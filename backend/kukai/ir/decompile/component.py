...
def build_library(
    ...
    *,
    ...
    # ЗАЯВКА СТОЯЛА НЕ НА ТОМ КЛЮЧЕ (2026-08-13).
    #
    # `claimed_paths` заявляет ПУТИ в дереве, а столкновение происходит в
    # АБСОЛЮТНОМ ОПЕ: два экземпляра честно берут РАЗНЫЕ узлы и порождают ОДИН
    # И ТОТ ЖЕ абсолютный элемент. Заявка на пути этого не видит по построению —
    # это наша именованная форма дословно: множество входов заявки обязано быть
    # РОВНО множеством того, что меняет ответ, а здесь оно покрывало слишком
    # мало.
    #
    # ЗАМЕРЕНО на `k2_ar_rd_v7` до правки: развёртка 115 979 против 115 880
    # листьев источника — 99 лишних, недостачи 0. Все 99 в одной категории
    # (`OST_RoomSeparationLines`), каждый порождён РОВНО двумя экземплярами,
    # четырьмя парами компонентов. На `v6` — том же здании, прочитанном без
    # этой категории, — избыток 0. Воспроизводится на 3 зданиях из 7.
    #
    # ЧТО ПРАВИЛО ЗАПРЕЩАЕТ, А ЧТО НЕТ. Оно запрещает не ДЕЛЕНИЕ ключа, а
    # ПРЕВЫШЕНИЕ кратности источника: измерено, что из 421 ключа, порождённого
    # двумя и более экземплярами, 322 ЗАКОННЫ (источник несёт столько же,
    # вплоть до кратностей 438 и 1 507), и ровно 99 избыточны — у всех 99
    # источник несёт элемент ОДИН раз.
    #
    # ИНВАРИАНТ, КОТОРЫЙ ЭТО УСТАНАВЛИВАЕТ: развёртка библиотеки обязана быть
    # РАЗБИЕНИЕМ листьев источника, а не покрытием. Это C-RT, сказанное
    # правилом ПОСТРОЕНИЯ, а не свойством, на которое надеются: свойство,
    # проверяемое после, ломается на масштабе (тест C-RT гоняется на сетках в
    # 1–4 этажа и держится; ложным оно становится на 115 880 листьях).
    #
    # НОЛЬ ПОСЛЕ ПРАВКИ ДВУСТОРОННИЙ, и второй стороне цена выше первой.
    # ИЗБЫТОК 0 — «мы не заявили лишнего», ровно то, что чинилось.
    # НЕДОСТАЧА 0 — «чиня избыток, мы не выбросили законное», и без неё правило
    # «отвергай при столкновении» неотличимо от правила «не дели вовсе»: оба
    # дают избыток 0, но второе стёрло бы 322 законных деления с кратностями
    # источника вплоть до 438 и 1 507. Оба конца замерены на всех 7 зданиях и
    # оба стоят в сторожах — `test_component_overclaim_fixture.py` держит
    # избыток и недостачу ОДНИМ утверждением и отдельно требует, чтобы после
    # правки деление между экземплярами ПРОДОЛЖАЛОСЬ (5 ключей на фикстуре).
    #
    # ЦЕНА НАЗВАНА ЧИСЛОМ, а не умолчана: отвергается ЭКЗЕМПЛЯР, но если после
    # отказа их осталось меньше `min_occurrences`, компонент уходит целиком —
    # на фикстуре 2 компонента / 6 размещений становятся 1 / 4, а листья
    # падают в одиночки тем же механизмом, что и у отвергнутых компонентов.
    source_abs = _abs_multiset(iter_l1_leaves(index.root.tree_node))
    claimed_abs: Counter[str] = Counter()

    def _fits(occ: Occurrence) -> bool:
        """Не превысит ли это вхождение кратность источника ни по одному опу."""
    ...
    for entry in repeats:
        ...
        # ОТВЕРГАЕТСЯ ЭКЗЕМПЛЯР, А НЕ КОМПОНЕНТ И НЕ ШАБЛОН, и это выбор с
        # названной ценой: отвергнуть КОМПОНЕНТ значило бы потерять все его
        # размещения ради одного стыка; урезать ШАБЛОН — сломать размещения,
        # где элемент НЕ делится ни с кем. Отвергнутый экземпляр стоит одного
        # размещения, а его листья падают в одиночки ТЕМ ЖЕ механизмом, что
        # уже работает у отвергнутых компонентов (пути не заявляются).
        #
        # Порядок обхода детерминирован (`repeats` и `ordered` отсортированы),
        # поэтому детерминирован и выбор того, КАКОЙ из двух экземпляров
        # уступает. Это назначенное правило, а не свойство данных: здание
        # подсказки не даёт — измерено, что наши компоненты совпадают с
        # группами автора в 88 случаях из 40 803 (0.22%).
...
def _multiset_fits(
    ...
    """Влезает ли ещё одно вхождение в кратность источника по КАЖДОМУ ключу.

    ЖИВЁТ НА УРОВНЕ МОДУЛЯ РАДИ КОНТРОЛЯ, и это не косметика. Правило C-RT
    держится тем, что этот предикат отвергает вхождение; доказать, что опыт МОГ
    кончиться иначе, можно только заглушив ЕГО и получив избыток на том же
    входе. Пока предикат был замыканием внутри `build_library`, единственным
    способом показать доправочное поведение оставалось переписать цикл в
    тесте — то есть проверять КОПИЮ алгоритма вместо него самого.

    `test_component_overclaim_fixture.py::TheGuardCanBeDisabled` глушит именно
    эту функцию и требует ровно тот избыток (10), который неправленый код даёт
    на той же фикстуре.
    """
