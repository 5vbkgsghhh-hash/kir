...
OPS = [
    OpSpec(
            ...
            post=("direct shape exists (materialized or typed refusal); "
                  ...
                  "on the ±0.5mm canon grid (geometry), OR — when that "
                  "strict grid comparison alone disagrees — every "
                  "corresponding vertex pair sits within ±0.1mm of "
            ...
            #
            # ТРЕТИЙ ДОПУСК — `surface_point_mm` (14.08.2026), И ОН НЕ ПРО
            # ОГРУБЛЕНИЕ РЕШЁТКИ. Строгая ступень выше ложно отвергала
            # геометрию, совпадающую нацело: вершина в 0.01 мм от границы
            # ячейки 0.5 мм перебрасывается в соседнюю ячейку внутренней
            # сваркой `TessellatedShapeBuilder`, и строка перестаёт совпадать,
            # хотя РЕАЛЬНОЕ расстояние — сотые доли миллиметра. Замер 14.08 на
            # Revit 2023, 24 реальных семейства: 13 ложных отказов из 24.
            # Точечный допуск 0.1 мм выбран так, что он (а) на порядок больше
            # наблюдённого зазора (0.01 мм), (б) на порядок меньше
            # обязательного контроля-FAIL (сдвиг 1 мм обязан по-прежнему
            # отвергаться, `test_witness_tolerance_still_refuses_a_real_defect`),
            # (в) заметно меньше уже объявленного допуска габарита того же
            # опа (`bbox_mm=5.0`). Ступень СРАБАТЫВАЕТ ТОЛЬКО когда строгая
            # уже сказала «нет» — она не заменяет решётку, а спасает от её
            # собственной границы.
            tolerances={
                "bbox_mm": 5.0, "surface_canon_mm": 0.5,
                "surface_point_mm": 0.1,
            },
        ),
]
