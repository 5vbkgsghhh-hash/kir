...
ПОДКЛЮЧЕНО 09.08.2026 — и подключено НЕ ЗДЕСЬ. `program_schema()` по-прежнему
отдаёт плоскую схему (один источник правды о языке), а вынос делает
`schema_transport.program_schema_for_tool()` на границе инструмента —
`serving.inject_revit_ir_schema`. Условие там ЗАМЕРЕНО, а не предположено:
`$ref` доезжает целым и считается вчетверо дешевле на обоих живых транспортах
(openrouter/deepseek 42 390→11 751 токен, openai/gpt-5.6-sol через CLIProxy
20 628→3 633, обе пары — HTTP 200), а на gemini/vertex litellm разворачивает
`$defs` обратно сам, поэтому там вынос не применяется: он не сэкономил бы
ничего.

ОТКРЫТЫЙ РИСК, НАЗВАННЫЙ ЧЕСТНО И ВСЁ ЕЩЁ ОТКРЫТЫЙ: держит ли рабочая модель
`$defs`/`$ref` так же хорошо, как плоскую схему. Constrained decoding в проде
ВЫКЛЮЧЕН — схема едет модели текстом, — поэтому речь про понимание, а не про
грамматику декодера. Замерено, что ссылки ДОЕЗЖАЮТ и что они ДЕШЕВЛЕ; НЕ
замерено, что модель по ним строит так же. Это A/B на `kir_dojo`/`mission_bench`,
и он теперь стоит одну переменную: `KUKAI_KIR_SCHEMA_DEDUP=0` — плоское плечо,
`=1` — свёрнутое, прочий прогон байт в байт тот же.
"""
