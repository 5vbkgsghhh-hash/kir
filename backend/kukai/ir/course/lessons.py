...
LESSONS: dict[str, tuple[str, object]] = {
    ...
    # ДОМ ВЫТЕСНЕННЫХ ЛИТЕРАЛОВ (09.08). Единственный урок, который не пишется
    # здесь, а СОБИРАЕТСЯ из объектов `skill.py`: разборы живут там вместе со
    # своей цепочкой рассуждения и своими тестами (компиляция рабочих,
    # отказ отказных), и второй экземпляр тех же программ разошёлся бы с
    # первым молча. Импорт ленивый: `skill` тянет `macros` и `compiler`, а
    # этот модуль зовут из песочницы.
    # ВТОРОЙ ДОМ ВЫТЕСНЕННОГО (15.08). §4 курса — ситуационный справочник —
    # уехал сюда, когда описание пробило потолок 30 000: он нужен ПОСЛЕ выбора
    # формы, то есть ровно тогда, когда его можно спросить. Собирается из
    # `skill.TECHNIQUES`, а не переписан: две копии приёмов разошлись бы молча.
    "приёмы": ("что делать, когда упёрся: адресация, хост, уровень, бюджет",
               lambda: __import__(
                   "kukai.ir.skill", fromlist=["skill"]
               ).build_techniques_text()),
    "разборы": ("программы четырёх разборов и пары «плохая/хорошая» целиком",
                lambda: __import__(
                    "kukai.ir.skill", fromlist=["skill"]
