...
#: Самый длинный контракт реестра сегодня печатается в 2 392 символа при
#: `LESSON_CAP` 3 300 (замер 09.08 по всем 41 опу), то есть ветка ниже дежурит
#: впрок; при ~120 опах она дежурной быть перестанет. Тест гоняет её с
#: уменьшенным потолком — иначе она проверялась бы рассуждением.
