...
PRECONDITIONS: dict[str, tuple[Callable, str]] = {
    ...
    # ЖИЛИЩЕ ОПОЗНАЁТСЯ ПО СОСТАВУ, А НЕ ПО ТОМУ, ЧТО КОМПОНЕНТА СВЯЗНА.
    #
    # Замер 15.08.2026, три входа, `derive_apartments` без единой правки:
    #
    #   эталон генератора (истина известна)   не-жилищ   0 из 20   → предусловие ДЕРЖИТ
    #   sob62_r23_v5                          не-жилищ  11 из 12   → НЕ держит
    #   k2_ar_rd_v15                          не-жилищ 621 из 995  → НЕ держит
    #
    # Оракул на здоровом входе даёт 100% по составу (20 из 20 точь-в-точь), то есть
    # записанные «0%» — свойство ВХОДА, а не алгоритма, и входы эти двух РАЗНЫХ родов:
    #
    #   * `sob62_r23_v5` — ДЕТСКИЙ САД («Групповая ячейка», «Буфетная», «ПУИ»,
    #     «ВРУ ДОО»). Девять «квартир» — это КАБИНЕТЫ: лексикон шлёт «кабинет» в
    #     ЖИЛАЯ, и врач получает жилище. Квартир там нет вовсе, и верный ответ —
    #     не «12 квартир», а «здесь не жильё»;
    #   * `k2_ar_rd_v15` — настоящий жилой дом, но 614 одиночек это «Жилая
    #     комната N», а 259 — «Кухня-ниша N» ОТДЕЛЬНО: у ниши нет ДВЕРИ в комнату,
    #     а вывод стоит на дверях. Открытая планировка режет квартиру на части.
    #
    # Предикат не порог и не калибровка: «жилище имеет кухню либо санузел» —
    # определение жилища, а не подобранное число. Компонента без обоих есть артефакт
    # вывода, а не квартира.
