...
def category_manifest() -> dict:
    """ЗАСЕВ манифеста — только когда файла нет. Дальше он живёт руками.

    🔴 ЭТА ФУНКЦИЯ ОБЕЩАЛА НЕЗАВИСИМОСТЬ, КОТОРОЙ У НЕЁ НЕТ, И ОБЕЩАЛА ЕЁ
    СОБСТВЕННОЙ ДОКСТРОКОЙ. Прежний текст гласил: «манифест — независимая
    сторона равенства, и меняется он только руками», — а функция читает
    `H.coverage_matrix()`, то есть ТУ ЖЕ `KIND_TABLE`, против которой её
    сравнивает `test_12_category_manifest_is_frozen_and_counted`, и `main()`
    перезаписывал ею файл при каждом прогоне генератора.

    То есть заморозка держалась ровно до первого запуска `make_fixtures` —
    после него обе стороны равенства читают один источник, и тест зелен ПО
    ПОСТРОЕНИЮ при любой ошибочной правке таблицы. Это в точности тот дефект,
    ради которого тест написан, внутри самого теста.

    Найдено 14.08.2026 при допуске полосы стене: генератор молча переписал
    ручную правку манифеста, и «моя намеренная правка одной строки» стала
    неотличима от случайной.

    Поэтому: засев — да (иначе первый запуск в свежем дереве упирается в
    отсутствие файла), перезапись — нет. Строка меняется руками, и тогда её
    видно в диффе.
    """
    rows = {}
